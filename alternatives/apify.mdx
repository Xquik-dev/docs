...
For account backfills, pass a Search Terms value such as `from:xquikcom since:2026-05-01 until:2026-05-02`, or use the structured Actor fields `from`, `since`, `until`, `since_time`, and `until_time`. The X Tweet Scraper Actor applies the account and time bounds before writing dataset rows, and `maxItems` still caps the run or each `searchTerms` entry.
