...
| Product type | Monitored-account X/Twitter WebSocket alert service for trading workflows. | X data, write, monitor, webhook, and export platform. |
| Coverage | WebSocket tweet events, account events, OCR text, detected crypto assets, market enrichment, history replay, and Discord delivery on supported plans. | 47 dashboard tools, 126 REST operations, 23 extraction tools, 17 X write actions, account and keyword monitors, webhooks, giveaway draws, Radar, 10 SDKs, MCP, and pay-per-use reads. |
