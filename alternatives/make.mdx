...
| Product type | Visual automation platform with apps, scenarios, HTTP requests, custom apps, and AI automation features. | X data, write, monitor, webhook, export, dashboard, REST API, SDK, and MCP platform. |
| X/Twitter path | Use Make's HTTP app or a private custom app to call a focused X service, then route the result through Make modules. | Start with a dashboard tool, REST endpoint, SDK call, export, webhook subscription, or MCP tool for a defined X task. |
...
| Cost model | Make pricing is based on credits, plan features, active scenarios, schedule intervals, file sizes, and execution limits. | Starter is USD 20/month with 140,000 included credits. Top-ups are USD 0.00015/credit, webhook management is free, and active monitors bill only while enabled. |
