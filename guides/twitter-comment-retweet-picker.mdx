...
description: "Build Twitter comment, retweet, and hashtag picker entry lists from reply authors and retweeters. Preserve cursors, user IDs, filters, and selection proof."
keywords:
  [
...
Then pass eligible participant IDs into one winner-selection draw.
...
Then create one draw to randomly select winners.
Send eligible IDs to the [Twitter giveaway picker](/guides/twitter-giveaway-picker).
Store giveaway rules beside the eligible user IDs.
Use that stored draw to pick a random winner from eligible IDs.
Do not randomly pick again when a client loses its response.
Search draw history before retrying an uncertain request.
...
8. Send only eligible IDs into [Create Draw](/api-reference/draws/create).
