...
- `allowed`: whether the current balance can fund the job.
- `source`: the count source, such as `replyCount` or `resultsLimit`.
- `estimatedResults`: the expected row count.
- `creditsRequired` and `creditsAvailable`: the projected cost and balance.
...
Xquik converts them to X search operators internally. Omit every filter the
caller did not request. A topic plus a like minimum includes every language,
reply, repost, and quote. A minimum alone does not request `queryType=Top`.
