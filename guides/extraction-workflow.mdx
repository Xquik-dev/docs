...
```text
Check Credits → Estimate Cost → Run Extraction → Get Results → Export (optional)
```
...
| Consumer | Recommended handoff | Fields to store | Notes |
|----------|---------------------|-----------------|-------|
| App or API pipeline | `GET /extractions/{id}` | `job`, `results`, `hasMore`, `nextCursor` | Use `limit` up to 1,000 and pass `nextCursor` as `after` |
| CRM import | `GET /extractions/{id}/export?format=csv` | `User ID`, `Username`, `Display Name`, `Followers`, `Verified` | Upsert by stable X user ID when possible |
| Warehouse or queue | `GET /extractions/{id}/export?format=json` or paginated JSON | `xUserId`, `xUsername`, `tweetId`, `tweetText`, `createdAt` | Keep the extraction ID with each load for replay and audit |
| Analyst review | `GET /extractions/{id}/export?format=xlsx` | Export columns plus enrichment fields | Use CSV/JSON for automation and XLSX for manual review |
