...
- **Launch check:** Can your integration platform support clear API documentation and production ready rollback steps?
...
Xquik MCP list and search results use `has_more` and `next_cursor`. Pass `cursor` for X resources. Pass `after` for events and extractions.
...
Use `GET /api/v1/events` with `after`. Store event ID, type, monitor ID, time, `has_more`, and `next_cursor`.
