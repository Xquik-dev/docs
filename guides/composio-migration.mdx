...
| **Credentials** | 6 (API Key, API Secret, Bearer Token, OAuth Client ID, OAuth Client Secret, Composio API Key) | 0 manual credentials with MCP OAuth; 1 API key as a fallback |
| **Setup steps** | 14 | 2 |
| **OAuth flow** | Browser redirect plus manually configured X credentials | Automatic Xquik login and consent |
...
<Steps>
  ...
  <Step title="Authorize Xquik">
    ...
    browser. CIMD or DCR handles client registration automatically.
  </Step>
</Steps>

Use an API key only for programmatic clients that cannot complete OAuth. Store
the key in a secret manager and send it as a Bearer header.
...
server = MCPServerStreamableHTTP(
    "https://xquik.com/mcp",
    headers={"Authorization": f"Bearer {os.environ['XQUIK_API_KEY']}"},
)
