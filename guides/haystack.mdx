...
Haystack is an open source framework for Python AI applications. Use [`xquik-haystack`](https://github.com/Xquik-dev/xquik-haystack) for current tweets. This Haystack AI framework returns typed `Document` objects with text, authors, metrics, and URLs.
...
Choose components when the workflow already uses `Document` objects. They normalize tweet text, metadata, URLs, and pagination.
...
Pin `haystack-ai==3.0.0` while testing existing components. Check the [official migration guide](https://docs.haystack.deepset.ai/docs/migration) for other changes.
...
No. Run the open-source components inside your existing Haystack environment.
