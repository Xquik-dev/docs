...
These components are building blocks for pipelines and agents. Wrap `XquikTweetSearch` with `ComponentTool` when an agent needs a search tool called `search_current_tweets`.
...
Use components when your pipeline already returns `Document` objects. They normalize tweet text, metadata, URLs, and pagination.
