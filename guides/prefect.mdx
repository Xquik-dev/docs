...
Use [`prefect-xquik`](https://github.com/Xquik-dev/prefect-xquik) for repeatable tweet searches, profile lookups, timeline refreshes, and trend checks. Use it for scheduled Twitter API reads in Python. You get typed credentials and six read-only Prefect tasks. This collection is not a general Prefect Python SDK.
...
Use environment variables for a local Prefect profile or server. Store Xquik keys in `XquikCredentials` for deployed flows.
...
Use a separate Prefect concurrency limit for in-flight task runs. Concurrency limits cap active work, not request frequency. Keep retry policies aligned with [Xquik rate-limit guidance](/guides/rate-limits).
...
Choose Prefect for dynamic Python control flow and task retries. Choose Airflow when your team already runs DAG-based schedules. Either system still needs cursor, idempotency, and rate-limit controls.
