---
title: "Build a Twitter Search Pipeline in Python With Prefect"
sidebarTitle: "Prefect"
description: "Build scheduled Twitter search, profile, timeline, and trend workflows in Python with Prefect. Paginate tweet and user pages, then retry failures."
keywords: ["Twitter search pipeline Python", "Twitter API workflow Python", "Twitter API Python", "tweet search Python", "Prefect", "workflow orchestration", "X API", "Xquik", "pagination", "retries"]
---
...
Use the [`prefect-xquik`](https://github.com/Xquik-dev/prefect-xquik) collection for automated Twitter API workflows in Python. Schedule tweet search, tweet lookup, profile search, or profile lookup. Add timeline refreshes and regional trend checks.
...
<CardGroup cols={2}>
  ...
  <Card title="API Contract" icon="braces">
...
<CardGroup cols={2}>
  ...
  <Card title="Returns" icon="braces">
...
When a Prefect flow needs writes, monitors, webhooks, event replay, or extraction jobs, keep the `prefect-xquik` task scope focused on reads. Call REST, a generated SDK, or MCP from a follow-up task, then normalize REST camelCase fields to compact snake_case handoff rows before downstream retries.
...
<CardGroup cols={2}>
  ...
  <Card title="Billing" icon="credit-card">
    Tweet, profile, follower, reply, timeline, community, and list reads use credits. Handle `402 no_subscription`, `402 no_credits`, and `402 insufficient_credits` before retrying.
  </Card>
  <Card title="Rate Limits" icon="timer">
    Read endpoints share a 300 per 1s user bucket. Retry after `Retry-After` elapses.
  </Card>
  <Card title="Raw JSON" icon="braces">
