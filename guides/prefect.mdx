...
<CardGroup cols={2}>
  ...
  <Card title="Downstream Rows" icon="table">
    Normalize raw dictionaries in a follow-up task before writing to Slack, Sheets, a warehouse, or a dashboard. Keep `tweet_rows` with `id`, `text`, `author.username`, `createdAt`, and `url`; `user_rows` with `id`, `username`, `name`, and `followers`; and `trend_rows` with `name`, `rank`, `query`, and `description`.
  </Card>
</CardGroup>
