...
PyPI does not currently publish a `prefect-xquik` distribution. Tag `v0.1.7` supports published Prefect 3 releases. It also uses the canonical Xquik API base URL. Tag `v0.1.8` only changes package maintenance. It also requires newer Prefect releases.
...
Use the standard upgrade command only inside a separate compatibility sandbox.
...
Run `pip install -U prefect` only for that sandbox. Run `prefect server start` for a local Prefect server.
...
Prefect runs ordinary Python functions. The `@flow` decorator creates a tracked flow run. The `@task` decorator creates a task with retries and state.
...
Environment variables can select a local Prefect profile or server. Store Xquik keys in `XquikCredentials` for deployed flows.
...
Use focused searches to reduce irrelevant rows and review work.
...
Use one limit across tweet search, profile lookup, timelines, and trends. Share that limit across tasks using the same account capacity. Slow low-priority refreshes before delaying interactive reads.
...
Do not pass raw dictionaries directly into alerts or generators. Define compact row contracts first. Stable row contracts isolate consumers from new response fields.

## Prefect Collection or Direct Xquik API

Choose `prefect-xquik` for its six supported reads. It provides blocks, async calls, validation, and task metadata.
...
It builds a Prefect Python pipeline for scheduled Twitter searches and timeline reads. The flow preserves tweets, cursors, time windows, and documented errors.
...
No. It is a focused Prefect Python library with six asynchronous read tasks. Use Xquik SDKs when a workflow needs broader REST coverage.
...
The [prefect-xquik repository](https://github.com/Xquik-dev/prefect-xquik) contains the package, examples, tests, and release tags. Review source changes before upgrading a pinned deployment.

### Python Prefect vs Airflow: Which Fits Twitter Search?

Prefect fits dynamic Python control flow and task-level retries. Airflow fits teams already operating DAG-based scheduling infrastructure. Either system still needs cursor, idempotency, and rate-limit controls.
...
Store one row per tweet ID. Save time windows and cursors after each committed page.
