...
MCP clients then translate model tool calls into authenticated HTTP requests.
...
Your model provider may require an OpenAI API key or another model credential.
Keep that credential separate from the Xquik key. Never send either credential
through prompts, user context, tool arguments, or saved handoffs.
...
Every API call follows the documented Xquik route and response contract.
