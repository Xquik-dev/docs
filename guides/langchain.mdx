---
title: "LangChain Integration for X API"
sidebarTitle: "LangChain"
description: "Connect LangChain agents to tweet search, profile lookup, follower export, monitors, webhooks, and X account actions through MCP."
...
## Choose LangChain for composable tool flows

Use LangChain when tweet tools must join retrievers, prompts, and application
services. Load the Xquik MCP tools once. Pass the resulting tools to the agent
that needs them.

Keep tool results outside conversational prose. Preserve tweet IDs, author
usernames, media URLs, and pagination cursors as structured values. Feed only
the relevant fields into later prompts.

Use LangGraph when the workflow needs checkpoints or approval branches. Place
tweet search before review. Place write actions after a human decision. Save
the approved text, connected account, target ID, and idempotency key in graph
state.

Prefix tools when several MCP servers expose similar names. This prevents an
agent from selecting the wrong search, profile, or publishing tool.
