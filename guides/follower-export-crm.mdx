...
Download a Twitter follower list as CSV, XLSX, or JSON. Keep stable user IDs,
usernames, bios, verification, profile images, and audience counts. Import the
rows into a CRM, spreadsheet, queue, or warehouse.
...
before starting. Keep the source username and resolved user ID. Poll the job
until completion.

<Accordion title="File Formats & Follower Fields">
Download CSV for CRM lists or spreadsheet imports. Choose
XLSX for analyst review and JSON for queues. Store one profile per user ID.
...
username and `follower_explorer`. Set `resultsLimit` before creating a large
job. Use `GET /api/v1/x/users/{id}/followers` for current profile pages. That
route returns `users`, `has_next_page`, and `next_cursor`.

<Accordion title="Cursor, API Key & Rate-Limit Rules">
Treat `next_cursor` as opaque. Persist every follower page before saving its cursor. Keep the API
key outside exported lists. Handle each documented rate limit and error status.
Deduplicate retries by user ID. Save the collection time.
</Accordion>

### What API Can I Use to Get Someone’s Twitter Followers?

Resolve a public profile to its stable user ID. Then use the
[live Followers API contract](https://docs.xquik.com/api-reference/x/followers)
for current follower pages. Use `follower_explorer` for a saved export or list.

<Accordion title="Public Profile Boundaries">
A private account does not make its followers public. Never infer private
profiles from a missing page. Save each username, user ID, cursor, and
collection time. Request another page only after the current profiles become
durable. Respect API key scope, credits, and every returned rate limit.
</Accordion>
...
Use the saved Twitter followers scraper for cost estimates and reusable files.
...
Both paths return user ID, username, bio, and verification. They can also return
follower counts, following counts, location, and profile images. Keep every API key
scoped. Respect the read rate limit. Store cursors with their source profile.
...
list before its opaque cursor. Deduplicate retries by numeric user ID. Store the
mutable username, bio, follower count, following count, and verification. Stop
at a documented limit. Never infer private followers from an incomplete page.
Record completion time.
</Accordion>

### Export List of Twitter Followers to CSV or Excel

Use `follower_explorer` to export a list of Twitter followers. Choose CSV for
CRM imports and Excel-compatible XLSX for analyst review. Choose JSON for
queues, warehouses, or apps.
...
cap beside the file. Save rejected import rows. Use paginated JSON when a
custom sync needs larger lists.
</Accordion>
...
This flow needs no customer cookies. Keep the API key outside source files.
Check `402`, `424`, `429`, and `502` before retrying. Normalize each profile by
user ID. Write one
follower per JSON Lines record. Store the username, list source, and collection
time.
</Accordion>
...
Repeated cursors can also create incomplete lists. Treat scraped data as public
follower profile rows.

<Accordion title="Follower Scraper Risk Controls">
Never infer consent from a bio. A private account never exposes private
followers. Do not infer inactive
...
Public data means public follower profiles and documented fields. Collect only
fields required by the social media workflow. Review the
[X Terms of Service](https://x.com/en/tos) at `https://x.com`.

<Accordion title="Responsible Export Checklist">
Follow applicable laws, retention rules, and deletion requests. Restrict every follower export to
its intended reviewers. Store the source profile, user ID, username, cursor,
...
API or CRM-ready CSV export. Use Xquik for repeatable public follower lists with
row caps. Keep the source user ID and username. Never request another account's
private archive. Record which source produced every downstream file.
</Accordion>
...
Use `GET /x/users/{id}/followers` for the newest follower page. Use this page
for a CRM enrichment job, queue worker, audience sync, or agent.
...
The response returns `users`, `has_next_page`, and `next_cursor`.
Treat `next_cursor` as opaque. Store it with the import checkpoint. When
`has_next_page` equals `true`, pass the cursor back unchanged.
...
`source_username` and `results_limit` for audit. Replace `status` with each
value returned by `GET /extractions/{id}`.
...
Export files after the API returns `status: completed`.

### Paginated JSON handoff

Use `GET /extractions/{id}` when an integration needs rows before file export.
The API returns camelCase JSON fields. Every result guarantees `id` and
...
Use `limit` up to `1000`. Pass `nextCursor` as `after` until `hasMore` becomes
`false`. Map `xUserId` to `x_user_id`. Map usernames, names, follower counts,
and verification to the CRM columns shown below. Add description and location
when needed.
...
Follow the CRM's unique-field rules. HubSpot accepts CSV, XLSX, and XLS imports.
Updates and deduplication need a unique identifier. Salesforce Bulk API 2.0
upserts use CSV data and an external ID field.
