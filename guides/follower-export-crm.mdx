...
Use `format=json` when a queue, warehouse, CRM sync, or agent needs one follower per line. The JSON export is keyed by API field names. Map `xUserId`, `xUsername`, and `xDisplayName` before writing downstream rows.
...
for (const follower of followers) {
  const row = {
    ...
    verified: follower.xVerified,
    profile_image_url: follower.xProfileImageUrl,
  };
