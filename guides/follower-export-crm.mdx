...
Use `format=json` when a queue, warehouse, CRM sync, or agent needs one follower per line. JSON export rows use API field names, so map each row before writing `xquik-follower-export.jsonl`.
