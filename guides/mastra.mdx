...
monitors. It preserves every tweet ID, profile ID, cursor, and job ID.
...
| X actions | `requireToolApproval` | Pause before the `xquik` tool runs |
| Tool errors | `onToolError: "throw"` | Preserve failed MCP results |
...
A Mastra AI MCP server can publish local tools. This guide connects Mastra's
MCP client to Xquik's Twitter MCP server.
...
Xquik exposes one `xquik` execution tool. It can run authorized reads and
...
The `explore` tool returns routes, methods, parameters, and response fields.
...
| `429` | The Twitter API rate limit applies | Preserve the cursor and wait |
| `502` | The X dependency returned an invalid response | Retry later |

Keep `onToolError: "throw"`. It preserves MCP `isError` failures. Never restart
pagination after `429`. Read each route before designing retry logic.

## Verify Mastra Package Compatibility

These stable versions were checked on August 3, 2026.
...
It connects a Mastra AI agent to a Model Context Protocol (MCP) server.
Xquik supplies tweet, follower, monitor, webhook, and write tools.
...
Yes. The Twitter follower scraper API returns paginated profile rows and
cursors. Use the [followers API](/api-reference/x/followers) for bounded pages.
Use extraction jobs for larger CSV, JSON, or XLSX exports.
