---
title: "Mastra Twitter Agent Guide"
description: "Build a Mastra TypeScript agent that searches tweets, looks up profiles, exports followers, monitors keywords, and calls approved X actions."
...
## Choose Mastra for TypeScript agent services

Use Mastra when the surrounding worker already runs on TypeScript. Keep Xquik
tool discovery inside the MCP client. Give the agent only the tools required
for its current task.

Represent handoffs with typed objects. Preserve tweet IDs, user IDs, queries,
and cursors as strings. Store media URLs separately from generated summaries.
This keeps later steps tied to the original tweet.

Use dynamic headers when each user brings a separate Xquik API key. Do not
share one credential across unrelated tenants. Create the toolset after the
request identity is known.

Stream explanatory text to the interface. Keep write-action results in a
durable job record. Require approval before posting tweets, sending direct
messages, or changing follows.
