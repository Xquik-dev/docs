...
This Pydantic AI MCP server connection uses the Xquik URL and `x-api-key`
header.
...
The agent discovers `explore` and `xquik`. Use endpoint discovery before an
unfamiliar route. The `xquik` tool runs sandbox code containing
`xquik.request(...)`. Authentication is injected outside that code.
...
exists. Also stop after the server repeats one. Return `cursor_stalled` with
the saved row count.
...
Wrap related calls in `async with agent`. This reuses one MCP session for
profile and tweet requests.
...
write-capable agent must review every Xquik API call. Leave `explore` available
without approval.
...
[LangChain and LangGraph](/guides/langchain) when durable graph orchestration
matters. Both can call the same Xquik MCP tools.

### Can Pydantic AI Call the Twitter API?

Yes. MCP exposes eligible tweet, profile, follower, monitor, extraction, and X
action routes. Authenticate with an Xquik API key.
...
Pass a `BaseModel` as the agent's `output_type`. Serialize validated output with
...
route omits that field. Use `Literal` for finite stop reasons. Avoid `str`
return values for durable handoffs. Validate before storage, queues, exports,
or another agent receives the handoff.
...
Use the matching X write route through `xquik`. Require approval for every API
...
missed events through `GET /api/v1/events` with `after` pagination. Monitor
