...
Xquik exposes all API calls through one aggregate `xquik` tool. Review and
...
Use the matching X write route through `xquik`. Review and approve every
proposed X write action. Store the idempotency key and returned action ID.
