...
Wrap related calls in `async with agent`. One session fetches profiles and
tweets in sequence.
...
Xquik exposes all API calls through one aggregate `xquik` tool. Review each
validated Xquik tool invocation before execution. Leave `explore` available
without approval.
...
route omits that field. Use `Literal` for finite stop reasons. Prefer a Pydantic
model over plain strings. Validate before storage, queues, exports, or handoff.
...
Use the matching X write route through `xquik`. Require approval before each X
write. Store the idempotency key and returned action ID.
