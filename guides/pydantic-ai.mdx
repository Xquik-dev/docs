...
Wrap related calls in `async with agent`. One session fetches profiles, then
fetches tweets.
...
Use the matching X write route through `xquik`. Approve each X write action
before execution. Store the idempotency key and returned action ID.
