...
Wrap related calls in `async with agent`. One session handles related profile
and tweet requests.
...
write-capable agent must review every Xquik request. Leave `explore` available
without approval.
...
storing each validated output. Do not print `result.output`; validate and store
it. Never save conversational text as JSON.
...
route omits that field. Use `Literal` for finite stop reasons. Never use a str
return for a durable handoff. It discards typed tweet IDs, cursors, and error
fields. Validate before storage, queues, exports, or another agent receives the
handoff.
...
request. Store the idempotency key and returned action ID.
