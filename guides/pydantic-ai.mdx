...
`xquik.request(...)` with validated route values. Authentication is injected
outside that code.
...
Wrap related calls in `async with agent`. This reuses one session while
fetching profiles and tweets.
...
storing each validated output. Never save conversational text as JSON.
...
route omits that field. Use `Literal` for finite stop reasons. A plain `str`
return discards typed tweet IDs, cursors, and error fields. Validate before
storage, queues, exports, or another agent receives the handoff.
