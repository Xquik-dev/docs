...
`MCP` is Pydantic AI's primary MCP entry point. It runs this connection locally
by default. Headers, hooks, and traces remain inside your Python process. The
URL selects Streamable HTTP. `allowed_tools` limits discovery to Xquik's two
public MCP tools.
...
users with different Xquik keys. Create a request-scoped capability for each
credential.
...
The CLI prompt displays every validated tool call. Replace it with your
...
MCP and REST solve different integration problems. MCP does not replace the
Twitter API routes. It gives an agent a discoverable tool layer over them.

| Requirement | Choose | Reason |
| --- | --- | --- |
| A model selects tweet or profile operations | Pydantic AI MCP | The agent discovers `explore` and `xquik` |
...
the final tweet, profile, follower, or monitor handoff. Keep both layers. They
...
Local Pydantic AI tools are Python functions registered by your application.
MCP tools come from a connected server and can change independently. Xquik
publishes `explore` and `xquik` through MCP.

### Is Pydantic AI Better Than LangChain for a Twitter Agent?

Choose Pydantic AI when typed Python handoffs are the primary boundary. Choose
[LangChain and LangGraph](/guides/langchain) for wider integrations and durable
graph state. Both can call the same Xquik MCP tools.
...
Model tweet IDs and cursors as strings. Use optional fields only when the route
can omit them. Use `Literal` for finite stop reasons. Validate before storage,
queues, exports, or another agent receives the handoff.
