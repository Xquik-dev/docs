---
title: "Error handling | X API Workflow Guide"
...
keywords: ["error codes", "retry", "exponential backoff", "rate limit", "error recovery", "write action", "safeToRetry", "pending confirmation"]
---
...
This section covers common recovery paths. The OpenAPI
...
Every write can return a durable action. Inspect the JSON lifecycle fields
before generic success or error handling.
...
For reads, retry only on `429` and `5xx` responses. Use `Retry-After`, then
...
<AccordionGroup>
  <Accordion title="Distinguish client errors from server errors">
    Do not retry a `4xx` response unchanged. Fix authentication, billing,
    permissions, or input first. Retry `429` and `5xx` only as described above.
  </Accordion>
