...
<AccordionGroup>
  <Accordion title="Check the error category">
  ...
  <Accordion title="Handle 409 gracefully">
    Reuse `monitor_already_exists`. For a busy cursor, follow `Retry-After` and
    retry once.
  </Accordion>
  <Accordion title="Set request timeouts">
    Set client timeouts. Poll status endpoints for long-running jobs.
  </Accordion>
  <Accordion title="Use idempotent operations">
    Duplicate monitor creation returns `409`. Deleting a missing resource returns `404`.
