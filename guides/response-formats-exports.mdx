...
Append each completed page to JSON Lines. Commit `nextCursor` only after the
page reaches durable storage.
...
`GET /api/v1/extractions/{id}`. Keep `after`, `hasMore`, and `nextCursor` in one
...
<CardGroup cols={2}>
  ...
  <Card title="Markdown" icon="file-text">
    Best for docs and text review. `md` returns a Markdown table.
...
`local_sha256` is calculated by your workflow after download. Xquik does not
...
resume with `after`. One file export stops at 100,000 rows.
