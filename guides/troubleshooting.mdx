...
Yes, for eligible reads. Choose the access boundary:

- **Guest wallet:** Prepay 33 eligible GET routes through a confirmed $10-$250 USD Stripe-hosted Payment Link.
- **MPP:** Pay 7 fixed-price GET operations per request without an account or API key.
- **Full account:** Use a subscription for writes, monitors, webhooks, extractions, draws, connected-account reads, and account management.

Subscribe from the [dashboard](https://xquik.com) billing page to unlock full account metered endpoints. Full account access activates after payment.
...
This workaround applies to affected Codex and Goose releases. They can stop
...
Goose substitutes `XQUIK_API_KEY` before sending the header. Remove the custom header configuration only after your Goose release preserves the authorization response issuer.
...
When the [upstream Codex issue](https://github.com/openai/codex/issues/31573) identifies a fixed release, upgrade Codex, remove `bearer_token_env_var`, and run `codex mcp login xquik` again.
