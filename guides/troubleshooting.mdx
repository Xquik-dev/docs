...
Supported OAuth clients include:

- Claude.ai, Claude Desktop, Claude Code, ChatGPT, Codex, Cursor, VS Code, Windsurf, OpenCode, and Gemini CLI
...
Modern clients should use CIMD. DCR remains available as a fallback. If a
URL-form `client_id` still fails, the client metadata URL must return public
HTTPS JSON, repeat its exact URL in `client_id`, and list the exact callback in
`redirect_uris`.
