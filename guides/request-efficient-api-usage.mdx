...
keywords: ["Twitter API pagination", "X API pagination", "Twitter cursor pagination", "Twitter API usage", "Twitter API endpoints", "Twitter search tweets", "get tweets by user API", "batch tweet lookup", "tweet scraper API", "Twitter API export"]
---
...
| Save a durable file | `POST /extractions` | `resultsLimit`, then `after` | Depends on the selected tool |
...
Duplicate user IDs preserve only their first position.
...
| Route family | Request parameter | Current range or behavior |
| --- | --- | --- |
...
<Steps>
  ...
  <Step title="Export">
    Download CSV, JSON, XLSX, JSONL, Markdown, XML, or PDF.
...
Stored extraction pages return `nextCursor` and accept `after`.
Events, draws, and Radar also accept `after`.
...
This TypeScript example preserves query intent across cursor pages.
...
Tweet search can request up to 200 results through `limit`.
