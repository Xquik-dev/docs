---
title: "CrewAI Twitter Agent Guide"
description: "Build a CrewAI research crew that searches tweets, looks up profiles, exports followers, summarizes replies, and monitors keywords through MCP."
...
## Choose CrewAI for role-based tweet work

Use CrewAI when separate agents own research, review, and reporting. Give each
agent a narrow role. A researcher can search tweets and profiles. A reviewer
can inspect replies and media. A reporter can save approved tweet IDs and
follower rows.

Keep task outputs concrete. Pass tweet IDs, user IDs, queries, and cursors
between agents. Do not pass an unstructured summary when the next task needs
the original tweet.

Set write tools apart from read tools. Require explicit approval before likes,
reposts, follows, direct messages, or published tweets. Keep the connected
account visible in every write task.

Use a sequential crew when one result becomes the next input. Use hierarchical
delegation only when a manager agent must choose among independent specialists.
...
Restrict an agent to endpoint discovery without API execution:

```python
from crewai.mcp import MCPServerHTTP
from crewai.mcp.filters import create_static_tool_filter

# Discovery only: cannot execute read or write API operations
researcher = Agent(
    role="API Catalog Researcher",
    goal="Inspect Xquik endpoint schemas without executing API calls",
    backstory="Researcher who maps available API operations",
    mcps=[
        MCPServerHTTP(
            url="https://xquik.com/mcp",
            headers={"x-api-key": "xq_YOUR_KEY_HERE"},
