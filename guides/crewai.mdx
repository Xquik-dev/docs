...
CrewAI offers an agent framework for complex tasks with distinct agent roles. Xquik supplies Twitter API operations through `explore` and `xquik`.
...
Define the expected output before building the task. CrewAI validates the final handoff against its Pydantic model.
...
Use hierarchical delegation for independent specialists. Use sequential tasks for cursor-dependent agent collaboration.

## Apply CrewAI MCP Integration Patterns

A CrewAI agent with MCP server access should own one permission boundary. Teams of AI agents must not share write-capable keys.
...
The `xquik` tool runs every route allowed by its key. Never give write permissions to autonomous research crews. Use guest `paid_reads` for eligible GET routes. See [guest wallets](/guides/guest-wallets).
...
`human_input=True` reviews the result. `tools=[]` blocks execution. After approval, send one REST request. Never retry pending writes automatically.
...
<CardGroup cols={2}>
  ...
  <Card title="Monitor Events" icon="radio">
    Store `monitor_id`, `event_id`, `occurred_at`, `next_cursor`, and `after`.
  </Card>
