...
The current package version is `0.1.7`. The plugin name is `hermes-tweet`, and the Python entry point is `hermes-tweet = hermes_tweet`.
...
<CardGroup cols={2}>
  ...
  <Card title="Media Tweet or DM Action" icon="image">
    ...
    `media_ids`, then store `messageId`. Keep full DM bodies out of shared
