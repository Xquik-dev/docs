...
keywords: ["hermes tweet", "hermes agent", "hermes agent plugin", "hermes agent skills", "hermes twitter", "hermes agent twitter", "twitter api python", "tweet search agent", "twitter follower export", "x api alternative"]
---
...
Hermes Tweet is Xquik's native Hermes Agent plugin. It provides a structured X automation toolset.

The plugin adds catalog-guided Twitter tools to the Hermes Agent runtime.

The Python package includes read tools, optional action tools, and 2 slash commands. It also bundles a Hermes skill that teaches safe tool selection.
...
For a tweet research agent, summarize patterns after collecting the selected pages. Keep source tweet IDs beside each conclusion.
...
A follower export supports audience analysis and CRM enrichment. A following export reveals accounts that the target follows.
