...
<CardGroup cols={2}>
  ...
  <Card title="Tweet or reply write" icon="send">
    Store `tweet_id`, `charged`, and `charged_credits` on a `200` response. For `202 x_write_unconfirmed`, store `write_action_id`, `status`, `charged`, and `charged_credits`, then poll `GET /x/write-actions/{id}` before retrying.
  </Card>
