...
keywords: ["make twitter automation", "make.com twitter integration", "make webhook", "twitter api integration", "tweet search automation", "twitter monitor webhook"]
---
...
- A `400` response identifies invalid request fields. Fix the module mapping.
- A `401` response identifies invalid authentication. Replace the API key.
- A `402` response identifies billing requirements. Update the subscription first.
- A `404` response identifies a missing tweet, profile, monitor, or webhook.
- A `424` response identifies a dependency failure. Retry safe reads with backoff.
- A `429` response identifies rate limiting. Respect `Retry-After` before retrying.
- A `502` response identifies a temporary upstream failure. Retry safe reads cautiously.
