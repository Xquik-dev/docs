...
keywords: ["Twitter API rate limits", "Twitter API limits", "API rate limit", "API rate limit exceeded", "429 Too Many Requests", "Retry-After", "rate limit recovery", "Twitter rate limit", "API throttling"]
---
...
| --- | --- | --- |
| `429 rate_limit_exceeded` | Xquik request bucket was exhausted. | Wait for `Retry-After`, then retry once. |
...
<Steps>
  ...
  <Step title="Retry a Bounded Number">
    Stop after 3 attempts. Surface the final error to the caller.
  </Step>
</Steps>
