...
Running a giveaway without these checkpoints makes later review unreliable.
...
pass one rule and fail another. Preserve accepted and rejected states separately.
This lets an operator explain every inclusion and rejection later.
...
This prevents one successful relationship from satisfying every follow rule.
...
Save the original draw request and returned draw ID immediately.
...
the check. Preserve rejection reasons without exposing private operator notes.
...
Persist completed pages so retries never restart the whole campaign audit.
