...
For strict account date windows, `q=from:username` with `sinceTime` or
`untilTime` is optimized for account timeline completeness. Raw X tokens such
as `from:username since:2026-05-01 until:2026-05-02` use the same behavior. Use
`queryType=Latest` for chronological backfills. Add a keyword, hashtag, or
other operator when you need normal search ranking.
