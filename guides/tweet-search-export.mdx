...
Pass `next_cursor` back as `cursor` to fetch the next page. Omit `limit` for a
simple cursor-driven page loop. Pass `limit` when you want Xquik to collect up
to that count in one bounded request; if `has_next_page` is `true`, continue
with the same query, filters, `queryType`, `limit`, and `cursor`. Cost is 1
