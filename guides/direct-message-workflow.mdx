...
keywords: ["Twitter DM API", "Twitter API DM", "X direct message API", "Twitter direct message API", "send direct messages", "Twitter DM history API", "send DM with media", "Twitter DM automation", "direct message history"]
---
...
It stores each returned message ID for retries and audits.
...
This prevents invalid first-page cursor requests.
...
Use a participant account when reading private conversations.
A `422` means X rejected the unchanged message request.
...
Follow the published OpenAPI contract for supported message conversations.
