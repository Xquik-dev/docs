...
<CardGroup cols={2}>
  ...
  <Card title="Publish tweet or reply" icon="send">
    Post tweets, post tweet replies, and hand off public image URLs. Low setup effort.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="5. Post image tweets or replies" icon="image">
    Publishes media-backed tweets or replies from public image URLs without a separate upload step.
  ...
  <Card title="6. Send direct messages with returned IDs" icon="send">
    ...
    First endpoint: `GET /x/dm/{userId}/history`, then `POST /x/dm/{userId}`.
    Handoff: `messages`, `has_next_page`, `next_cursor`, `messageId`, and `success`.
    Cost model: 1 credit per message returned; 10 credits per DM send.
  </Card>
</CardGroup>
