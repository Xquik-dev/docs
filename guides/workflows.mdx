...
| Publish tweet or reply | Post tweets, post tweet replies, public image URL handoff | N/A | Low |
| Tweet composition | Draft generation, reply monitoring | N/A | Low |
...
| Tweet replies export | Tweet ID and optional `resultsLimit` | `POST /extractions/estimate` with `reply_extractor` | Reply author fields, reply tweet fields, engagement counts, and metadata | CSV, JSON, XLSX, JSONL, or moderation queue rows | 1 credit per reply returned or extracted |
...
| Priority | Workflow | Why it is valuable | First endpoint | Returned handoff | Cost model |
|----------|----------|--------------------|----------------|------------------|------------|
| 1 | Scrape tweets to CSV, JSON, or XLSX | Turns search intent into rows for research, lead lists, support triage, and AI retrieval | `POST /extractions/estimate` with `tweet_search_extractor` | Export file, paginated JSON, or direct `GET /x/tweets/search` page | 1 credit per tweet returned or extracted |
| 2 | Scrape tweet replies to CSV, JSON, or XLSX | Turns a post's conversation into moderation, support, giveaway, research, or AI review rows | `POST /extractions/estimate` with `reply_extractor` and `targetTweetId` | Export file, paginated JSON, JSONL rows, or direct `GET /x/tweets/{id}/replies` page | 1 credit per reply returned or extracted |
| 3 | Export followers to CRM or warehouse | Builds owned audience tables with stable X user IDs for import and upsert | `POST /extractions/estimate` with `follower_explorer` | CSV, JSON, XLSX, or CRM field map keyed by `x_user_id` | 1 credit per follower returned |
| 4 | Monitor tweets to signed webhooks | Delivers new posts, replies, quotes, and retweets to queues without polling | `POST /monitors`, then `POST /webhooks` | Signed payload with `deliveryId`, `streamEventId`, `eventType`, and tweet data | 21 credits per active monitor-hour; webhook delivery is included |
| 5 | Post image tweets or replies | Publishes media-backed tweets or replies from public image URLs without a separate upload step | `POST /x/tweets` with `media` | `tweetId`, `success`, original `media` URLs, and optional `reply_to_tweet_id` | 10 credits per post tweet call |
| 6 | Send direct messages with returned IDs | Lets support, sales, and agent systems read a conversation and store the outbound message ID | `GET /x/dm/{userId}/history`, then `POST /x/dm/{userId}` | `messages`, `has_next_page`, `next_cursor`, `messageId`, and `success` | 1 credit per message returned; 10 credits per DM send |

### 1. Scrape tweets to CSV, JSON, or XLSX

Use this when a user asks to scrape tweets, search tweets, build a tweet scraper, export hashtag results, or hand matching posts to an analyst. Estimate first, create the extraction job, poll it, then export the file format your downstream system expects. For tweet replies, use `reply_extractor` with `targetTweetId` and export the same CSV, JSON, or XLSX formats.
...
The extraction returns a job ID. `GET /extractions/{id}` returns `job`, `results`, `hasMore`, and `nextCursor`; `GET /extractions/{id}/export?format=csv` downloads the rows. For live pagination without a stored job, call `GET /x/tweets/search` with `q`, `limit`, and optional `cursor`; it returns `tweets`, `has_next_page`, and `next_cursor`.
...
Use this when an agent or app has public image URLs and needs to post a tweet or reply. Pass those URLs directly in the `media` array on `POST /x/tweets`. Do not call `POST /x/media` first for tweet posts; `POST /x/tweets` rejects `media_ids` with `400 unsupported_field`.
...
Use this when a support, sales, or agent workflow needs direct messages with audit trails. Look up the recipient user ID, read history when needed, then send the DM from a connected account.
...
For queue, warehouse, or agent handoff, write one normalized JSON object per delivery to `xquik-monitor-events.jsonl`:
