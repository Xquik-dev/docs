...
| 4 | Upload media before posting tweets | Converts generated images or local files into publishable tweet media | `POST /x/media`, then `POST /x/tweets` | `mediaUrl`, `mediaId`, `tweetId`, and `success` | 10 credits per media upload call plus 10 credits per post tweet call |
...
Use this when an agent or app generates an image, accepts a local file, or needs a media handoff before posting a tweet or reply. `POST /x/media` returns `mediaUrl` for tweet posts and `mediaId` for one-item DM `media_ids` arrays.
...
Pass the returned `mediaUrl` to `POST /x/tweets` as a `media` array. To post a reply, also pass `reply_to_tweet_id`; the response returns `tweetId` and `success`. Do not send `media_ids` to `POST /x/tweets`.
