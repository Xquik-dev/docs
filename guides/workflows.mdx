---
title: Workflows
description: X API workflow and endpoint finder for tweet lookup, tweet search, monitoring, signed webhooks, MCP agents, exports, post tweets, DMs, and media handoff.
keywords: ["X API workflows", "Twitter API workflows", "endpoint finder", "tweet lookup", "tweet search", "tweet monitoring", "signed webhooks", "follower exports", "post tweets", "post tweet replies", "media upload", "MCP agents"]
---
...
Start here when a user asks which Xquik endpoint to call. Pick the row that
matches the job, then open the API page for params, responses, and examples.

| Job | First API | Best handoff |
| --- | --- | --- |
| One tweet by ID | `GET /x/tweets/{id}` | Tweet row with media and metrics |
| Many known tweet IDs | `GET /x/tweets` | Batch response before single-tweet loops |
| Keyword or advanced search | `GET /x/tweets/search` | Cursor pages, or `tweet_search_extractor` exports |
| Profile timeline | `GET /x/users/{id}/tweets` | `cursor`, `includeReplies`, and `includeParentTweet` |
| Article body | `GET /x/articles/{tweetId}` | Body blocks, cover image, author, and not-found handling |
| Replies, quotes, or threads | `GET /x/tweets/{id}/replies` | Conversation rows from replies, quotes, or threads |
| Follower export | `POST /extractions/estimate` | `follower_explorer` estimate, job, JSON, CSV, JSON, or XLSX |
| Post tweet or reply | `POST /x/tweets` | `tweetId`, write status, credits, and public media URLs |
| Direct messages | `GET /x/dm/{userId}/history` | History, cursor state, outbound `messageId`, and success |
| Real-time monitoring | `POST /monitors` or `POST /monitors/keywords` | Signed webhook payloads and delivery IDs |
| AI agent handoff | `xquik.request(...)` | Normalized Xquik API results for MCP clients |
| Saved file exports | `GET /extractions/{id}/export` | CSV, JSON, XLSX, Markdown, PDF, or TXT |
...
<AccordionGroup>
  ...
  <Accordion title="Post tweets or replies">
    ...
    Cost check: 10 credits text-only, plus 2 credits per started MB across attached media.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="5. Post media tweets or replies" icon="image">
    Publishes media-backed tweets or replies from public image URLs or 1 public MP4 video URL up to 100 MB without a separate upload step.
    ...
    Cost model: 10 credits text-only, plus 2 credits per started MB across attached media.
  </Card>
