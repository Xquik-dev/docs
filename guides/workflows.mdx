...
| 2 | Export followers to CRM or warehouse | Builds owned audience tables with stable X user IDs for import and upsert | `POST /extractions/estimate` with `follower_explorer` | CSV, JSON, XLSX, or CRM field map keyed by `x_user_id` | 1 credit per follower returned |
| 3 | Monitor tweets to signed webhooks | Delivers new posts, replies, quotes, and retweets to queues without polling | `POST /monitors`, then `POST /webhooks` | Signed payload with `deliveryId`, `streamEventId`, `eventType`, and tweet data | 21 credits per active monitor-hour; webhook delivery is included |
| 4 | Post image tweets or replies | Publishes media-backed tweets or replies from public image URLs without a separate upload step | `POST /x/tweets` with `media` | `tweetId`, `success`, original `media` URLs, and optional `reply_to_tweet_id` | 10 credits per post tweet call |
| 5 | Send direct messages with returned IDs | Lets support, sales, and agent systems read a conversation and store the outbound message ID | `GET /x/dm/{userId}/history`, then `POST /x/dm/{userId}` | `messages`, `has_next_page`, `next_cursor`, `messageId`, and `success` | 1 credit per message returned; 10 credits per DM send |
