---
title: "TweetClaw for OpenClaw Tweet Search & X Actions"
sidebarTitle: "TweetClaw"
description: "Install TweetClaw for OpenClaw to search tweets, inspect profiles, export followers, monitor X accounts, and run approved account actions. Follow exact steps."
keywords: ["tweetclaw", "openclaw plugin", "tweet search", "x api alternative", "twitter follower export"]
---
...
`@xquik/tweetclaw` is the official package. The plugin id is `tweetclaw`. The current published npm version and source-truth version are both `1.6.41`.
