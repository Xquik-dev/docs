...
```bash
openclaw plugins install @xquik/tweetclaw@1.6.29 --pin
```

`@xquik/tweetclaw` is the official package. The plugin id is `tweetclaw`. The current source-truth version is `1.6.29`.
...
| Symptom | Fix |
|---------|-----|
| `tweetclaw` is not available | Add `explore` and `tweetclaw` to `tools.alsoAllow`, then restart OpenClaw |
