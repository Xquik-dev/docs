...
Export Twitter replies as CSV, JSON, or XLSX rows. Use `reply_extractor`
for saved files with a defined result cap. Scrape Twitter replies through the
live API when an application needs current pages. Send reply authors, text,
media, and engagement counts downstream. Route rows to moderators, CRMs,
warehouses, queues, or agents.
...
These frequently asked questions explain how to export replies. Preserve the
original tweet, author, cursor, and engagement context.
