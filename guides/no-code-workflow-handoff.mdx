...
Map `eventType` before branching. Map `data.id`, `data.text`, and
`data.author.id`. Keyword events can include `query`. Account events can include
`username`.
...
Stop when `has_next_page` is false or no new IDs arrive. A repeated cursor
indicates a stalled workflow. Commit rows and `next_cursor` together. Upsert on
tweet ID or X user ID.
...
Store source IDs as strings. Keep API keys, endpoint signing values, raw request
bodies, raw signatures, and full headers out of workflow history.
...
Use these guides to automate workflows with provider-specific nodes. Each guide
covers credentials, state, retries, and its Twitter account handoff.
