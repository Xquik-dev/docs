...
<Tabs>
  <Tab title="Response types">
    ...
    interface Account {
      ...
      subscription: {
        status: "none" | "active" | "past_due" | "canceling" | "incomplete";
...
Returned by `GET /api/v1/account`. `subscription` reports status, tier, and the renewal or end date. `creditInfo` is omitted until the account has a balance record.
