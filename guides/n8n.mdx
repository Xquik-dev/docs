...
<CardGroup cols={2}>
  ...
  <Card title="Tweet or reply write" icon="send">
    Store `tweetId` on `200 Success`. For `202 x_write_unconfirmed`, store `writeActionId`, `status`, and `charged`, then poll `GET /x/write-actions/{id}` before retrying.
  </Card>
