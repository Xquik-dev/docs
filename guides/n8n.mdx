...
<CardGroup cols={2}>
  ...
  <Card title="Tweet or reply write" icon="send">
    Store `tweetId`, `charged`, and `chargedCredits` on `200 Success` as `tweet_id`, `charged`, and `charged_credits`. For `202 x_write_unconfirmed`, store `write_action_id`, `status`, `charged`, and `charged_credits`, then poll `GET /x/write-actions/{id}` before retrying.
  </Card>
