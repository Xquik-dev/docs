...
keywords: ["X API pricing", "Twitter API pricing", "subscription", "guest wallet", "credits", "unused credits carry over", "monitor tweets pricing", "real-time tweet monitor cost", "tweet alerts cost", "tweet search cost", "follower export cost", "top-up", "MPP pay-per-use"]
---
...
Account workflows use a subscription and shared credit balance. Accountless guest wallets prepay 33 eligible GET routes through Stripe. Anonymous MPP callers can pay per request on 7 fixed-price operations.
...
Subscribe from the [dashboard](https://xquik.com). Your subscription activates immediately and renews monthly. An active subscription is required for full authenticated metered API access. Guest wallets and MPP are separate accountless read paths.
...
Account users can explicitly confirm a [quick top-up](/api-reference/credits/quick-topup) that charges a saved payment method. Guest wallets always use a Stripe-hosted Payment Link through [guest top-up](/api-reference/guest-wallets/topup).
