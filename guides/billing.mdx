---
title: Xquik API Pricing, Credits & Billing
sidebarTitle: Pricing & Billing
description: Compare tweet search and follower export costs, then manage Xquik subscriptions, guest wallets, credits, top-ups, carry-over, and MPP pay-per-use.
keywords: ["Xquik API pricing", "Twitter API pricing", "Twitter API cost", "X API pricing", "subscription", "guest wallet", "credits", "unused credits carry over", "monitor tweets pricing", "1-second tweet monitor cost", "tweet alerts cost", "tweet search cost", "follower export cost", "top-up", "MPP pay-per-use"]
---
...
Use the [Twitter API alternative and pricing guide](/alternatives/x-api) to
compare current official X API costs with Xquik. This page documents Xquik
pricing for tweet search, follower exports, replies, profiles, timelines,
monitors, webhooks, and write actions.
...
hour and check every second. Stored events and webhook delivery are included.
...
> **Warning:** Metered calls are rejected when available credits cannot cover the operation. [Top up credits](/api-reference/credits/topup) or subscribe to continue.
...
| Active monitor | per monitor-hour | 21 |
...
- Use the [quick top-up endpoint](/api-reference/credits/quick-topup) to charge your saved payment method
...
3. After confirmation, use quick top-up only when the account already has a saved payment method:
...
Guest wallets prepay the 33 eligible GET routes without an account. After the user confirms $10-$250 USD, `POST /api/v1/guest-wallets` creates a one-use Stripe-hosted Payment Link and returns a `paid_reads` key. Creation does not charge. The key stays inactive until a verified Stripe webhook. A guest `402` offers only `POST /api/v1/guest-wallets/topups`. See the [guest wallet guide](/guides/guest-wallets).

## Pay-per-use (MPP)

Seven fixed-price read operations accept direct [MPP](/mpp/overview) payments. No subscription is required. Media downloads require full account authentication because they create account-tied gallery links.

### MPP per-call pricing

Use [MPP overview](/mpp/overview#eligible-endpoints) for the complete 7-operation list. Direct MPP uses fixed `charge` pricing:
...
Open [subscription settings](https://dashboard.xquik.com/en/account?tab=subscription), select **Manage Plan**, and cancel in Stripe's portal.
