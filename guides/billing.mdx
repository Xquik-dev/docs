...
<CardGroup cols={2}>
  ...
  <Card title="Post tweets or replies" icon="send">
    Use [Create tweet](/api-reference/x-write/create-tweet). Send public media URLs in `media` when posting media. Cost: 10 credits text-only, plus 2 credits per started MB across attached media.
  </Card>
...
| [Create tweet](/api-reference/x-write/create-tweet) | text-only call | 10 |
...
31 X-API read-only endpoints accept [Machine Payments Protocol](/mpp/overview) payments. No subscription required. Media downloads require API key or session-cookie authentication because they create gallery links tied to the authenticated account.

### MPP per-call pricing

Use [MPP overview](/mpp/overview#eligible-endpoints) for the full 31-endpoint route list. This guide keeps price bands here so mobile readers can estimate before opening the full table.
