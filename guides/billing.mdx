...
description: Subscription tiers, guest wallets, credit carry-over, per-operation costs, top-ups, and MPP pay-per-use for the Xquik API
keywords: ["X API pricing", "Twitter API pricing", "subscription", "guest wallet", "credits", "unused credits carry over", "monitor tweets pricing", "1-second tweet monitor cost", "tweet alerts cost", "tweet search cost", "follower export cost", "top-up", "MPP pay-per-use"]
---
...
Account workflows spend shared credits. Plans grant monthly credits that carry over. Guest wallets prepay 33 GET routes. MPP pays per request on 7 operations.
