...
For `active`, `subscription.currentPeriodEnd` reports the renewal date. For
`canceling`, it reports the plan end date. `creditInfo.balance` shows remaining
credits. At 0, metered calls stop until you [top up credits](/api-reference/credits/topup).

> **Tip.** Alert before `creditInfo.balance` reaches your next expected charge.
