...
<CardGroup cols={2}>
  ...
  <Card title="Tweet or reply write" icon="send">
    Export `tweet_id`, `charged`, and `charged_credits` on `200 Success`. For `202 x_write_unconfirmed`, export `write_action_id`, `status`, `charged`, and `charged_credits`, then poll `GET /x/write-actions/{id}` before retrying.
  </Card>
