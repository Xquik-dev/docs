...
follow-up questions, published signal names, and a topic `intentUrl`. Production
signal weights are `null` because X does not publish them. `refine` returns
writing guidance and 3 example patterns. `score` runs 9 deterministic text
checks. These checks do not predict reach. `styleUsername` selects a cached
style. Compose responses may include `savedStyles`, `styleTweets`, or
`styleNote`.
