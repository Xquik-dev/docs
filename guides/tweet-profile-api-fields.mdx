...
Interrupted extraction preserves available data and adds a free `partial` diagnostic.
...
| `availableResults` | Accepted data rows preserved by the run                |
...
It includes `gameId`, `title`, `url`, `state`, and `statusText` when available.
`scheduledAtMs` preserves the source's numeric or string Unix timestamp in milliseconds.
