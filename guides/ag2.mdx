...
AG2 binds search parameters when a tool is constructed, not when the model calls it. The agent chooses the query. You choose the window, the ordering, and the page size.
...
| Delegation | `Agent.as_tool()` | Keep large tweet payloads out of the coordinator's context |
...
`Agent.as_tool()` exposes an agent as a tool for another agent. Each delegated task runs on its own stream with its own history, so large tweet pages stay out of the coordinator's context.
