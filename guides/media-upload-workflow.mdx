...
If you already have public image URLs or a public MP4 video URL for a tweet or reply, skip `POST /x/media` and pass those URLs directly in the `media` array on `POST /x/tweets`. Send up to 4 images or exactly 1 MP4 video. Use `POST /x/media` when you need Xquik to host a local file, validate a generated media URL, or produce a `mediaId` for a DM attachment.
...
Use JSON URL upload when an AI agent, MCP client, or workflow tool has a generated image or MP4 URL that Xquik should validate and host. For tweet-only workflows with already public image URLs or exactly 1 public MP4 video URL, call `POST /x/tweets` directly with `media`.
...
`POST /x/tweets` accepts public media URLs in `media`. Send up to 4 images or exactly 1 MP4 video. Use the `mediaUrl` returned by `POST /x/media`.
