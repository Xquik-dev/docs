...
Build a private Zapier Platform CLI integration. Add routes after Zap history
proves demand.
...
Map friendly region names to WOEID values. Then call Xquik.
...
Zapier stores this result in `bundle.subscribeData`. Never expose `secret` as
trigger output. Use this path only for isolated testing.
...
Yes. A private Platform app calls Xquik with one API key.
...
Verify raw bodies and headers. Store nonces durably.
