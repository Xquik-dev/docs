...
and agent handoffs. Model Context Protocol, or MCP, provides a standard tool
contract. Xquik exposes both `explore` and `xquik` as remote MCP tools.
...
It normalizes `createdAt` to the Unix-second field `created`. Keep the Pydantic
schema aligned with that contract.
...
Yes. The [followers API](/api-reference/x/followers) returns followers through
cursor-based pagination. Extraction jobs produce larger CSV, JSON, or XLSX exports.
