...
preserve tweet IDs, profile IDs, cursors, and job IDs.

## Why Use Google ADK With a Twitter API?

Google's Agent Development Kit (ADK) manages Gemini calls, tools, sessions,
and agent handoffs. Model Context Protocol, or MCP, standardizes tool discovery
and execution. Xquik exposes Twitter API operations through `explore` and
`xquik`.
...
serves as the MCP client, while Xquik hosts the remote server.
...
Each documented status requires a separate recovery action.
...
Yes. The [followers API](/api-reference/x/followers) returns cursor-paginated
follower profiles. Extraction jobs produce larger CSV, JSON, or XLSX exports.
