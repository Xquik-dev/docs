...
It also replays monitors and reviews approved X actions. Preserve every tweet
ID, profile ID, cursor, and job ID.

## Why Use Google ADK With a Twitter API?

Google's Agent Development Kit manages Gemini calls, tools, sessions, and
agent handoffs. Model Context Protocol, or MCP, standardizes tool discovery
and tool calling. Xquik exposes Twitter API operations through `explore` and
`xquik`.

This Google ADK guide assigns one responsibility to each boundary. The Agent
Development Kit MCP client discovers current Xquik route schemas.
...
The environment variables keep Xquik and Google credentials outside agent
code. Never place either key inside an agent instruction.
...
acts as the MCP client. Xquik remains the remote MCP server.

`McpToolset` discovers `explore` and `xquik` through standard tool calling.
...
Python example validates every final response with Pydantic. The Twitter API
...
This Google ADK multi agent pattern separates research from publishing.
Production multi agent systems require separate keys and confirmation rules.
...
For public research, a guest `paid_reads` key enforces read-only access. It
exposes eligible GET routes only. [Guest wallets](/guides/guest-wallets)
...
This setup blocks Twitter API execution. Adding `xquik` enables key-authorized
...
Keep compact identifiers in session state. Keep complete tweet pages outside
prompts and session state.

Plain state keys belong to one conversation. `user:` keys persist across a
...
The tweet search contract assigns distinct meanings to these responses.
...
tweet identifiers. After recovery, deduplicate results through `tweet_id`.
...
`MCPToolset` now emits a deprecation warning. `McpToolset` uses lowercase `cp`
inside its class name.
...
calls. Xquik serves the tools, while `McpToolset` connects the agent.
...
Yes. The [followers API](/api-reference/x/followers) returns paginated profile
rows. Extraction jobs produce larger CSV, JSON, or XLSX exports. Poll each job
before loading its rows.
...
emphasizes a broader model ecosystem and graph orchestration. Xquik supports
both frameworks.

### Why Does a Remote MCP Connection Close Between Calls?

Keep one runner active during related turns. Let its context manager close each
toolset. Increase connection and SSE timeouts for long operations. Do not
create one `McpToolset` per prompt.

### Does Tool Filtering Make the Twitter API Read-Only?

Only `explore` without `xquik` blocks execution. The `xquik` tool can run every
