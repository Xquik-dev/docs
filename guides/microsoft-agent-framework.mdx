...
async def main():
    async with Agent(
        ...
        tools=MCPStreamableHTTPTool(
            ...
            description="X data platform with 118 MCP-compatible operations",
...
The agent discovers `explore` and `xquik`. It can call 118 MCP-compatible JSON or text operations. The MCP runtime returns normalized snake_case fields through `xquik.request()`. Keep prompts aligned with `tweet_id`, `has_more`, `next_cursor`, and returned job IDs.
