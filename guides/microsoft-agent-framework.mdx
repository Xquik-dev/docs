...
Xquik exposes one `xquik` execution tool. It runs routes authorized by the API
key.
...
Yes. Connect an X account first. Apply approval to every `xquik` request.
...
Verify signatures and de-duplicate deliveries before connecting a webhook.
