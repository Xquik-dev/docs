...
const [packageJson, lockfile, policy] = await Promise.all(
  ["package.json", "package-lock.json", "config/dependency-license-policy.json"].map(async (path) =>
...
const failures = [];
if (await Bun.file("bun.lock").exists()) {
...
for (const [path, metadata] of dependencies) {
  ...
  const license =
    policy.licenseReferences?.find(
      (/** @type {{declared: string, packages: string[], license: string}} */ reference) =>
        reference.declared === metadata.license && reference.packages.includes(key),
    )?.license ??
    metadata.license ??
