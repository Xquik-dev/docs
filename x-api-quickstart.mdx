...
keywords: ["Twitter API integration", "REST API integration", "Twitter webhook", "Twitter API webhook", "REST API key authentication example", "X API quickstart", "monitor tweets", "signed webhooks"]
---
...
For accountless reads, use a [guest wallet](/guides/guest-wallets) across 33
...
| --- | --- | --- |
| Check access and credits | `GET /account` | Subscription period, credit balance, and monitor billing |
...
<CardGroup cols={2}>
  <Card title="First API call" icon="terminal">
    Call `GET /account` to confirm authentication, renewal, credits, and monitor billing.
  </Card>
  ...
  <Card title="Accountless reads" icon="wallet-cards" href="/guides/guest-wallets">
    Prepay 33 eligible GET routes with a $10-$250 USD hosted checkout. No account required.
  </Card>
</CardGroup>
