...
const updateAccountSource = readFileSync(
  'api-reference/account/update.mdx',
  'utf8',
);
...
describe('account locale documentation', (): void => {
  it('limits the update page to the canonical locale contract', (): void => {
    ...
    expect({
      ...
      frontmatterClaimsProfileSettings:
        /^(?:title|description):.*profile settings.*$/imu.test(
          updateAccountSource,
        ),
  ...
  it('does not claim that account details return the saved locale', (): void => {
    ...
    expect({
      getDescriptionClaimsLocale:
        /^description:.*\blocale\b.*$/imu.test(getAccountSource),
      updateExplainsGetScope: /It does not return the\s+saved locale\./u.test(
        updateAccountSource,
      ),
    }).toStrictEqual({
