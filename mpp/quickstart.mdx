---
title: "MPP quickstart | MPP Payment Guide"
sidebarTitle: "MPP quickstart"
description: "Make your first pay-per-use API call with the Machine Payments Protocol. Review examples, responses, and integration guidance."
...
The `application/problem+json` body also includes an optional `payment_options.guest_wallet.create_checkout` action. It does not replace the MPP header, and the failed request creates no checkout. Ignore it when completing MPP. Use it only after a user explicitly chooses and confirms a $10-$250 USD guest wallet.
...
- [MPP overview](/mpp/overview): 7 direct MPP operations, pricing, and protocol details.
...
- [Get user](/api-reference/x/get-user): Full endpoint reference for user lookups.
