...
const REQUIRED_SDK_OVERVIEW_SNIPPETS = [
  ...
  'Store `job.id`, `job.status`, `results`, `hasMore`, and `nextCursor`.',
  'public media URLs',
...
const REQUIRED_CRM_EXPORT_WORKFLOW_SNIPPETS = [
  ...
  '`source` can be `followers`, `paginationCap`, or `resultsLimit`',
...
const REQUIRED_TWEET_REPLIES_EXPORT_SNIPPETS = [
  ...
  '`source` can be `replyCount`, `paginationCap`, or `resultsLimit`',
  '"job": "reply_extraction"',
...
const REQUIRED_TWEET_REPLIES_API_HANDOFF_SNIPPETS = [
  ...
  'author ID, username, display name, follower count, verified state, profile image URL',
...
const REQUIRED_TWEET_QUOTES_API_HANDOFF_SNIPPETS = [
  ...
  '<ResponseField name="verified" type="boolean">Whether the author is verified.',
  '<ResponseField name="profilePicture" type="string">Author profile image URL.',
  '`sinceTime`, `untilTime`, and tweet result',
...
const REQUIRED_FOLLOWERS_API_HANDOFF_SNIPPETS = [
  ...
  '`GET /x/users/{id}/followers`',
  'CRM, warehouse, audience, or agent',
  '`follower_explorer`',
  '[Follower Export CRM Workflow](/guides/follower-export-crm)',
  'saved CSV, JSON, or XLSX files',
  'imports or upserts',
...
const REQUIRED_WEBHOOK_TESTING_SNIPPETS = [
  ...
  '"handoff_state": "verified_signature_then_store_delivery_rows"',
...
const REQUIRED_KEYWORD_MONITOR_GET_API_HANDOFF_SNIPPETS = [
  ...
  'The Node.js, Python, and Go examples convert the fetched keyword monitor',
...
const REQUIRED_ZAPIER_GUIDE_SNIPPETS = [
  ...
  'carry `has_next_page` and `next_cursor` when a Zap loops pages.',
  ...
  'For `202 x_write_unconfirmed`, return `writeActionId`, `status`, `charged`, and `chargedCredits`, then poll `GET /x/write-actions/{id}` before retrying.',
  '<Card title="Public media upload" icon="image">',
  'Pass `mediaUrl` to `POST /x/tweets` in `media`; reserve uploaded media IDs for one-item DMs.',
  ...
  'fetch detail rows and carry `hasMore` plus `nextCursor` into batch Zaps.',
  '## Test Coverage',
...
const REQUIRED_PIPEDREAM_GUIDE_SNIPPETS = [
  ...
  '<Card title="Tweet or reply write" icon="send">',
  'For `202 x_write_unconfirmed`, export `writeActionId`, `status`, `charged`, and `chargedCredits`, then poll `GET /x/write-actions/{id}` before retrying.',
  '<Card title="Public media upload" icon="image">',
...
const REQUIRED_GOOGLE_ADK_GUIDE_SNIPPETS = [
  'Build a Google ADK agent that can search tweets, hand off IDs and cursors, monitor accounts, and run extraction jobs',
  'from pathlib import Path',
  'query, route_used, tweets[{tweet_id,text,author_username,created_at}]',
  'response_parts = []',
...
const REQUIRED_LANGCHAIN_GUIDE_SNIPPETS = [
  'Build a LangChain agent that can search tweets, hand off IDs and cursors, post tweets, and run extraction jobs',
  'from pathlib import Path',
  'query, route_used, tweets[{tweet_id,text,author_username,created_at}]',
...
const REQUIRED_N8N_GUIDE_SNIPPETS = [
  ...
  '<Card title="Tweet or reply write" icon="send">',
  'For `202 x_write_unconfirmed`, store `writeActionId`, `status`, `charged`, and `chargedCredits`, then poll `GET /x/write-actions/{id}` before retrying.',
  '<Card title="Public media upload" icon="image">',
...
const REQUIRED_MAKE_GUIDE_SNIPPETS = [
  ...
  '<Card title="Tweet search page" icon="search">',
  'Store `q`, each tweet `id`, `text`, `author.username`, `createdAt`, `body.has_next_page`, and `body.next_cursor`.',
  '<Card title="User profile rows" icon="users">',
  'Store source `id` as `userId`, plus `username`, `name`, `followers`, `verified`, and `profilePicture`.',
  'For user-list modules, carry `body.has_next_page` and `body.next_cursor`.',
  '<Card title="Trend rows" icon="trending-up">',
