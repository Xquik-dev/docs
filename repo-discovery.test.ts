...
const REQUIRED_CREATE_TWEET_API_SNIPPETS = [
  ...
  'Send a unique `Idempotency-Key`, store the durable action, and poll `statusUrl` while `terminal` is `false`.',
...
const REQUIRED_TWEET_SEARCH_EXPORT_SNIPPETS = [
  ...
  'Use direct search for low-latency pages.',
  '### How Do I Scrape Tweets Without Getting Blocked?',
  'Respect each',
  'rate limit and wait for `retryAfter` after a `429` response.',
  ...
  'GET /extractions/{id}',
  'GET /x/tweets/search',
  'one Tweet ID or X status URL lookup, without a stored extraction job.',
  ...
  'A plain Tweet ID or X status URL in `q` is a direct lookup, not a saved extractor job.',
...
const REQUIRED_BRAND_MONITORING_WORKFLOW_SNIPPETS = [
  ...
  '### What API Tracks Twitter Keyword Mentions?',
  ...
  '### How Do I Monitor a Twitter Account Programmatically?',
  ...
  'Test the query with tweet search before starting continuous monitoring.',
...
const REQUIRED_APIFY_ALTERNATIVE_SNIPPETS = [
  ...
  'raw output modes when you need',
