...
const REQUIRED_BILLING_MONITOR_SNIPPETS = [
  ...
  'Send public image URLs in `media` when posting media.',
...
const REQUIRED_CREATE_TWEET_API_SNIPPETS = [
  ...
  '`reply_to_tweet_id`',
  '`media`',
  'Array of 1-4 public image URLs to attach directly.',
  'Use [Upload Media](/api-reference/x-write/upload-media) first if you need Xquik to host a local file',
  '## Post with public image URLs',
  'Use `media` when your image is already a public HTTPS URL or when [Upload Media](/api-reference/x-write/upload-media) returned `mediaUrl`.',
...
const REQUIRED_WORKFLOW_OVERVIEW_SNIPPETS = [
  ...
  '### 5. Post image tweets or replies',
...
const REQUIRED_KEYWORD_MONITOR_API_HANDOFF_SNIPPETS = [
  ...
  '<Card title="Signed Webhook Rollout" icon="shield-check">',
  'Create the webhook with matching `eventTypes`',
  'Store the one-time `secret`, then run `POST /webhooks/{id}/test`',
  '<Card title="Delivery Verification" icon="key-round">',
  '`X-Xquik-Signature`, `X-Xquik-Timestamp`, and `X-Xquik-Nonce`',
  'Deduplicate with `deliveryId`',
  'join with `streamEventId` and',
  '`keywordMonitorId`',
