...
description: OAuth 2.1, PKCE, CIMD, and DCR for Xquik MCP authentication
...
Publish a JSON document at a stable HTTPS URL with a non-root path and use that
exact URL as the `client_id`:
...
The HTTPS `client_id` URL must have a non-root path and no user information,
