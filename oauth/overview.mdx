...
description: "Authorize MCP clients and agents with OAuth 2.1, PKCE, claimed service identities, rotating refresh tokens, discovery metadata, and revocation examples."
keywords: ["OAuth", "OAuth 2.1", "PKCE", "MCP", "agent authentication", "authorization", "token", "refresh token", "CIMD", "dynamic client registration"]
---
...
<Warning>
  Codex CLI 0.147.0 and newer preserve the RFC 9207 `iss` value. Upgrade if an older release reports `Authorization server response missing required issuer: expected https://xquik.com`. Use the [Codex API key fallback](/guides/troubleshooting#codex-oauth-issuer-validation-error) only when upgrading is unavailable. [Issue #31573](https://github.com/openai/codex/issues/31573) records the older defect and fix.
</Warning>

Interactive OAuth supports 2 client registration paths:
...
The `agent_auth` object follows the current Auth.md service identity profile.
...
`/.well-known/oauth-protected-resource/en`, `/es`, and `/tr`. MCP clients should
keep using the `/mcp` resource shown above.
...
| Identity assertion | 1 hour | Exchanges only for its active registration |
...
&client_id=urn:xquik:agent-auth\
&claim_token=CLAIM_TOKEN"
    ```

    Handle `authorization_pending`, `slow_down`, `access_denied`, and
    `expired_token`. Success returns an access token and rotating refresh
    token. It also returns `identity_assertion`.
  </Step>

  <Step title="Exchange an identity assertion">
    Exchange a current assertion only when a fresh OAuth pair is required.
...
&assertion=IDENTITY_ASSERTION"
    ...
    Xquik accepts only service-signed assertions for active registrations.
  </Step>
</Steps>
...
For claimed agent registration, use `client_id=urn:xquik:agent-auth`. Revoke
the current refresh token to revoke the full registration. Revoking only an
access token leaves its refresh family active.
