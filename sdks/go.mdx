...
The generated Go response model covers confirmed `TweetID` responses. Use `option.WithResponseBodyInto` when a write worker must branch on the REST `202 x_write_unconfirmed` response, store `writeActionId` and `chargedCredits`, and poll [Get Write Action Status](/api-reference/x-write/get-write-action-status) before sending another write.
