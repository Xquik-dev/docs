...
Use this page when you need a TypeScript service to search tweets, scrape tweets to JSON, export follower or profile data, post tweets, upload media, send DMs, monitor tweets, or hand X data to another system through SDK calls.
...
This job is for Node.js, Bun, and queue workers that need searchable tweet data in a stable handoff format. It calls `GET /x/tweets/search` through `client.x.tweets.search`, uses the generated `TweetSearchParams` shape, and writes each returned tweet as one JSON object per line.
...
| TypeScript field | REST query parameter | Use it for |
| --- | --- | --- |
...
| TypeScript field | JSON field | Handoff value |
| --- | --- | --- |
| `page.tweets` | `tweets` | Array of tweet records. Each tweet includes `id`, `text`, optional `author`, `createdAt`, `likeCount`, `replyCount`, `retweetCount`, `quoteCount`, `bookmarkCount`, `viewCount`, and `isNoteTweet` when available. |
...
Write `page.tweets` as JSON Lines for queues and data lakes, transform the same records into CSV or XLSX for analysts, or pass `id`, `text`, `author.username`, `createdAt`, and engagement counts into your CRM or enrichment pipeline.
