...
The generated TypeScript SDK models confirmed `tweetId` responses. The REST API can also return `202 x_write_unconfirmed`; treat that runtime value as pending, store `writeActionId`, and poll [Get Write Action Status](/api-reference/x-write/get-write-action-status) before sending another write.
