...
title: Kotlin SDK
sidebarTitle: Kotlin
description: Use the Xquik Kotlin SDK to search tweets, export JSON Lines or CSV, post tweets, upload media, send DMs, and build JVM X API workers.
keywords: ["Kotlin SDK", "X API Kotlin", "JVM client", "tweet search Kotlin", "Twitter API Kotlin", "JSON export", "source build"]
---
...
Use this workflow when a Kotlin service, scheduled worker, queue consumer, or agent needs tweet search results in a durable handoff file for a data lake, CRM enrichment step, analyst CSV export, or downstream processor.
...
Files.newBufferedWriter(Paths.get("tweets.jsonl"), StandardCharsets.UTF_8).use { writer ->
    do {
