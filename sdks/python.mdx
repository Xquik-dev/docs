...
The generated Python model covers confirmed `tweet_id` responses. Use `with_raw_response.create` when a write worker must branch on the REST `202 x_write_unconfirmed` response, store `writeActionId`, and poll [Get Write Action Status](/api-reference/x-write/get-write-action-status) before sending another write.
