...
Map `User ID` or result `xUserId` as the CRM unique key. Cost: 1 credit per follower extracted or returned. Exports are free after the extraction job exists.
...
The response returns `tweetId` and `success`. If the API returns `202 x_write_unconfirmed`, store `writeActionId` and poll `GET /x/write-actions/{id}` before sending another write. Keep one JSON Lines handoff shape for both outcomes:
...
<CardGroup cols={2}>
  ...
  <Card title="Post a tweet or reply" icon="send">
    Run `x-twitter-scraper x:tweets create` to return `tweetId` and `success`, or `writeActionId` when confirmation is pending.
  </Card>
