...
Write `page.tweets` to CSV for analysts. Write JSON Lines for queues and data lakes to `xquik-tweet-search.jsonl` with `tweet.to_json`, or call `tweet.deep_to_h` before loading a warehouse. Load the projected rows into an XLSX writer when account teams need a workbook. Pass `id`, `text`, `author.username`, `created_at`, and engagement counts to a CRM or enrichment job.
