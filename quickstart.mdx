...
Use this X API quickstart to create an API key, call `GET /account`, monitor tweets every 1 second with `POST /monitors`, and send signed webhook events with `POST /webhooks`. API MCP can run the same endpoints through `xquik.request()` and returns the normalized v1 contract.
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Server" icon="bot" href="/mcp/overview">
    Connect API MCP v2.5.1 through Streamable HTTP. Use `explore`, then `xquik`.
  </Card>
