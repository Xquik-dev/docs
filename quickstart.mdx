...
<Steps>
  ...
  <Step title="Subscribe">
    <Warning>
      An active subscription is required for this full account workflow. Guest wallets cover 33 prepaid GET routes without a subscription. Direct MPP covers 7 fixed-price operations.
    </Warning>
    Full account API access requires an active subscription. Starter is USD 20/month and includes 140,000 monthly credits. Subscribe from the [dashboard billing page](https://dashboard.xquik.com/en/account?tab=subscription).
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Server" icon="bot" href="/mcp/overview">
    Connect API MCP v2.5.4 through Streamable HTTP. Use `explore`, then `xquik`.
  </Card>
