...
<AccordionGroup>
  ...
  <Accordion title="Rate limited (429)">
    The API uses a 3-tier fixed window: 10 reads/1s, 30 writes/60s, 15 deletes/60s. Implement exponential backoff and respect the `Retry-After` header. See [Rate Limits](/guides/rate-limits).
  </Accordion>
</AccordionGroup>
