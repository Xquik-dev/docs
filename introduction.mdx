...
<CardGroup cols={2}>
  ...
  <Card title="Tweet Composer" icon="pen-line">
    Compose, refine, and score tweet drafts against X ranking factors. Free; no credits required.
  </Card>
...
<Steps>
  ...
  <Step title="MCP Server">
    Connect AI agents through API MCP v2.5.4. Use OAuth 2.1 where supported or the [documented client-specific fallback](/mcp/overview#client-compatibility). Streamable HTTP exposes 2 code-mode tools: `explore` searches the catalog and `xquik` runs authenticated calls with normalized snake_case results.
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Server" icon="bot" href="/mcp/overview">
    Use `explore` and `xquik` through API MCP v2.5.4.
  </Card>
</CardGroup>
