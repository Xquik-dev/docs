...
- Connect API MCP v2.5.6 at `https://xquik.com/mcp` for authenticated tasks. Use the [current client path](/mcp/overview#client-compatibility) for OAuth, API-key fallback, or an adapter. Full credentials expose 119 catalog routes. Guest `paid_reads` keys expose 33 GET routes.
...
<Steps>
  ...
  <Step title="AI Agent Skill">
    Install the [Xquik Skill](https://github.com/Xquik-dev/x-twitter-scraper) in Claude Code, Cursor, Codex, Copilot, Windsurf, or another compatible agent. It covers the Xquik API, MCP tools, and 127 REST operations. Run `npx skills add Xquik-dev/x-twitter-scraper`.
  </Step>
