...
- Connect API MCP v2.5.3 at `https://xquik.com/mcp` when an agent should run authenticated tasks through Streamable HTTP. Complete OAuth 2.1 in the client. Full credentials expose 118 operations. Guest `paid_reads` keys expose 33 eligible GET routes.
...
<Steps>
  ...
  <Step title="MCP Server">
    Connect AI agents through API MCP v2.5.3 with OAuth 2.1. Streamable HTTP exposes 2 code-mode tools: `explore` searches the catalog and `xquik` runs authenticated calls with normalized snake_case results.
  </Step>
