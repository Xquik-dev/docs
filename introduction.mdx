...
- Connect API MCP v2.6.0 at `https://xquik.com/mcp` for authenticated tasks. Use the [current client path](/mcp/overview#client-compatibility) for OAuth, API-key fallback, or an adapter. Full credentials expose 120 catalog routes. Guest `paid_reads` keys expose 33 GET routes.
...
<Steps>
  ...
  <Step title="MCP Server">
    Connect AI agents through API MCP v2.6.0. Use OAuth 2.1 where supported. Otherwise, use the [documented client fallback](/mcp/overview#client-compatibility). Streamable HTTP exposes 2 tools. `explore` searches. `xquik` runs authenticated calls with normalized snake_case results.
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Server" icon="bot" href="/mcp/overview">
    Use `explore` and `xquik` through API MCP v2.6.0.
  </Card>
</CardGroup>
