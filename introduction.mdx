...
- Connect API MCP v2.5.1 at `https://xquik.com/mcp` when an agent should run authenticated tasks through Streamable HTTP.
...
<Steps>
  ...
  <Step title="MCP Server">
    Connect AI agents through API MCP v2.5.1. Streamable HTTP exposes 2 code-mode tools: `explore` searches the catalog and `xquik` runs authenticated calls with normalized snake_case results.
  </Step>
  <Step title="AI Agent Skill">
    Install the [Xquik Skill](https://github.com/Xquik-dev/x-twitter-scraper) to give 40+ AI coding agents (Claude Code, Cursor, Codex, Copilot, Windsurf, and more) deep knowledge of the Xquik API, MCP tools, and 123 REST operations. Install via `npx skills add Xquik-dev/x-twitter-scraper`.
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card title="MCP Server" icon="bot" href="/mcp/overview">
    Use `explore` and `xquik` through API MCP v2.5.1.
  </Card>
</CardGroup>
