...
const page = readFileSync("api-reference/account/subscription-checkout.mdx", "utf8");
...
describe("subscription checkout documentation", (): void => {
  it("requires confirmation and routes every billing state", (): void => {
    ...
    expect({
      confirmationRequired: page.includes("after an authenticated user confirms a billing action."),
      paymentNotAutomatic: page.includes("It never completes payment automatically."),
      statusesDocumented: [
