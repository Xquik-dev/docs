...
<ResponseExample>
  ...
  ```json 400 · x/community-moderators
  {
    "error": "invalid_input",
  ...
  ```json 401 · x/community-moderators
  {
  ...
  ```json 402 · x/community-moderators
  {
  ...
  ```json 404 · x/community-moderators
  {
    "error": "not_found",
  ...
  ```json 424 · x/community-moderators
  {
  ...
  ```json 429 · x/community-moderators
  {
  ...
  ```json 502 · x/community-moderators
  {
    "error": "x_api_unavailable",
    "message": "X data source temporarily unavailable. Try again later."
  }
...
Community URLs expose visible member and moderator lists. Review the official
[Communities guide](https://help.x.com/en/using-x/communities) for that public
view.
...
Request another page only when `has_next_page` is exactly `true`. Reuse the
exact `next_cursor`. Mark the roster complete only after `has_next_page` is
`false`.
...
Keep those workflows separate from public profile exports. This boundary
prevents a profile row from becoming an unsupported moderation decision.
...
<CardGroup cols={2}>
  ...
  <Card title="Next page" icon="arrow-right">
    Persist each page before its `has_next_page` flag and `next_cursor`.
  </Card>
...
Treat each response page as a durable checkpoint. Save every profile row
...
Classify credit-bounded, failed, and interrupted runs as partial moderator
snapshots. Do not compare a partial roster with a completed baseline. A partial
comparison can falsely report removed moderators.

Deduplicate resumed pages with `community_id` and `moderator_id`. Keep the
latest username as a label. Preserve the numeric user ID as the durable join
key.
...
These fields let a reviewer distinguish a real role change from an unfinished
collection.
...
Join the moderator roster to [Community Info](/api-reference/x/community-info)
only when `community_id` and your local `export_manifest_id` match. Keep rules,
member counts, and moderator profiles in separate tables. This preserves each
endpoint's public response contract.

The response may include `communityRole`. Store its returned value without
normalizing it into a broader permission. An omitted role cannot prove admin,
creator, or member permissions.
...
4. Review added and missing IDs with an authorized human.
5. Store the decision outside the public profile snapshot.

Keep one row per visible moderator. Store the username only as a mutable label.
Use the numeric user ID for durable joins.

Escalate unexpected changes to a human reviewer. Require human approval before
any account action.
