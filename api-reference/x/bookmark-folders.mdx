...
printing the full response. Persist `folderRows` or `folder_rows`.
...
The route returns one folder page with `has_next_page: false` and
`next_cursor: ""`. Use each folder ID with
...
Treat every folder list as an account-specific snapshot. Compare folder IDs
