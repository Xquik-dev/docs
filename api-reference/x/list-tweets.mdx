...
description: "Retrieve tweets from an X list with authors, text, replies, reposts, likes, quotes, media, timestamps, and cursor-based pagination. See request fields."
api: "GET /x/lists/{id}/tweets"
keywords: ["Twitter list tweets API", "Twitter Lists API", "X Lists API", "list timeline", "tweet list", "list tweet export"]
---
...
Use this Twitter Lists API to retrieve Twitter List tweets from one curated
...
accounts are not visible. Read X's official [Help with Lists](https://help.x.com/en/using-x/x-lists-not-working)
...
Preserve the returned Tweet sequence in every page. Do not treat that order as
...
cursor. Preserve the author ID because list membership can change later.
...
Deduplicate pages by list ID and tweet ID. Persist each page before advancing
