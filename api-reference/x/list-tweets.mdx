...
Store `list_id`, `tweet_id`, `text`, `author_username`, `created_at`,
engagement counts, & `media_urls` for each row. Keep `has_next_page` &
`next_cursor` with the export checkpoint so the next run can resume the list
timeline without duplicating earlier rows.
