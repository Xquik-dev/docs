...
keywords: ["Twitter follower checker", "Twitter follow checker API", "check Twitter followers", "X follower checker", "follow check API", "campaign verification", "giveaway verification", "relationship API", "MPP"]
---
...
<ResponseExample>
  ...
  ```json 400 · x/check-follower
  {
    "error": "missing_params"
  }
  ```

  ```json 401 · x/check-follower
  {
...
`@usernames`, or supported profile URLs and needs one proof for a follow task.
...
<CardGroup cols={2}>
  ...
  <Card title="Accepted inputs" icon="at-sign">
    ...
    Numeric user IDs are not accepted. Use [Get user](/api-reference/x/twitter-profile-lookup)
...
<ResponseField name="sourceUsername" type="string">Canonical lowercase username resolved from `source`.</ResponseField>
<ResponseField name="targetUsername" type="string">Canonical lowercase username resolved from `target`.</ResponseField>
