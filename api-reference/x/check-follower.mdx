---
title: "Check follower"
description: "Check the follow relationship between two X users to verify if one follows the other"
api: "GET /x/followers/check"
keywords: ["follower", "check", "relationship", "follow", "x api"]
---

import LlmsDirective from "/snippets/llms-directive.mdx";

<LlmsDirective />

<Callout icon="coins" color="#5c3327">
  **5 credits per call** · [All plans](https://xquik.com/#pricing) from $0.00012/credit
</Callout>

<CodeGroup>
  ```bash cURL
  curl -G https://xquik.com/api/v1/x/followers/check \
    -d "source=xquikcom" \
    -d "target=elonmusk" \
    -H "x-api-key: xq_YOUR_KEY_HERE" | jq
  ```

  ```javascript Node.js
  const source = "xquikcom";
  const target = "elonmusk";
  const params = new URLSearchParams({ source, target });
  const response = await fetch(`https://xquik.com/api/v1/x/followers/check?${params}`, {
    headers: { "x-api-key": "xq_YOUR_KEY_HERE" },
  });
  const data = await response.json();
  const relationshipRow = {
    source_username: data.sourceUsername,
    target_username: data.targetUsername,
    source_follows_target: data.isFollowing,
    target_follows_source: data.isFollowedBy,
    checked_at: new Date().toISOString(),
  };
  ```

  ```python Python
  import requests
  from datetime import datetime, timezone

  source = "xquikcom"
  target = "elonmusk"

  response = requests.get(
      "https://xquik.com/api/v1/x/followers/check",
      params={"source": source, "target": target},
      headers={"x-api-key": "xq_YOUR_KEY_HERE"},
  )
  data = response.json()
  relationship_row = {
      "source_username": data["sourceUsername"],
      "target_username": data["targetUsername"],
      "source_follows_target": data["isFollowing"],
      "target_follows_source": data["isFollowedBy"],
      "checked_at": datetime.now(timezone.utc).isoformat(),
  }
  ```

  ```go Go
  package main

  import (
  	"fmt"
  	"io"
  	"log"
  	"net/http"
  	"net/url"
  )

  func main() {
  	baseURL := "https://xquik.com/api/v1/x/followers/check"
  	params := url.Values{}
  	params.Set("source", "xquikcom")
  	params.Set("target", "elonmusk")

  	req, err := http.NewRequest("GET", baseURL+"?"+params.Encode(), nil)
  	if err != nil {
  		log.Fatal(err)
  	}
  	req.Header.Set("x-api-key", "xq_YOUR_KEY_HERE")

  	resp, err := http.DefaultClient.Do(req)
  	if err != nil {
  		log.Fatal(err)
  	}
  	defer resp.Body.Close()

  	body, err := io.ReadAll(resp.Body)
  	if err != nil {
  		log.Fatal(err)
  	}
  	fmt.Println(string(body))
  }
  ```
</CodeGroup>

The Node.js and Python snippets shape a durable relationship row instead of
printing raw API output. Persist `relationshipRow` or `relationship_row` with
your CRM contact, moderation queue, giveaway rule, or support ticket record.

## Direct follow relationship handoff

Use `GET /x/followers/check` when a CRM, support, giveaway, moderation, or
agent workflow needs a yes/no relationship check between two X usernames. Store
`source_username`, `target_username`, `source_follows_target`,
`target_follows_source`, and `checked_at` so later workflow steps can audit the
exact account pair that was checked.

## Query parameters

<ParamField query="source" type="string" required>
  Source username. The user whose follow action is being checked.
</ParamField>

<ParamField query="target" type="string" required>
  Target username. The user who may be followed by the source.
</ParamField>

## Headers

<ParamField header="x-api-key" type="string" required>
  Your API key. Session cookie authentication is also supported.
</ParamField>

## Response

<Tabs>
  <Tab title="200 OK">
    <ResponseField name="sourceUsername" type="string">The source username from the request.</ResponseField>
    <ResponseField name="targetUsername" type="string">The target username from the request.</ResponseField>
    <ResponseField name="isFollowing" type="boolean">`true` if the source user follows the target user.</ResponseField>
    <ResponseField name="isFollowedBy" type="boolean">`true` if the target user follows the source user.</ResponseField>

    ```json
    {
      "sourceUsername": "xquikcom",
      "targetUsername": "elonmusk",
      "isFollowing": true,
      "isFollowedBy": false
    }
    ```
  </Tab>
  <Tab title="400 Missing params">
    ```json
    { "error": "missing_params", "required": ["source", "target"] }
    ```
    One or both query parameters are missing. Provide both `source` and `target`.
  </Tab>
  <Tab title="401 Unauthenticated">
    ```json
    { "error": "unauthenticated" }
    ```
    Missing or invalid API key. Check the `x-api-key` header value.
  </Tab>
  <Tab title="402 Subscription required">
    ```json
    { "error": "no_subscription" }
    ```
    No active subscription or insufficient credits. Possible error values: `no_subscription`, `subscription_inactive`, `no_credits`, `insufficient_credits`.
    For [MPP](/mpp/overview) requests without a valid payment credential, 402 returns a `WWW-Authenticate: Payment` challenge header instead.
  </Tab>
  <Tab title="502 X API unavailable">
    ```json
    { "error": "x_api_unavailable" }
    ```
    The read service returned an error. Retry after a short delay.
  </Tab>
  <Tab title="429 Rate Limit Exceeded">
    ```json
    { "error": "rate_limit_exceeded", "retryAfter": 60 }
    ```
    Your tier rate limit was exceeded. Wait for the `Retry-After` header before retrying.
  </Tab>
  <Tab title="424 Dependency Failed">
    ```json
    { "error": "x_api_unavailable" }
    ```
    The normalized v1 response contract can return 424 when the read service is unavailable.
  </Tab>
</Tabs>

<Note>
  **Next steps:** [Get User](/api-reference/x/get-user) to look up full profile details for either user, or [Get Account](/api-reference/account/get) to check remaining credits.
</Note>
