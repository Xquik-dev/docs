...
description: "Check the follow relationship between two X users to verify if one follows the other"
...
The Node.js and Python snippets shape a durable relationship row instead of
printing the full response object. Persist `relationshipRow` or
`relationship_row` with your own `checked_at` timestamp when you audit follow
requirements.

## Direct follow relationship handoff

Use `GET /x/followers/check` when an app, CRM, giveaway eligibility check,
moderation workflow, or agent needs a yes/no follow relationship result for one
source and target pair.

Store `source_username`, `target_username`, `source_follows_target`, and
`target_follows_source`. Treat the two booleans separately: `isFollowing` means
the source follows the target, while `isFollowedBy` means the target follows
the source. Missing `source` or `target` returns `400 missing_params`.
...
<Note>
  **Next steps:** [Get User](/api-reference/x/get-user) to look up full profile details for either user, or [Get Account](/api-reference/account/get) to check remaining credits.
</Note>
