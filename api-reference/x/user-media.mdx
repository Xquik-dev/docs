...
description: "Scrape tweets with photos, videos, and GIFs from a Twitter or X profile. Export media posts with authors, metrics, cursors, and timestamps. See costs."
api: "GET /x/users/{id}/media"
keywords: ["Twitter media scraper", "Twitter media API", "X media API", "user media", "tweet photos", "tweet videos", "GIFs", "media export"]
---
...
One tweet can contain several media items. Preserve the tweet-to-media
relationship. Do not flatten several URLs into an ambiguous single value.
...
profile timeline. Use download media only after selecting a specific asset.
...
relevant tweets. Then call the download route for selected media.

Preserve attribution and source links in any archive. The media URL alone does
...
Keep failed downloads retryable by tweet ID and media URL. Do not rerun the
...
| `tweet_url` | `tweets[].url` | Preserve the original X post for attribution and review. |
| `tweet_text` | `tweets[].text` | Keep the caption or post context beside each asset. |
| `author_id` | `tweets[].author.id` | Preserve the original publisher as a stable identity. |
| `created_at` | `tweets[].createdAt` | Store the tweet publication time. |
| `media_index` | Position inside `tweets[].media` | Preserve the order of several assets on one tweet. |
...
| Approved image download | Image media type and approved usage | Call the download route for the selected media URL. |
