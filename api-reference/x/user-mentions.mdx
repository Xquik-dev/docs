...
<CodeGroup>
  ...
  for (let pageIndex = 0; pageIndex < 3; pageIndex += 1) {
    ...
    const response = await fetch(
      `https://xquik.com/api/v1/x/users/${userId}/mentions${params}`,
...
Store `mentioned_user`, `tweet_id`, `text`, `tweet_url`, `author_id`,
`author_username`, `created_at`, `conversation_id`, reply context, engagement
counts, media URLs, `page_cursor`, `has_next_page`, and `next_cursor`. Treat
`next_cursor` as opaque and pass it back as `cursor` only when
`has_next_page` is true. Use `sinceTime` and `untilTime` to bound a poller
