...
<CodeGroup>
  ...
  const response = await fetch(
    `https://xquik.com/api/v1/x/users/${userId}/mentions`,
    {
  ...
  const mentionRows = data.tweets.map((tweet) => ({
  ...
  response = requests.get(
      f"https://xquik.com/api/v1/x/users/{user_id}/mentions",
      headers={"x-api-key": "xq_YOUR_KEY_HERE"},
  )
  data = response.json()
...
The Node.js and Python snippets shape durable mention rows instead of printing
full tweet pages. Persist `mentionRows` or `mention_rows` with `nextCursor` or
`next_cursor` before requesting the next page.
...
It returns one JSON page at a time. Use [`mentions`](/api-reference/extractions/create)
when you need a saved extraction, estimate, or CSV/JSON/XLSX file export.

Store `mentioned_user`, `tweet_id`, `text`, `author_id`, `author_username`,
`created_at`, `conversation_id`, engagement counts, media URLs,
`has_next_page`, and `next_cursor`. Treat `next_cursor` as opaque and pass it
back as `cursor` only when `has_next_page` is true. Use `sinceTime` and
`untilTime` to bound a poller window. Zero affordable results return
