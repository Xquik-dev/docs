...
description: "Retrieve one user's X mentions timeline with cursor pagination, time windows, author fields, engagement metrics, and media. Includes costs and errors."
api: "GET /x/users/{id}/mentions"
keywords: ["get user mentions", "user mentions timeline", "mentions API", "brand mentions", "X user mentions", "x api"]
---
...
<Info>
  Twitter mentions timeline API returns tweets that mention one X account. Use it
...
| `media_urls` | `tweets[].media[].mediaUrl` | Preserve image and video context for review. |
...
| High-reach author | Approved `author.followers` threshold | Raise the review priority without changing the author identity. |
| Reply in an existing thread | Non-empty `inReplyToId` or `conversationId` | Attach the existing conversation context. |
| Media evidence | Non-empty `media` | Preserve image or video URLs with the triage record. |
| No approved rule | No explicit rule matches | Store the mention without inventing a category. |
