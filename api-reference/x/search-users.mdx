---
title: "Twitter user search & profile lookup"
sidebarTitle: "Search users"
description: "Search Twitter or X users by name or username. Return matching profiles, biographies, follower counts, verification status, and pagination."
api: "GET /x/users/search"
keywords: ["Twitter user search", "Twitter account search", "Twitter user finder", "search Twitter profiles", "X user search API", "username lookup", "profiles"]
---
...
remain the safer handoff for followers, following, timelines, and profile
lookups.

Search results can be empty without representing an API failure. Separate no
