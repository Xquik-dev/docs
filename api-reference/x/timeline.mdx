...
as a complete public archive. Read X's [timeline documentation](https://docs.x.com/x-api/posts/timelines/introduction)
for its home and user feed model.
...
Respect the `Retry-After` header after a 429 response. Retry 424 or 502 responses
from the stored cursor. Never advance a checkpoint after a failed destination write.
