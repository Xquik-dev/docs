...
api: "GET /x/timeline"
keywords: ["Twitter timeline API", "Twitter API timeline", "X timeline API", "Twitter home timeline", "Twitter API timeline pagination", "home feed API", "timeline tweets", "replies", "reposts", "likes", "media"]
---
...
as a complete public archive. [X explains its home and user timeline
feeds](https://docs.x.com/x-api/posts/timelines/introduction).
...
Deduplicate timeline tweets with `tweets[].id`. Save each page before advancing
its cursor. Keep the prior cursor until validating its replacement.
...
Respect `Retry-After` after a `429` response. Retry `424` or `502` from the
saved cursor. Never advance a checkpoint after a failed destination write.
...
Fix the API key after `401`. Add credits after `402`. Respect `Retry-After`
after `429`. Resume from the saved cursor after `424` or `502`.
...
<ParamField header="x-api-key" type="string" required>
  Your API key. You can also authenticate with a session cookie.
</ParamField>
...
{ "error": "x_api_unavailable" }
```
The normalized v1 contract returns `424` when the read service fails.
