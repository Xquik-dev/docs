...
description: "Search a Twitter or X profile by username or user ID. Retrieve the bio, follower and following counts, verification status, location, and links. See costs."
api: "GET /x/users/{id}"
keywords: ["Twitter profiles", "Twitter profile search", "Twitter user finder", "Twitter user API", "X profile API", "username lookup", "followers", "following"]
---
...
<ResponseExample>
  ...
  ```json 400 · x/twitter-profile-lookup
  {
    "error": "invalid_input",
  ...
  ```json 401 · x/twitter-profile-lookup
  {
  ...
  ```json 402 · x/twitter-profile-lookup
  {
  ...
  ```json 404 · x/twitter-profile-lookup
  {
  ...
  ```json 424 · x/twitter-profile-lookup
  {
  ...
  ```json 429 · x/twitter-profile-lookup
  {
  ...
  ```json 502 · x/twitter-profile-lookup
  {
    "error": "x_api_unavailable",
    "message": "X data source temporarily unavailable. Try again later."
  }
