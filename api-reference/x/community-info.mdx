...
Start with community details for each supplied numeric community ID.
...
Confirm `community.id` and `community.name` first. Add both values to each
...
This route creates the qualification record. Collection routes create later
profile and tweet rows.
...
- Record viewer membership state and role.
...
Do not copy member profiles into this metadata record. Keep tweets out of this
metadata record. Link later exports by community ID and manifest ID.
...
separately. A member-count change does not prove a policy change. Rule changes
do not imply tweet changes.
...
<ResponseField name="community" type="object">
  ...
  **Community object fields:**
    ...
    <ResponseField name="moderator_count" type="number">Total moderators. Omitted if unavailable.</ResponseField>
