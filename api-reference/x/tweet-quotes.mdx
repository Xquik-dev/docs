...
keywords: ["quote tweets", "view quote tweets", "get quote tweets", "quote tweets API", "tweet quotes API", "Twitter quote tweets API", "X quote tweets API", "quoted posts", "conversation analysis", "CSV export"]
---
...
<Info>
  ...
  contains the quoting tweet's commentary, author, engagement, and media. The
  canonical route remains `GET /api/v1/x/tweets/{id}/quotes`.
</Info>
...
each row for attribution, deduplication, and campaign analysis.
...
remove otherwise visible rows. Never treat a missing row as proof that no
quote tweet exists.
...
A repost shares the original without added commentary. A reply joins the
tweet's conversation. Use retweeters for reposting profiles and replies for
conversation rows.

### How Can Teams Analyze Quote Tweets?

Store the quote tweet's text, author, creation time, engagement counts, and
media. Keep the source relationship separate. Deduplicate by source tweet ID
and quote tweet ID. Use monitors and signed webhooks for future quote alerts.
