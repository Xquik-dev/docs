---
title: "Twitter Quote Tweets API, Authors & Export Cursors"
sidebarTitle: "Get quote tweets"
description: "Retrieve quote tweets for one source tweet with authors, text, media, likes, reposts, replies, views, filters, and cursor checkpoints. See API fields."
api: "GET /x/tweets/{id}/quotes"
keywords: ["get quote tweets", "quote tweets API", "tweet quotes API", "Twitter quote tweets API", "X quote tweets API", "quoted posts", "conversation analysis", "CSV export"]
---
...
## Preserve quote-tweet commentary

Use this route when added commentary matters. Keep the quoted source tweet ID
with every quote tweet.

Store the quote tweet’s ID, text, author, creation time, engagement counts,
media, cursor, and collection time. Also preserve the relationship to the
source tweet.

Quote tweets can support reaction research, campaign review, or moderation.
Do not merge the quoting author’s text with the source tweet text.

Deduplicate by source tweet ID and quote tweet ID. Save each page before
advancing the cursor.

Use retweeters when no added commentary is required. Use tweet replies for the
direct conversation. Use tweet thread for connected posts in one thread.
