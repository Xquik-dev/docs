...
description: "Read participant-scoped Twitter DMs with a connected account. Store private message rows, preserve sender and recipient IDs, and resume pages with next_cursor."
api: "GET /x/dm/{userId}/history"
keywords: ["Twitter DM API", "Twitter API DM", "direct messages", "DM history", "Twitter DM history API", "X DM history API", "direct message export", "next_cursor", "participant-scoped DM"]
---
...
<ResponseExample>
  ...
  ```json 400 · x/dm-history
  {
    "error": "invalid_user_id"
  }
  ```

  ```json 401 · x/dm-history
  {
...
Pass the connected participant account through `account`. Store message IDs and
...
<ResponseField name="messages" type="object[]">
  ...
  **Message object fields:**
    <ResponseField name="id" type="string">Identifies the message.</ResponseField>
    <ResponseField name="text" type="string">Contains message text when available.</ResponseField>
    <ResponseField name="senderId" type="string">Identifies the sender.</ResponseField>
    <ResponseField name="receiverId" type="string">Identifies the recipient.</ResponseField>
    <ResponseField name="createdAt" type="string">Reports the ISO 8601 timestamp when available.</ResponseField>
    <ResponseField name="mediaUrl" type="string">Links attached media when present.</ResponseField>
...
Store each message ID before requesting the next page. Keep message text private.

### Can I Send a Reply Through This Endpoint?

No. This endpoint reads history only. Use [Send DM](/api-reference/x-write/send-dm)
for replies. Store its `messageId` beside the source DM ID.
...
Poll from the last stored cursor when your workflow needs newer context.

### How Do I Authenticate?

Send an Xquik API key or OAuth bearer token. Keep the credential server-side.
Pass a connected participant handle through `account`.

### How Do I Handle a Rate Limit?

Wait for `Retry-After`, then retry the same page. Do not advance its cursor.

### Do I Need an X Developer Account?

No official X developer account is required for this Xquik endpoint. Create an
Xquik API key and connect a participating X account. Xquik is an independent
...
Yes. Read participant-scoped history and deduplicate messages by ID. Route new
DMs into a private support queue. Require approval when your policy needs it.
Then reply through [Send DM](/api-reference/x-write/send-dm). Store the returned
`messageId` beside the source DM ID.

### How Does This Differ From X's Official DM API?

This page documents Xquik's normalized Twitter DM API. It does not document
X's official API. Compare source concepts with
...
<Note>
  **Related:** [Direct Message Workflow](/guides/direct-message-workflow) for
  ...
  handoff; [Get User](/api-reference/x/twitter-profile-lookup) to resolve the recipient
