...
<CodeGroup>
  ...
  for (let pageIndex = 0; pageIndex < 3; pageIndex += 1) {
    ...
    const nextResponse = await fetch(
      `https://xquik.com/api/v1/x/users/${userId}/tweets?${new URLSearchParams({ cursor: pageCursor })}`,
      { headers: { "x-api-key": "xq_YOUR_KEY_HERE" } },
    );
...
cursor fields so a worker can resume from the last saved `next_cursor`.
