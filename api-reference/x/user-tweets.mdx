...
description: "Search one Twitter or X profile's tweets with full text, replies, reposts, likes, quotes, views, attached media, and cursor pagination."
api: "GET /x/users/{id}/tweets"
keywords: ["search user tweets", "Twitter user tweets", "get user timeline", "user timeline API", "profile timeline", "X user timeline", "tweet scraper", "X API"]
---
...
Use these checkpoints when a timeline sync needs to switch between plain profile
posts, replies, media-only rows, and resumable page pulls.
...
<ParamField query="includeReplies" type="boolean" default="false">
  Include reply tweets in the profile timeline. Defaults to `false`, which
  returns original profile posts without replies.
</ParamField>
