...
description: "Retrieve one user's X profile timeline with cursor-based pagination, engagement metrics, replies, and media"
api: "GET /x/users/{id}/tweets"
keywords: ["get user timeline", "user timeline api", "user tweets", "profile timeline", "X user timeline", "x api"]
---
...
one user's profile timeline. This Get User Timeline endpoint accepts either a
username or numeric user ID and returns recent public posts from that profile.
...
<Note>
  **Related:** [User Media](/api-reference/x/user-media) · [User Likes](/api-reference/x/user-likes) · [User Mentions Timeline](/api-reference/x/user-mentions)
</Note>
