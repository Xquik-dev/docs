...
extract the final status ID first; URLs, usernames, and short IDs return
`400 invalid_tweet_id`. Use the response when you need normalized text,
optional author data, engagement counts, quote metadata, and media URLs for 1
record.
...
Use `GET /x/tweets/{id}` when a workflow needs one tweet plus author context.
The Node.js and Python snippets shape a durable handoff row instead of printing
the full lookup response. Store `tweet_id`, `text`, `author_id`,
`author_username`, `author_followers`, `author_verified`,
`author_profile_picture`, `created_at`, and `media_urls` with the downstream
record.
