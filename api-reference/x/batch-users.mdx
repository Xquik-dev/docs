...
description: "Retrieve up to 100 X user profiles by ID in one request, including usernames, bios, verification state, follower counts, and profile media. See costs."
...
JSON response. Use [Get User](/api-reference/x/twitter-profile-lookup) when you have one ID
...
Treat missing IDs explicitly. Do not shift remaining profiles into another
...
before starting the next. This makes a failed enrichment job safe to resume.
