...
Use `GET /x/articles/{tweetId}` when a workflow needs one long-form post body
plus article metadata. The Node.js and Python snippets shape a durable handoff
row instead of printing the full lookup response. Store `tweet_id`,
`article_title`, `preview_text`, `author_username`, `created_at`,
`cover_image_url`, and `body_text` with the downstream record.

## Path parameters

<ParamField path="tweetId" type="string" required>
  Numeric tweet ID of the X Article, 15-20 digits. If you have a tweet URL,
  use the final status ID.
</ParamField>
