...
<CardGroup cols={2}>
  ...
  <Card title="Single tweet" icon="message-square">
    Store `tweetId` and `cacheHit`. `cacheHit: true` means the single-tweet request used cached media and is free.
  </Card>
  <Card title="Bulk result" icon="list-checks">
    Store `totalTweets` and `totalMedia`. `totalTweets` counts successful tweets with media after invalid or failed IDs are skipped.
  </Card>
