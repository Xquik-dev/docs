...
description: "Download images, videos, and animated GIFs from 1-50 tweets. Return a gallery URL and download metadata for bulk review. Includes API request examples."
...
Use this Twitter media downloader API with 1-50 tweet IDs or URLs. It downloads
photos, videos, and GIFs into a review gallery. Use it to download Twitter media
from one tweet or a 50-tweet batch. Each response includes gallery metadata.
...
| --- | --- | --- |
| `download_request_id` | Integration value | Link the selected tweet inputs to one gallery. |
...
| Partial bulk result | `totalTweets` below valid input count | Store successes and review the skipped Tweet IDs. |
| No media | `400 no_media` | Keep the Tweet ID and choose another source tweet. |
| Invalid bulk items | Valid IDs remain | Continue with valid IDs and preserve rejected inputs. |
| More than 50 items | Request exceeds the limit | Split the backfill into groups of 50 or fewer. |
...
No. This Twitter video downloader route creates a gallery. It does not expose
per-file downloads or resolution controls.

### Can I Download Media From Many Tweets?

Yes. Send 1-50 tweet IDs through `tweetIds`. The response returns one combined
gallery with tweet and media counts.
