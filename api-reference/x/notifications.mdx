...
keywords: ["Twitter notifications API", "Twitter API webhook", "X notifications API", "get notifications", "mentions notifications", "verified notifications", "notification triage", "next_cursor", "agent inbox"]
---
...
`has_next_page` is true. Commit the page's notification rows before advancing
the checkpoint.

Use an atomic commit when the destination supports transactions. Otherwise,
upsert rows by notification ID and update the cursor after durable writes
finish. Keep the preceding cursor until the new checkpoint is confirmed. This
sequence prevents skipped notifications after a worker failure.
...
Activity API for real-time account events. [Its official documentation covers
mentions, replies, reposts, likes, follows, and direct messages delivered by
webhook.](https://docs.x.com/x-api/account-activity/introduction)
...
No. `GET /x/notifications` is read-only. Deleting a local triage row does not
remove the notification from X or another connected client.

### What Happens When a Notification Request Fails?

A `401` response requires a valid key for the connected account. A `402`
response requires more credits. Respect `Retry-After` guidance for `429`.
Retry temporary `424` and `502` failures without advancing the saved cursor.
