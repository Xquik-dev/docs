...
confirms storage. Keep the preceding cursor until confirming the new
checkpoint. This order prevents skipped notifications after a worker failure.
...
needs more credits. Wait for `Retry-After` after `429`. After `424` or `502`,
retry from the saved cursor.
