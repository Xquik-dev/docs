...
api: "GET /x/bookmarks"
keywords: ["export Twitter bookmarks", "Twitter bookmarks API", "Twitter API get bookmarks", "Twitter saved tweets", "bookmark export", "CSV file", "bookmark folders"]
---
...
Run the Node.js or Python example for each cursor page. Both examples write one
JSON Lines row per saved tweet. Each row keeps tweet text, authors, likes,
...
the stable key. Map explicit columns before importing rows into Notion
databases, spreadsheets, CRMs, or research queues.

### How Does Bookmark Authentication Work?

Connect the X account that owns the saved tweets. Send its Xquik API key with
each request. The endpoint never accepts a username for another account's
bookmarks.
...
bookmarks. Store one `next_cursor` checkpoint per folder.

### What Limits a Bookmark Export?

The endpoint returns bookmarks currently visible to the connected account. A
low balance can shorten a page. A `429` response includes retry guidance. A
`424` or `502` response means the read service failed temporarily.

### How Should I Store a Bookmark Export?

Store the connected account ID beside each row. Deduplicate by account, folder,
and tweet ID. Keep private folder names out of public logs. Delete temporary
files after the approved handoff finishes.
