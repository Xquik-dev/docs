...
Map explicit columns before importing rows into Notion databases or CRMs.

### How Does Bookmark Authentication Work?

Connect the X account that owns the saved tweets. Include that account's Xquik
API key in every request. The endpoint never accepts another username. This
matches the Twitter developer privacy model.
...
You receive the bookmarks that the connected account can view. A low balance
can shorten a page. A `429` response includes retry guidance. A `424` or `502`
response means the read service failed temporarily.

### How Should I Store a Bookmark Export?

Store the connected account ID beside each row. Use the account ID, folder ID,
and tweet ID as the unique row key. Keep private folder names out of public logs.
