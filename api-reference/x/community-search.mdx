---
title: "X community tweet search API & keyword results"
sidebarTitle: Search community posts
description: "Search tweets inside one X community by community ID and keyword. Return authors, text, engagement counts, media, and cursor pagination."
...
Search one X community without scanning unrelated timelines. Store tweet IDs,
authors, engagement counts, media, and cursors for exports.
...
<CardGroup cols={2}>
  ...
  <Card title="Equivalent scoped route" icon="search">
    ...
    when your integration already uses that route name. It accepts the same
...
Search by community name, topic, or description. Keep the submitted query beside the returned ranking.

Store each community ID, name, description, and member count. Use the stable community ID for later community routes.

Review candidate states before building a workflow. Validate the selection through the community information route. Search order can change between requests.

Use community tweet search only after selecting a community. This route discovers communities, not their tweets.
