...
| --- | --- |
| `users[]` | The follower profile rows returned on this page. |
| `users[].id` | Stable X user key for upserts. Store it as `x_user_id`. |
| `users[].username` and `users[].name` | Handle and display name for CRM labels, enrichment, and dedupe. |
| `users[].description`, `location`, and `url` | Profile enrichment columns. These fields are omitted when empty. |
| `users[].followers`, `users[].following`, `verified`, and `verifiedType` | Segmentation, scoring, and account-quality filters. |
| `users[].profilePicture` and `coverPicture` | Profile media URLs for enrichment or review queues. |
| `has_next_page` and `next_cursor` | Cursor handoff. Continue only when `has_next_page` is true, and pass `next_cursor` back as `cursor`. |
