...
<CodeGroup>
  ...
  for (let pageIndex = 0; pageIndex < 3; pageIndex += 1) {
    ...
    const response = await fetch(
      `https://xquik.com/api/v1/x/users/${userId}/followers?${params}`,
  ...
  for page_index in range(3):
      ...
      response = requests.get(
          f"https://xquik.com/api/v1/x/users/{user_id}/followers",
          params=params,
          headers={"x-api-key": "xq_YOUR_KEY_HERE"},
      )
      page = response.json()
...
Use `GET /x/users/{id}/followers` when a CRM, warehouse, audience, or agent
workflow needs follower rows now. The examples above write JSON Lines rows with
`source_user_id`, `x_user_id`, `x_username`, `display_name`, profile
enrichment, segmentation fields, `page_index`, `page_cursor`, `next_cursor`,
and `has_next_page` for imports or upserts. Use
