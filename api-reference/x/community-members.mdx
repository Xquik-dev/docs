...
bio, location, and verification. Record an ISO 8601 collection time for every
page. Keep each available follower count, following count, profile image, and
page cursor.
...
route for role-specific profiles. The members route returns the complete member
roster.
...
Treat `has_next_page` as the only pagination guard. The next request requires
the boolean value `true`. Pass `next_cursor` without modification. Persist each
page before saving its cursor. Deduplicate by user ID when a resumed job repeats
its previous page.

Profile fields can change after collection. Record an ISO 8601 time on each
profile row. Refresh the profile through user lookup when the workflow requires
...
<ResponseField name="users" type="object[]">
  ...
  **User object fields:**
    ...
    <ResponseField name="protected" type="boolean">Indicates whether the account protects its Tweets. Omitted if unavailable.</ResponseField>
