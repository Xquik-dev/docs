...
match a keyword. Join those posts to member profiles by `manifest_id`,
`community_id`, and stable user ID.
...
Request another page only when `has_next_page` is exactly `true`. Pass
`next_cursor` without modification. Persist each page before saving its cursor.
Deduplicate by user ID when a resumed job repeats the previous page.
