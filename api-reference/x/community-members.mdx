...
when `has_next_page` is true. Pass `next_cursor` back unchanged. Deduplicate a
...
Request the next page only when `has_next_page` is true. Pass `next_cursor`
back unchanged. Save each profile page before advancing its cursor.

Deduplicate resumed pages by stable user ID. Preserve the community ID on
every row. Record the collection time because usernames, bios, verification,
and audience counts can change. Do not infer moderator status from membership.
...
Report newly observed and missing members separately. Do not infer why a user
joined or left. Refresh a profile when later work needs current counts or bio.
...
## Build a community membership roster

Use this route to enumerate profiles that belong to one community. Keep the
community ID on every exported row. A username alone does not preserve the
membership source.
...
Use the roster for member directories, CRM enrichment, or moderation review.
Do not label every member as a moderator. Use the moderators route for that
smaller role-specific group.
...
Profile fields can change after collection. Record the collection time beside
the row. Refresh the profile through user lookup when a later workflow needs
current biography, counts, or avatar details.
...
Use a separate role column when another process joins moderator status onto
the member roster.
