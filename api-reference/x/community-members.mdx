...
bio, location, and verification. Record each available follower count,
following count, profile image, page cursor, and collection timestamp.

Save the page before advancing its opaque cursor. Request another page only
when `has_next_page` is exactly `true`. Reuse the exact `next_cursor`. Deduplicate a
...
profiles. The members route returns the complete member roster.
...
Complete every cursor page before marking the community member export complete.
...
posts. Use [Community Search](/api-reference/x/community-search) for Community
posts that contain a keyword. Join those posts to member profiles by `manifest_id`,
...
- Record the Community ID, page cursor, and collection timestamp.

Build member directories, enrich CRM contacts, or review moderators from this
Community member roster.
Do not label every member as a moderator. Use the moderators route for that
role-specific moderator roster.
...
Profile fields can change after collection. Stamp each profile row with its
collection timestamp. Refresh the profile through user lookup when the
workflow requires current bios, follower counts, following counts, or avatars.
