...
requesting another cursor. Keep each available follower count, following
count, and profile image. Store the cursor returned with each profile batch.
...
[Community Search](/api-reference/x/community-search) for posts matching a
search term. Join those posts to member profiles by `manifest_id`, `community_id`,
and stable user ID.
...
<ResponseField name="next_cursor" type="string">Cursor for subsequent results. Pass as the `cursor` query parameter.</ResponseField>
