...
Store every returned user ID, username, profile name, bio, verification state,
follower count, and page cursor.
...
Build one roster row per community and user ID. Keep the collection time and
source cursor beside each row. Deduplicate resumed pages by stable user ID.

Use JSON for app ingestion and queue handoffs. Create CSV or XLSX downstream
when analysts need spreadsheets. Preserve the profile fields returned by the
API instead of flattening every member into a username.
