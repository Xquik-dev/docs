---
title: "Live X community tweet search"
sidebarTitle: "Search community tweets"
description: "Search tweets inside one X community by community ID and keyword. Return authors, text, engagement metrics, media, and cursor pages."
api: "GET /x/communities/tweets"
keywords: ["X community tweet search", "search X community tweets", "Twitter community search API", "community tweets", "keyword search", "live review queue"]
---

import LlmsDirective from "/snippets/llms-directive.mdx";
import AudienceCommunityApiLinks from "/snippets/x-audience-community-api-links.mdx";

<LlmsDirective />
<AudienceCommunityApiLinks />

<Note>
  Requested result counts are upper bounds for paid authenticated calls. When remaining credits cannot cover the full page or ID list, Xquik returns fewer results. If zero paid results are affordable, it returns `402 insufficient_credits`.
</Note>

<Callout icon="coins" color="#5c3327">
  **1 credit per tweet returned** · [All plans](https://xquik.com/#pricing) from $0.00012/credit
</Callout>

<CodeGroup>
  ```bash cURL
  curl -G https://xquik.com/api/v1/x/communities/tweets \
    --data-urlencode "communityId=1234567890" \
    --data-urlencode "q=machine learning" \
    -H "x-api-key: xq_your_api_key_here" | jq
  ```

  ```javascript Node.js
  const communityId = "1234567890";
  const searchQuery = "machine learning";
  const queryType = "Latest";
  const pageSize = "100";
  const params = new URLSearchParams({ communityId, q: searchQuery, queryType, pageSize });
  const response = await fetch(`https://xquik.com/api/v1/x/communities/tweets?${params}`, {
    headers: { "x-api-key": "xq_your_api_key_here" },
  });
  const data = await response.json();
  const tweetRows = data.tweets.map((tweet) => {
    const author = tweet.author ?? {};

    return {
      community_id: communityId,
      search_query: searchQuery,
      query_type: queryType,
      tweet_id: tweet.id,
      text: tweet.text,
      author_id: author.id ?? null,
      author_username: author.username ?? null,
      author_name: author.name ?? null,
      author_followers: author.followers ?? null,
      author_verified: author.verified ?? null,
      author_profile_picture: author.profilePicture ?? null,
      created_at: tweet.createdAt ?? null,
      like_count: tweet.likeCount ?? null,
      reply_count: tweet.replyCount ?? null,
      retweet_count: tweet.retweetCount ?? null,
      media_urls: tweet.media?.map((item) => item.mediaUrl).filter(Boolean) ?? [],
    };
  });
  const nextCursor = data.has_next_page ? data.next_cursor : null;

  for (const row of tweetRows) {
    process.stdout.write(`${JSON.stringify(row)}\n`);
  }
  if (nextCursor !== null) {
    const checkpoint = {
      community_id: communityId,
      search_query: searchQuery,
      query_type: queryType,
      page_size: Number(pageSize),
      next_cursor: nextCursor,
    };
    process.stdout.write(`${JSON.stringify(checkpoint)}\n`);
  }
  ```

  ```python Python
  import json
  import requests

  community_id = "1234567890"
  search_query = "machine learning"
  query_type = "Latest"
  page_size = 100
  response = requests.get(
      "https://xquik.com/api/v1/x/communities/tweets",
      params={
          "communityId": community_id,
          "q": search_query,
          "queryType": query_type,
          "pageSize": page_size,
      },
      headers={"x-api-key": "xq_your_api_key_here"},
  )
  data = response.json()
  tweet_rows = []
  for tweet in data["tweets"]:
      author = tweet.get("author") or {}
      tweet_rows.append(
          {
              "community_id": community_id,
              "search_query": search_query,
              "query_type": query_type,
              "tweet_id": tweet["id"],
              "text": tweet.get("text"),
              "author_id": author.get("id"),
              "author_username": author.get("username"),
              "author_name": author.get("name"),
              "author_followers": author.get("followers"),
              "author_verified": author.get("verified"),
              "author_profile_picture": author.get("profilePicture"),
              "created_at": tweet.get("createdAt"),
              "like_count": tweet.get("likeCount"),
              "reply_count": tweet.get("replyCount"),
              "retweet_count": tweet.get("retweetCount"),
              "media_urls": [
                  item["mediaUrl"]
                  for item in tweet.get("media", [])
                  if item.get("mediaUrl")
              ],
          }
      )

  next_cursor = data["next_cursor"] if data["has_next_page"] else None

  for row in tweet_rows:
      print(json.dumps(row))
  if next_cursor is not None:
      print(json.dumps({
          "community_id": community_id,
          "search_query": search_query,
          "query_type": query_type,
          "page_size": page_size,
          "next_cursor": next_cursor,
      }))
  ```
</CodeGroup>

The Node.js & Python snippets shape one durable row per matching community
tweet instead of printing the full response page. Persist the final
`next_cursor` row when `has_next_page` is true, then pass it back as `cursor`
with the same `communityId`, `q`, `queryType`, and `pageSize`.

## Direct community tweet search handoff

Use `GET /x/communities/tweets` when a monitoring job, research queue,
moderation review, social listening workflow, or agent needs matching tweets
from one known X community.

Store `community_id`, `search_query`, `query_type`, `tweet_id`, `text`, `author_id`,
`author_username`, `author_name`, `author_followers`, `author_verified`,
`author_profile_picture`, `created_at`, engagement counts, & `media_urls` for
each row. Keep
`has_next_page` & `next_cursor` with the export checkpoint so the next run can
continue the same scoped search without duplicating earlier rows.

Set `queryType=Latest` for recent queues or backfills. Set `queryType=Top` for
relevance-ranked review.

<CardGroup cols={2}>
  <Card title="Search row checkpoint" icon="search">
    Store `community_id`, `search_query`, `query_type`, `page_size`,
    `has_next_page`, and `next_cursor` with the tweet rows.
  </Card>
  <Card title="Sort mode" icon="arrow-down-up">
    Use `Latest` for recent collection and `Top` for relevance-ranked review.
    Keep the same `queryType` when you pass a cursor.
  </Card>
  <Card title="Default page" icon="rows-3">
    Request 1 to 100 tweets with `pageSize`. The default is 20. Treat the value
    as an upper bound because filters, source results, or credits can return fewer.
  </Card>
  <Card title="Saved export" icon="file-spreadsheet">
    Use `community_search` with `targetCommunityId` and `searchQuery` when the
    workflow needs a saved job with CSV/JSON/XLSX output.
  </Card>
</CardGroup>

## Feed a live community review queue

Choose this route for direct, page-by-page community tweet retrieval. It works
well behind moderation screens, support consoles, and analyst dashboards.

Build the first request from the selected community and visible filter. Show
the active query and sort order above the results. Reviewers should always know
why each tweet appeared.

Render concrete tweet fields:

- Tweet text and creation time.
- Author name, username, and verification state.
- Like, reply, and repost counts.
- Attached photo or video URLs.
- The tweet ID for follow-up actions.

Keep `next_cursor` outside the visible tweet list. Attach it to the current
community, query, sort, and page size. Disable the next-page control when
`has_next_page` is false.

Use `Latest` for fast-moving discussions. New tweets may arrive between page
requests. Deduplicate the interface by tweet ID. Use `Top` for a
relevance-oriented review where recency matters less.

Treat an empty page as a valid search result. It does not mean the community
is missing. Distinguish empty matches from authentication, credit, and request
errors in the interface.

## Hand tweets to another action

Pass only reviewed tweet IDs into later workflows. A moderator can open the
tweet, inspect replies, or save media. A support agent can copy the author
username into a profile lookup. A monitoring worker can preserve the tweet ID
as evidence.

Do not pass a displayed row position as identity. Result order can change.
Use the tweet ID and community ID for every durable handoff.

## Design the reviewer interface

Place the query, community, and sort mode above the tweet list. Show loading,
empty, error, and final-page states separately.

Keep the next-page action disabled during a request. Append new rows only
after verifying the response. Reject a repeated cursor before requesting
another page.

Allow reviewers to copy tweet IDs and open source URLs. Keep profile lookup,
reply inspection, and media download as explicit actions. Do not trigger them
automatically for every row.

For shared queues, store the reviewer decision beside the tweet ID. A changed
result order should not move that decision to another tweet.

## Handle reviewer pagination

Keep the current cursor in server-side queue state. Do not expose it as an
editable field.

When a reviewer changes the query, clear existing rows and cursor state. Show
that a new result stream has started.

Preserve selected tweet IDs across harmless interface refreshes. Clear them
only after an explicit decision or query change.

Display a source link only when `tweets[].url` is present. Otherwise, show the
tweet ID as the fallback identifier.

Keep selection state independent from pagination. Returning to an earlier page
should restore decisions by tweet ID, not row position.

Show the collection time beside loaded pages. Reviewers can then distinguish
new results from an older cached queue without inspecting server logs.

## Preserve Decisions Across a Live Moderation Queue

Create one queue identity from the community ID, query, and sort mode. Keep
that identity unchanged while reviewers page through tweets.

Store each decision against `tweet.id`. Never attach a label to the visible
row number. New posts can change ordering between `Latest` requests.

Separate four queue states in the interface:

- Unreviewed tweets loaded from the current page.
- Selected tweets waiting for an explicit action.
- Reviewed tweets with a stored decision.
- Failed pages that still retain their previous cursor.

When a page request fails, keep the current rows and cursor. Show the error
beside the next-page control. Retry with the same query parameters.

When `has_next_page` is false, mark the loaded stream complete. Do not create
another request from an empty or remembered cursor. A later refresh starts a
new review window.

Treat `Latest` as a moving queue. Deduplicate incoming rows by tweet ID before
display. Keep earlier decisions when the same tweet appears again.

Treat `Top` as a relevance review. Record the collection time because ranking
can change. Do not compare row positions across separate requests.

Pass reviewed tweet IDs into reply, thread, profile, or media workflows only
after approval. Keep each follow-up result separate from the search response.
This preserves the evidence that caused the reviewer to act.

For shared teams, record reviewer identity and decision time outside tweet
fields. The API returns post and author facts. Your queue owns moderation
state.

## Build a Query-Specific Community Review Batch

Save the exact search query before requesting tweets. Keep every operator,
quoted phrase, exclusion, language choice, and engagement filter. Reviewers
must see the expression that produced the queue.

Store Tweet ID, author ID, text, creation time, engagement, and media for every
match. Add the request time and returned cursor. Keep reviewer fields in a
separate table keyed by Tweet ID.

Assign one review purpose to each batch. Examples include campaign replies,
support complaints, product feedback, rule violations, or event coverage.
Avoid mixing unrelated queries in one queue.

Persist the page before advancing its cursor. Deduplicate repeated Tweet IDs
after retries. Never discard a prior reviewer decision when engagement counts
change.

Record an explicit terminal state. Use completed, capped, credit-bounded, or
interrupted. A live search page never proves a complete historical community
archive.

## Separate Search Matches From Community Feed Coverage

This route returns community tweets matching a search expression. It does not
return every recent post. Use the community feed route for a cursor-based
timeline without a query.

Keep search-match counts separate from total community activity. A narrow
query can return zero rows while the community remains active. A broad query
can produce more review work without improving relevance.

Test the expression before opening a long review queue. Inspect several Tweet
IDs, authors, timestamps, replies, reposts, likes, and media. Narrow recurring
false matches with supported operators.

When comparing two queries, use the same review window and result cap. Report
unique Tweet IDs for each expression. List overlapping matches separately.
Never combine both result sets under one unrecorded query label.

## Query parameters

<ParamField query="communityId" type="string" required>
  Numeric ID of the community whose tweets you want to search.
</ParamField>

<ParamField query="q" type="string" required>
  Search query for community tweets.
</ParamField>

<ParamField query="queryType" type="string">
  Sort order. `Top` returns most relevant tweets, `Latest` returns most recent. Defaults to `Latest`.
</ParamField>

<ParamField query="cursor" type="string">
  Pagination cursor from a previous response. Omit for the first page.
</ParamField>

<ParamField query="pageSize" type="number">
  Upper bound for tweets per page. Range: 1-100. Default: `20`.
</ParamField>

## Which community search route?

<CardGroup cols={2}>
  <Card title="Scoped community search" icon="search">
    Use `GET /x/communities/tweets` with `communityId` and `q` for scoped search.
  </Card>
  <Card title="Community search route" icon="list-filter">
    Use [`GET /x/communities/search`](/api-reference/x/community-search) when
    your integration already uses that route name. It accepts the same
    `communityId`, `q`, `queryType`, `cursor`, and `pageSize` shape.
  </Card>
  <Card title="Known community posts" icon="message-square-text">
    Use [`GET /x/communities/{id}/tweets`](/api-reference/x/community-tweets)
    for posts from one known community ID.
  </Card>
  <Card title="Bulk community jobs" icon="file-spreadsheet">
    Use [`Create extraction`](/api-reference/extractions/create) with
    `community_search` with `targetCommunityId` and `searchQuery` when the
    workflow needs a saved filtered export. Use `community_post_extractor` for
    all posts from a known community.
  </Card>
</CardGroup>

## Headers

<ParamField header="x-api-key" type="string">
  Full account key. Sessions and OAuth also work.
</ParamField>

<ParamField header="Authorization" type="string">
  `Bearer xq_your_guest_key_here` for `paid_reads`.
</ParamField>

## Response

### 200 OK

<ResponseField name="tweets" type="object[]">
  Array of matching community tweets.
  **Tweet object fields:**
    <ResponseField name="id" type="string">Tweet ID.</ResponseField>
    <ResponseField name="text" type="string">Tweet text.</ResponseField>
    <ResponseField name="type" type="string">Tweet type. Omitted if unavailable.</ResponseField>
    <ResponseField name="createdAt" type="string">ISO 8601 creation timestamp.</ResponseField>
    <ResponseField name="isNoteTweet" type="boolean">Whether this is a Note Tweet. Omitted if unavailable.</ResponseField>
    <ResponseField name="likeCount" type="number">Like count. Omitted if unavailable.</ResponseField>
    <ResponseField name="retweetCount" type="number">Retweet count. Omitted if unavailable.</ResponseField>
    <ResponseField name="replyCount" type="number">Reply count. Omitted if unavailable.</ResponseField>
    <ResponseField name="quoteCount" type="number">Quote tweet count. Omitted if unavailable.</ResponseField>
    <ResponseField name="viewCount" type="number">View count. Omitted if unavailable.</ResponseField>
    <ResponseField name="bookmarkCount" type="number">Bookmark count. Omitted if unavailable.</ResponseField>
    <ResponseField name="url" type="string">Permalink URL on X. Omitted if unavailable.</ResponseField>
    <ResponseField name="lang" type="string">Tweet language code. Omitted if unavailable.</ResponseField>
    <ResponseField name="isReply" type="boolean">Whether the tweet is a reply. Omitted if unavailable.</ResponseField>
    <ResponseField name="inReplyToId" type="string">Tweet ID being replied to. Omitted if not a reply.</ResponseField>
    <ResponseField name="inReplyToUserId" type="string">User ID being replied to. Omitted if unavailable.</ResponseField>
    <ResponseField name="inReplyToUsername" type="string">Username being replied to. Omitted if unavailable.</ResponseField>
    <ResponseField name="conversationId" type="string">Conversation thread ID. Omitted if unavailable.</ResponseField>
    <ResponseField name="source" type="string">Client used to post the tweet. Omitted if unavailable.</ResponseField>
    <ResponseField name="displayTextRange" type="number[]">Start and end offsets for rendered tweet text. Omitted if unavailable.</ResponseField>
    <ResponseField name="isLimitedReply" type="boolean">Whether replies are limited. Omitted if unavailable.</ResponseField>
    <ResponseField name="isQuoteStatus" type="boolean">Whether this tweet quotes another tweet. Omitted if unavailable.</ResponseField>
    <ResponseField name="entities" type="object">Parsed entities. Omitted if unavailable.</ResponseField>
    <ResponseField name="contentDisclosure" type="object">Disclosure metadata for paid partnership and AI-generated media labels. Includes `advertising.isPaidPromotion` and `aiGenerated.hasAiGeneratedMedia` when X returns them. Omitted if unavailable.</ResponseField>
    <ResponseField name="author" type="object">
      Tweet author profile. Omitted if unavailable.
      **Author object fields:**
        <ResponseField name="id" type="string">Author user ID.</ResponseField>
        <ResponseField name="username" type="string">Author handle without `@`.</ResponseField>
        <ResponseField name="name" type="string">Author display name. Omitted if unavailable.</ResponseField>
        <ResponseField name="followers" type="number">Follower count. Omitted if unavailable.</ResponseField>
        <ResponseField name="verified" type="boolean">Whether the author is verified. Omitted if unavailable.</ResponseField>
        <ResponseField name="profilePicture" type="string">Author profile image URL. Omitted if unavailable.</ResponseField>

    </ResponseField>
    <ResponseField name="media" type="object[]">
      Media attachments. Omitted when the tweet has no media.
      **Media object fields:**
        <ResponseField name="mediaUrl" type="string">Direct media URL.</ResponseField>
        <ResponseField name="videoVariants" type="object[]">Available video renditions with bitrate, content type, and URL. Omitted for images.</ResponseField>
        <ResponseField name="type" type="string">Media type.</ResponseField>
        <ResponseField name="url" type="string">Shortened URL from the tweet text.</ResponseField>

    </ResponseField>
    <ResponseField name="quoted_tweet" type="object">Embedded quoted tweet. Omitted if not a quote tweet.</ResponseField>
    <ResponseField name="retweeted_tweet" type="object">Original retweeted tweet. Omitted if not a retweet.</ResponseField>

</ResponseField>
<ResponseField name="has_next_page" type="boolean">Whether more results are available.</ResponseField>
<ResponseField name="next_cursor" type="string">Cursor for the next page. Pass as the `cursor` query parameter.</ResponseField>

```json
{
  "tweets": [
    {
      "id": "1893456789012345678",
      "text": "Interesting ML discussion in this community",
      "createdAt": "2026-02-24T10:00:00.000Z",
      "likeCount": 200,
      "retweetCount": 35,
      "replyCount": 15,
      "viewCount": 8500,
      "author": {
        "id": "987654321",
        "username": "mlresearcher",
        "name": "ML Researcher",
        "followers": 25000,
        "verified": true,
        "profilePicture": "https://pbs.twimg.com/profile_images/example.jpg"
      }
    }
  ],
  "has_next_page": true,
  "next_cursor": "DAACCgACGE..."
}
```

### 400 Missing query

```json
{ "error": "missing_query" }
```
The `q` query parameter is empty or missing.

### 401 Unauthenticated

Anonymous requests get `WWW-Authenticate: Bearer` and a guest wallet checkout action. This is not a Payment challenge.
```json
{ "error": "unauthenticated" }
```
Missing or invalid API key.

### 402 Payment required

Account keys get account options; guest keys get guest top-up only.
No checkout starts automatically. Confirm any payment action.

### 502 X API unavailable

```json
{ "error": "x_api_unavailable" }
```
The read service returned an error. Retry after a short delay.

### 429 Rate Limit Exceeded

```json
{ "error": "rate_limit_exceeded", "retryAfter": 60 }
```
Your tier rate limit was exceeded. Wait for the `Retry-After` header before retrying.

### 424 Dependency Failed

```json
{ "error": "x_api_unavailable" }
```
The normalized v1 response contract can return 424 when the read service is unavailable.

<Note>
  **Next steps:** [Community Search](/api-reference/x/community-search) for an alternative search endpoint, or [Search Tweets](/api-reference/x/search-tweets) for general tweet search.
</Note>
