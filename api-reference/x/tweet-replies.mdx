...
| Store this field | Why it matters |
|------------------|----------------|
| `tweets[]` | Reply rows for moderation queues, support triage, giveaway audits, or agents |
| `tweets[].id` | Stable reply key for dedupe, labels, and follow-up actions |
| `tweets[].text` and `tweets[].createdAt` | Reply context and time ordering |
| `tweets[].author.id` and `tweets[].author.username` | Stable author key and readable handle |
| `tweets[].inReplyToId` and `conversationId` | Parent tweet and thread joins |
| Engagement counts | Prioritize high-signal or high-risk replies |
| `tweets[].media`, `quoted_tweet`, `retweeted_tweet` | Preserve attachments and relationship context when available |
| `has_next_page` and `next_cursor` | Fetch older reply pages by passing `next_cursor` as `cursor` |
