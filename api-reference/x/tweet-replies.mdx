...
<ResponseExample>
  ...
  ```json 424
  {
    ...
    "message": "Replies are incomplete. Use tweet search with a conversation_id filter."
  }
  ```
...
<Warning>
  Reply visibility depends on X. An unfiltered first page can return
  `424 replies_incomplete` when X reports more replies than it exposes. Use
  tweet search as the broader fallback. Set `conversation_id:{id}` before
  collecting fallback rows.
...
lists reply coverage and every optional field.

## Twitter API Reply Questions

### How Does the Twitter API Get Replies to a Tweet?

Start with the original tweet's numeric ID. Request the first reply page with
`GET /api/v1/x/tweets/{id}/replies`. Set `pageSize` from `1` through `100`.
...
X controls which public replies each page exposes. Protected, deleted, hidden,
or unavailable replies may not appear. A `424 replies_incomplete` response
marks the first page as incomplete. Search the same `conversation_id` for
broader coverage. Filter `inReplyToId` when only direct replies belong in the
result. Never promise a complete set without a terminal cursor.

### How Can a Team Analyze or Moderate Replies?

Store the reply ID, text, author ID, username, and creation time. Keep likes,
reposts, quotes, views, bookmarks, and attached media. Build a moderation table
with one row per reply. Send uncertain rows to human reviewers. Add sentiment
or spam labels with your own classifier. Xquik does not return a sentiment
verdict.
...
Each returned reply costs 1 credit. Request only the page size your workflow
can store safely. Use `sinceTime` and `untilTime` for bounded support periods.
Save every completed page before requesting another cursor. Handle `402`
before retrying. Use `reply_extractor` when a fixed `resultsLimit` and saved
file matter more than current reply pages.
...
<CodeGroup>
  ...
  # Request the largest supported reply page
  curl -G "https://xquik.com/api/v1/x/tweets/1893456789012345678/replies" \
...
A `424 replies_incomplete` response means the first page is not exhaustive.
Search tweets with `conversation_id:<tweet_id>` as the broader fallback. Keep
only rows whose `inReplyToId` matches the requested tweet when needed.
...
Direct replies cost 1 credit per tweet returned. Low credit balances can return fewer replies than a full page; zero affordable results return `402 insufficient_credits`. Retry `429` with the `Retry-After` header. Retry `502 x_api_unavailable` after a short backoff. For `424 replies_incomplete`, use tweet search with `conversation_id:<tweet_id>`.
...
| Time window | `sinceTime` and `untilTime` | Reuse identical Unix-second bounds after a retry. |
| Completion state | `has_next_page` plus the terminal cursor | Mark the export complete only after the final page. |
| Incomplete first page | `424 replies_incomplete` | Stop exhaustive verification and search the conversation ID. |
...
<ParamField query="pageSize" type="number" default="20">
  Maximum replies requested from this page. Use a value from `1` to `100`.
...
<ResponseField name="tweets" type="object[]">
  ...
  <Expandable title="Tweet object fields">
    ...
    <ResponseField name="lang" type="string">This value contains the tweet language code when available.</ResponseField>
    ...
    <ResponseField name="contentDisclosure" type="object">This object contains disclosure labels. It can include `advertising.isPaidPromotion` and `aiGenerated.hasAiGeneratedMedia`. X may omit this object.</ResponseField>
...
X reports more replies than the terminal first page exposes. Use tweet search
with the same conversation ID. Disclose that coverage depends on X.
