...
See [Tweet, Profile, Media & Reply API Fields](/guides/tweet-profile-api-fields) for reply coverage and
...
| Collection mode | `mode` | Keep `complete` runs separate from paginated direct-reply runs. |
| Time window | `sinceTime` and `untilTime` | Reuse the same Unix-second bounds after a safe retry. |
| Completion state | `has_next_page` plus the terminal cursor | Mark the export complete only after the final page. |
| Partial coverage | `diagnostic` from `424 replies_incomplete` | Store safe rows and follow `recommendedFallback`. |
