...
marks the first page as incomplete. Search the matching `conversation_id` when
the first page is incomplete. Filter `inReplyToId` for direct replies only.
Never promise a complete set without a terminal cursor.
...
thread joins, engagement counts, media URLs, and cursor fields. Each output
line contains one reply tied to the requested tweet. Workers can restart after
storing the latest response page.
