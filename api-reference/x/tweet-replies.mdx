...
keywords: ["Twitter reply scraper", "tweet reply scraper", "get tweet replies", "tweet replies API", "Twitter replies API", "X replies API", "tweet comments", "reply export", "JSON export", "CSV export", "moderation queue"]
---
...
<Warning>
  Reply visibility can be incomplete. For an unfiltered first page, Xquik
  compares a terminal page with the post's reported reply count. It returns
  `424 replies_incomplete` when the page is visibly short. Do not treat that
  error as proof that a user did not reply. Use tweet search with
  `conversation_id:{tweetId}` as the broader fallback.
</Warning>

See [Read Data Richness](/guides/tweet-profile-api-fields) for reply coverage and
...
If the endpoint returns `replies_incomplete`, call
`GET /x/tweets/search?q=conversation_id:{tweetId}%20filter:replies` and paginate
that result. Treat both surfaces as visible-reply coverage, not proof that every
reported reply identity is available.
...
Direct replies cost 1 credit per tweet returned. Low credit balances can return fewer replies than a full page; zero affordable results return `402 insufficient_credits`. Retry `429` with the `Retry-After` header. Retry `424 x_api_unavailable` or `502` after a short backoff. Handle `424 replies_incomplete` with the conversation search fallback.
...
<Tabs>
  ...
  <Tab title="424 Replies Incomplete">
    ...
    ```
    The direct unfiltered page ended before the post's reported reply count.
    Use tweet search with `conversation_id:{tweetId}` and paginate its results.
  </Tab>
</Tabs>
