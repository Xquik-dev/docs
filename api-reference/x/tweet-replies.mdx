...
description: "Retrieve replies for one X tweet with cursor pagination, Unix timestamp windows, author fields, engagement metrics, media, and export handoffs. See costs."
...
<Warning>
  ...
  tweet search with a `conversation_id:{id}` query as the broader fallback.
...
When the route returns `424 replies_incomplete`, stop exhaustive verification.
Search tweets with `conversation_id:<tweet_id>` as the broader fallback. Filter
exact `inReplyToId` values when the workflow accepts only direct replies.
...
with a `conversation_id:{id}` query. Disclose that coverage depends on X.
