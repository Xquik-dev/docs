...
giveaway, or agent workflow needs the newest reply page as JSON. It returns one
row per reply for a single tweet. Store `parent_tweet_id`, `reply_id`, `text`,
`author_id`, `author_username`, `created_at`, `in_reply_to_id`,
`conversation_id`, engagement counts, media URLs, and `next_cursor`. For larger
CSV, JSON, or XLSX exports with an estimate and saved job, use
[`reply_extractor`](/guides/tweet-replies-export).
