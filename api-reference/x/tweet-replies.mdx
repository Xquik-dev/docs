...
giveaway, or agent workflow needs reply rows as JSON. The examples above write JSON Lines rows with `parent_tweet_id`, `reply_id`, `text`, author fields, thread joins, engagement counts, media URLs, and cursor fields. Each line represents one reply for a single tweet, so a worker can resume from the last saved `next_cursor`. For larger CSV, JSON, or XLSX exports with an estimate and saved job, use [`reply_extractor`](/guides/tweet-replies-export).
