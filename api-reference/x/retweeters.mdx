---
title: "Twitter Retweet Scraper API & Repost Users"
sidebarTitle: "Get retweeters"
description: "Retrieve user profiles for accounts that retweeted or reposted one tweet with cursor checkpoints and engagement handoffs. Includes exact API examples."
api: "GET /x/tweets/{id}/retweeters"
keywords: ["get retweeters", "tweet retweeters API", "retweet users API", "users who retweeted a tweet", "Twitter retweeters API", "X retweeters API", "repost audience", "giveaway verification"]
---
...
## Review who reposted one tweet

Use this route to identify profiles that redistributed a selected tweet. Keep
the source tweet ID with every returned profile.

Save user ID, username, profile name, biography, verification state, follower
counts, cursor, and collection time. Use numeric user IDs for snapshot
comparison.

Retweeter rows can support reach analysis, partner review, or moderation. A
repost does not establish endorsement or a business relationship. Preserve
the observable action without inventing intent.

Compare snapshots by user ID. Deduplicate repeated pages by tweet ID and user
ID. Record the collection time because repost relationships can change.

Use favoriters for profiles that liked the tweet. Use quote tweets when added
commentary matters. Use replies for the direct conversation under the tweet.

## Measure repost distribution

Group retweeters only by observable profile fields. Keep every grouping rule
with the report.

Compare complete snapshots to find newly observed reposting profiles. Treat a
missing profile as no longer observed. Do not infer why the repost changed.

For campaign handoff, preserve the original tweet ID and URL. Reviewers need
the source post beside every distribution summary.

Store reposting profiles as individual rows before calculating totals. This
keeps every summary traceable.

Use user lookup for current profile enrichment. Keep the original collection
time beside refreshed fields so reviewers can distinguish both observations.
