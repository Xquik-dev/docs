---
title: "Twitter Likes Scraper API & Tweet Favoriters"
sidebarTitle: "Get tweet favoriters"
description: "Retrieve visible users who liked a tweet with usernames, profile fields, follower counts, verification state, and cursor checkpoints. See request fields."
api: "GET /x/tweets/{id}/favoriters"
keywords: ["get tweet favoriters", "tweet likers API", "tweet likes API", "users who liked a tweet", "Twitter tweet likers API", "X tweet likers API", "CRM enrichment", "giveaway verification"]
---
...
## Choose Liker Identities

Use this route when liker identities are the required output. It returns user profiles and can report unavailable identities. Use retweeters when reposting accounts are the target population.
...
<Info>
  Get tweet favoriters returns user profiles when X exposes who liked a post. It
  is also useful as a tweet likers API, tweet likes API, X tweet likers API,
...
visible account that liked a post for giveaway checks, CRM enrichment, audience
...
## Review who liked one tweet

Use this route when the tweet is the fixed starting point. Return profiles that
liked that tweet. Keep the source tweet ID with every profile row.

Store user ID, username, profile name, biography, verification state, follower
counts, and collection time. Preserve the cursor for each completed page.

Liker profiles can support campaign review or audience research. A like does
not prove agreement, purchase intent, or affiliation. Keep conclusions
separate from the observed public action.

Compare repeated snapshots by user ID. Avoid using username changes as new
liker events. Deduplicate resumed pages by tweet ID and user ID.

Use retweeters when the question concerns repost distribution. Use tweet
replies for conversation participants. Each engagement route represents a
different public action.

## Segment liker profiles carefully

Create segments from explicit profile fields. Examples include verification
state, location, or follower-count ranges.

Keep the segmentation rule beside the output. Do not label a profile as a
customer, lead, or advocate from a like alone.

For repeated analysis, apply the same rule to each complete snapshot. Changes
in public profile fields can move a user between segments.

Keep segment totals beside the exact source tweet and collection time. Avoid
comparing totals produced by different rules.

Open selected profiles through user lookup when current biography or counts
matter. The liker page represents the collection moment.
...
### 424 Liker Identities Unavailable
...
```
The post reports likes, but X does not expose the corresponding user
identities. Use retweeters or replies when the workflow needs a visible
participant list.
