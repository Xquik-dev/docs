...
<CodeGroup>
  ...
  while (data.has_next_page) {
    const next = await fetch(
      `https://xquik.com/api/v1/x/tweets/search?${new URLSearchParams({ q: "giveaway", fromUser: "xquikcom", mediaType: "media", cursor })}`,
      { headers: { "x-api-key": "xq_YOUR_KEY_HERE" } },
    );
  ...
  response = requests.get(
      ...
      params={"q": "giveaway", "fromUser": "xquikcom", "mediaType": "media"},
      headers={"x-api-key": "xq_YOUR_KEY_HERE"},
  )
  data = response.json()
  print(data["tweets"])
...
It returns paginated JSON for live search pages and app ingestion. For CSV or
XLSX output, project the returned `tweets[]` rows locally or use
