---
title: "Twitter search API & tweet scraper"
description: "Search tweets by keyword, ID, or URL. Return text, authors, replies, metrics, media, and cursors for CRM, agents, or exports."
api: "GET /x/tweets/search"
keywords: ["Twitter search", "advanced Twitter search", "Twitter search API", "search tweets", "scrape tweets", "tweet scraper", "Twitter scraper API", "Twitter search by date", "Twitter keyword search", "X API search", "tweet search export"]
---
...
and `limit` plus `cursor`. De-duplicate stable IDs and reject repeated cursors.
Return a partial-result checkpoint if pagination stalls.
...
[`tweet_search_extractor`](/guides/tweet-scraper-csv-export) for saved CSV, JSON, or
XLSX files.
...
| --- | --- |
| Twitter keyword search | `q=product launch` |
...
<Note>
  **Next steps:** [Tweet Search Export Workflow](/guides/tweet-scraper-csv-export) when you need saved CSV, JSON, or XLSX files, [Get Tweet](/api-reference/x/get-tweet) to fetch full details for a specific tweet, or [Get User](/api-reference/x/get-user) to look up an author profile.
</Note>
