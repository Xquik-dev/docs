...
<ParamField query="q" type="string" required>
  Send the caller's query without adding terms or filters. Supports X search
  operators, Tweet IDs, and X status URLs.
</ParamField>

<ParamField query="queryType" type="string">
  `Top` ranks engagement. A like minimum alone uses `Latest`, the default.
  Use `result_type` as a Twitter-compatible alias.
</ParamField>
...
<ParamField query="limit" type="integer">
  Maximum Tweets per automatic page. Use `count` as an alias. Use `1` through
  `10000`. Keep the same value when continuing with `cursor`.
</ParamField>
