...
Use `GET /x/tweets/search` for live, paginated JSON. Store stable IDs, requested
fields, and `next_cursor` so workers can resume. Use
[`tweet_search_extractor`](/guides/tweet-scraper-csv-export) for estimates,
...
<ParamField query="q" type="string" required>
  Send the caller's query, Tweet ID, or status URL. `query` is accepted as a
  migration alias. Do not add terms or filters.
</ParamField>
