---
title: "Search tweets"
description: "Search tweets by keyword, hashtag, or X query operator and return paginated JSON tweet data for CRM, agents, or export handoff"
...
<CodeGroup>
  ...
  func mediaURLs(items []Media) []string {
    urls := make([]string, 0, len(items))
    for _, item := range items {
      if item.MediaURL != "" {
        urls = append(urls, item.MediaURL)
      }
    }
    return urls
  }

  func fetchPage(cursor string) SearchPage {
    baseURL := "https://xquik.com/api/v1/x/tweets/search"
    params := url.Values{}
    params.Set("q", "giveaway")
    params.Set("fromUser", "xquikcom")
    params.Set("mediaType", "media")
    if cursor != "" {
  ...
  func main() {
    ...
    for pageIndex := 0; pageIndex < 3; pageIndex++ {
      ...
      for _, tweet := range page.Tweets {
        ...
        if err := encoder.Encode(searchRow); err != nil {
          log.Fatal(err)
        }
      }
...
above write JSON Lines rows with tweet, author, media, and cursor fields so a
worker can resume from the last saved `next_cursor`. For CSV or XLSX output,
project the returned `tweets[]` rows locally or use
...
<CardGroup cols={2}>
  ...
  <Card title="Author joins" icon="user-round">
    Store `tweets[].author.id` and `tweets[].author.username` as the stable author join key and readable handle.
  </Card>
...
`limit` is an upper bound from 1 to 200 for a bounded one-request pull. Omit it when you want cursor pagination, then pass the previous response's `next_cursor` as `cursor`. Do not combine `limit` with `cursor` for page-by-page loops.
