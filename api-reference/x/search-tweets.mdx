---
title: "Search tweets"
description: "Search tweets by keyword, hashtag, X query operator, Tweet ID, or X status URL and return paginated JSON tweet data for CRM, agents, or export handoff"
...
<Info>
  ...
  (`GET /x/users/{id}/tweets`). A strict account date window such as
  `q=from:username&sinceTime=2026-05-01&untilTime=2026-05-02` is optimized for
...
`untilTime` favors account timeline completeness. Raw tokens such as
`from:username since:2026-05-01 until:2026-05-02` use the same behavior. Use
`queryType=Latest` for chronological backfills. Add keywords for ranked search.
