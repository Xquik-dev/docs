...
Request a free credit quote before starting any extraction. The response estimates results and required credits. It also checks your available credit balance. A quote never starts a scraping job. Keep `toolType`, target fields, filters, and `resultsLimit` unchanged when you start the extraction.
...
<ResponseField name="source" type="string">Data source used for the estimate. One of: `collection`, `followers`, `following`, `paginationCap`, `quoteCount`, `replyCount`, `resultsLimit`, `retweetCount`, `unknown`.</ResponseField>
...
<ResponseField name="resolvedXUserId" type="string">Resolved X user ID from count-based profile estimates.</ResponseField>
