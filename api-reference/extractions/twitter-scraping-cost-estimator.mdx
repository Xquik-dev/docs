...
<ResponseExample>
  ...
  ```json 400 · extractions/twitter-scraping-cost-estimator
  {
    "error": "invalid_input",
  ...
  ```json 401 · extractions/twitter-scraping-cost-estimator
  {
  ...
  ```json 402 · extractions/twitter-scraping-cost-estimator
  {
  ...
  ```json 404 · extractions/twitter-scraping-cost-estimator
  {
    "error": "not_found",
  ...
  ```json 429 · extractions/twitter-scraping-cost-estimator
  {
...
Set `resultsLimit` when you need a smaller export. The estimator caps `estimatedResults` at that limit. It then adjusts `creditsRequired` for the capped result count.
...
<Tip>
  Start with a conservative `resultsLimit`. Increase it after the first quote passes your budget.
</Tip>
...
<ParamField body="targetCommunityId" type="string">
  Community ID to estimate. Required for `community_extractor`, `community_moderator_explorer`, `community_post_extractor`, and `community_search`.
</ParamField>
