...
description: "Retrieve one giveaway draw's source tweet, eligibility rules, exclusions, selected winners, status, timestamps, and audit details. See request fields."
api: "GET /draws/{id}"
...
Use this route to retrieve one completed or pending draw by ID. Use list for draw history and export for downloadable results. Keep the draw ID with winner verification records.
...
<Note>
  **Related:** [Export Draw](/api-reference/draws/export) to download results as CSV/XLSX/Markdown &middot; [List Draws](/api-reference/draws/twitter-giveaway-history) to see all draws &middot; [Create Draw](/api-reference/draws/create) to run a new draw.
</Note>
