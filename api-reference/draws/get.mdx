...
api: "GET /draws/{id}"
keywords: ["Twitter giveaway winner API", "Twitter giveaway verification", "giveaway draw ID", "past giveaway winners", "X giveaway results", "backup winner", "tweet giveaway results"]
---
...
Winner order is part of the result. Preserve each 1-indexed `position` instead
of sorting by username or tweet ID.
...
<CardGroup cols={2}>
  ...
  <Card title="Backup Winner" icon="shield-plus">
    `isBackup` is `true`. Keep the row separate until replacement approval is
    recorded.
  </Card>
  ...
  <Card title="Winner Account" icon="at-sign">
    `authorUsername` identifies the selected X account at draw time. Keep the
    tweet ID as the stable join value.
  </Card>
</CardGroup>
...
Check the exact draw ID and authenticated account. Another account's draw is
not accessible with the current credential.
