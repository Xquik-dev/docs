...
api: "GET /draws"
keywords: ["Twitter giveaway history", "past Twitter giveaways", "Twitter giveaway results", "giveaway draw history", "X giveaway picker", "past giveaway winners", "draw audit trail"]
---
...
Use `GET /draws` for a Twitter giveaway history page or audit queue. Results
are ordered by `createdAt` and draw ID, newest first. Each row identifies its
...
Use list results for discovery. Use detail results for winner verification.
...
<CardGroup cols={2}>
  ...
  <Card title="Verify Winners" icon="badge-check">
    Call `GET /draws/{id}`. Preserve position, username, tweet ID, and backup
    state.
  </Card>
...
Preserve the source tweet URL instead of extracting a mutable username.
...
Fetch detail rows before publishing past giveaway winners. Preserve winner
position and `isBackup`. This prevents backup winners from appearing as primary
winners in dashboards or announcements.
...
<CodeGroup>
  ...
  ```bash Next page
  curl --fail-with-body "https://xquik.com/api/v1/draws?limit=10&after=MjAyNi0wMi0yM1QxNjozMDowMC4wMDBafDY2NjY1" \
    -H "x-api-key: xq_YOUR_KEY_HERE" | jq
  ```
</CodeGroup>
...
Use a stable loop for a complete history export:
...
do {
  ...
  after = page.hasMore ? next : undefined;
} while (after);

process.stdout.write(`${JSON.stringify(draws)}\n`);
```

Save the last accepted cursor after each durable batch. Restart from that
...
Store the list summary and draw ID. Fetch draw details and preserve winner
order. Export entries when the review needs candidate-level evidence.
