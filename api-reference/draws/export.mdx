...
<CodeGroup>
  ...
  process.stdout.write(
    `${response.headers.get("content-disposition") || "download saved"}\n`,
  );
  ```
...
Every format represents the same selected rows. Serialization and filename
extensions change. Column selection depends only on `type`.

<CardGroup cols={2}>
  <Card title="CSV for Google Sheets" icon="table">
    Use `format=csv` for spreadsheets and imports. Formula-like reply text is
...
Winner rows are ordered by their 1-indexed draw position. Do not sort them by
...
replies from the inspected candidate set.
...
Pair both exports with the draw snapshot. This keeps winner selection separate
from inspected candidate evidence.
...
Yes. Set `type=entries`. The file contains stored replies from the inspected
candidate set. It does not fetch new replies.
...
Sheets, imports, or lightweight processing.
