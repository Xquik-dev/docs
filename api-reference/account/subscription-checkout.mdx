...
api: "POST /subscribe"
keywords: ["X API subscription checkout", "Twitter API subscription", "API billing portal", "Xquik plan checkout"]
---
...
The pricing page remains authoritative for tiers, credits, and current terms.
...
Read `subscription.status` and `subscription.currentPeriodEnd` after checkout.
Read `creditInfo.balance` before searches, exports, writes, or monitor work.
...
The service can reuse a matching open checkout. Treat the latest returned URL
as authoritative. A different tier request can replace an older open checkout.

Do not repeatedly call this endpoint from a timer or automatic retry loop.
