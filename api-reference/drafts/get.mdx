...
api: "GET /drafts/{id}"
keywords: ["get tweet draft", "retrieve tweet draft", "tweet draft API", "saved post content", "draft ID"]
---
...
The API returns one compact draft object. Optional fields are omitted when they
were not supplied during creation.
...
| `topic` | Optional composition topic | Preserve the intended subject. |
| `goal` | Optional composition goal | Check engagement, followers, authority, or conversation intent. |
| `createdAt` | ISO 8601 creation timestamp | Record when the draft entered the workflow. |
...
The route returns the same `404` for missing drafts and other-account IDs. This
