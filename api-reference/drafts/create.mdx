---
title: "Tweet Draft API for Saved Post Creation & Review"
sidebarTitle: "Create draft"
description: "Save a tweet draft with text, an optional topic, and an optimization goal for later review, editing, or publishing without posting to X. See examples."
api: "POST /drafts"
...
Use this route to store tweet text without posting it to X. Add a topic and
goal when later review needs the draft's purpose. Keep the returned draft ID
for retrieval, editing, deletion, or a separate publishing workflow.

A saved draft differs from a published tweet. Draft creation does not return a
public tweet ID. It does not send text to followers or create engagement
metrics. Use the tweet write route only after the final text is approved.

This route stores one text draft. It does not store thread order, media, or a
reply target. Keep those publishing details in your workflow record.

Use one draft ID as the workflow checkpoint. Fetch that exact draft before
editing or approval. List drafts only when a person must browse several saved
posts.
...
| --- | --- | --- |
| Draft ID | Response `id` | Use this ID for retrieval or deletion. |
| Tweet text | Request and response `text` | Review the exact saved copy before publishing. |
| Topic | Response `topic` | Keep the composition context with the draft. |
| Goal | Response `goal` | Preserve engagement, followers, authority, or conversation intent. |
| Creation time | Response `createdAt` | Record when drafting began. |
| Last update | Response `updatedAt` | Reject stale review copies before publishing. |
