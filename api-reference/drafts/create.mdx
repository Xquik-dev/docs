...
description: "Save a tweet or thread draft with text, post order, media attachments, and an optional reply target for later editing or posting."
api: "POST /drafts"
...
Store thread order when the draft contains multiple posts. Preserve media
attachments and any reply target with the draft. Review those fields before
publishing because post order and reply context change the final conversation.
