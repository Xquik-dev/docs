...
description: "Authenticate tweets, follower exports, monitors, webhooks, and X writes with Xquik API keys, guest keys, OAuth 2.1, sessions, or MPP. See request fields."
keywords: ["X API key", "Twitter API key", "x-api-key", "REST API authentication", "REST API key", "X API token", "Xquik API key", "API key authentication", "OAuth 2.1", "guest API key"]
---
...
This access control matrix maps each API endpoint to its accepted method. Most
...
Create a guest key with `POST /api/v1/guest-wallets` after the user confirms a
$10-$250 USD amount. The response includes a one-use Stripe-hosted Payment
Link, the guest key, and a status URL. Creating the Payment Link does not
charge the user.
