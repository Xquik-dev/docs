...
description: "Authenticate tweet, follower, monitor, webhook, and write requests with account keys, guest paid-read keys, OAuth 2.1, sessions, or MPP."
...
Seven fixed-price read operations also accept direct [MPP](/mpp/machine-payments-protocol) payments instead of API key authentication. When you call one without an API key or session cookie, the server returns a 402 payment challenge.
