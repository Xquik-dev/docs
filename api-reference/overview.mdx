...
description: "Xquik REST API v1: authentication, rate limits, pagination, error handling, and 120 documented operations for X data"
...
<CardGroup cols={2}>
  ...
  <Card title="Rate limits" icon="timer">
    Respect `Retry-After`. Read calls are 10 per 1s, write calls are 30 per 60s, and delete calls are 15 per 60s. Back off on repeated `429 rate_limit_exceeded` responses.
  </Card>
...
<CardGroup cols={3}>
  <Card title="Read requests" icon="database">
    `GET`, `HEAD`, and `OPTIONS` requests share a 10 per 1s user bucket.
