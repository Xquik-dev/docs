---
title: "Twitter Scraper API, SDKs & Webhook Automation"
sidebarTitle: Twitter scraper API overview
description: "Use Xquik REST API v1 to search tweets, retrieve profiles and followers, export replies and timelines, monitor accounts, post tweets, and send webhooks."
keywords: ["Twitter API documentation", "Twitter REST API", "Twitter API tool", "API explorer", "X API documentation", "REST API", "base URL", "rate limits", "authentication", "error codes", "pagination"]
---
...
Xquik REST API v1 searches tweets, exports followers and replies, retrieves
profiles and timelines, posts tweets, monitors accounts, and sends webhooks.
...
Seven fixed-price GET operations also accept anonymous [MPP](/mpp/machine-payments-protocol) payments. The server returns a `402` with its `WWW-Authenticate: Payment` challenge and an optional guest wallet action. Your client can complete the MPP challenge or ask the user to confirm a $10-$250 USD guest wallet. A failed request creates no checkout.
