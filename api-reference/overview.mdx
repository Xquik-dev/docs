...
description: "Xquik REST API v1: authentication, rate limits, pagination, error handling, and 126 documented operations for X data"
...
Generate account keys from your [dashboard](https://xquik.com). Xquik API keys can also use `Authorization: Bearer xq_your_api_key_here`. OAuth 2.1 Bearer tokens keep the account access granted by their scopes.
...
Replace `xq_your_api_key_here` with your actual API key from the [dashboard](https://xquik.com). If you get `401`, double-check that the header name is `x-api-key` (lowercase) and the key starts with `xq_`.
