...
description: "Remove an X follower from one of your connected accounts"
...
<Tabs>
  ...
  <Tab title="429 Rate Limited">
    ...
    ```
    The request hit an Xquik tier or follow-action limit, or X throttled the write. Possible codes: `rate_limit_exceeded`, `x_rate_limited`, or `x_daily_limit`. Respect the `Retry-After` header when present.
  </Tab>
...
<Note>
  **Related:** [Follow User](/api-reference/x-write/follow) to follow a user, [Unfollow User](/api-reference/x-write/unfollow) to stop following a user, or [Get User](/api-reference/x/get-user) to look up user details.
</Note>
