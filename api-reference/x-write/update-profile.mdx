...
| Idempotency key | Request header | New field values need a new key. |
...
Do not create a new action after an uncertain network response. Check the saved
...
Schedule only reviewed replacement and rollback values. Use one key for the
