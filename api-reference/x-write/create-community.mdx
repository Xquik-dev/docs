...
keywords: ["create a community on twitter", "create a twitter community", "how to create a community on twitter", "create twitter community", "X community API", "community admins", "community rules", "community members"]
---
...
<ResponseExample>
  ...
  ```json 202 · x-write/create-community
  {
    "object": "x_write_action",
    "id": "12346",
    "writeActionId": "12346",
  ...
  ```json 400 · x-write/create-community
  {
    "error": "invalid_input",
  ...
  ```json 401 · x-write/create-community
  {
  ...
  ```json 402 · x-write/create-community
  {
  ...
  ```json 403 · x-write/create-community
  {
  ...
  ```json 404 · x-write/create-community
  {
  ...
  ```json 409 · x-write/create-community
  {
  ...
  ```json 422 · x-write/create-community
  {
    "error": "x_rejected",
  ...
  ```json 429 · x-write/create-community
  {
  ...
  ```json 500 · x-write/create-community
  {
  ...
  ```json 503 · x-write/create-community
  {
...
account. This create Twitter community workflow submits a name and optional
...
Choose the connected account before review. X treats the original creator as
the first admin. Community admins control names, descriptions, rules, and
moderator assignments through X. Xquik sends the creation request only.

X currently requires an eligible admin account. Its official moderator
playbook lists these checks:

- Keep the X account public.
...
inside X before approval. Preserve any rejection reason for the operator.
...
Define community rules before creating the container. Rules should describe
...
X supports open communities and restricted membership workflows. Open
...
promote or monetize the community. Use separately documented routes for later
posts. Keep unsupported launch actions in your own workflow.
...
After success, open the community page and verify its public identity. Search
...
No. The canonical request accepts `account`, `name`, and `description`. Set
