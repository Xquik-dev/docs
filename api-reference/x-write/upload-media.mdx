...
Call `POST /x/media` for one local file or hosted HTTPS media URL. Each call
uploads one image, GIF, or MP4 for one connected X account. The response returns
a media ID and reusable `mediaUrl` after completion.
...
Accept these media types: AVIF, GIF, JPEG, PNG, WebP, and MP4. Match the content
type to the file. Every URL must resolve to a public address. Finish within 30
seconds and stay under 15,728,640 bytes.
