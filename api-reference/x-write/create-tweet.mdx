...
Review X's [Create Post documentation](https://docs.x.com/x-api/posts/create-post).
Xquik uses different authentication and response fields.
...
Create standard tweets, replies, community tweets, image tweets, video tweets,
and note tweets. Include only fields required for the selected tweet type.
...
| Community tweet | `account`, `text`, and `community_id` | Check the account's membership in the target X Community. |
...
Create one `Idempotency-Key` for each automatic Twitter posting request.
...
post. Let your scheduler call it at the approved time. Each request creates 1
tweet. Create replies with `reply_to_tweet_id`. Create each thread post with a
