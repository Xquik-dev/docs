...
This Twitter API for posting tweets accepts text and public media URLs.
...
X also documents an official [Create Post](https://docs.x.com/x-api/posts/create-post) endpoint.
Its authentication and response fields differ from Xquik's fields.
...
tweet. Reuse that key only when replaying the same network request. A new
tweet, reply, caption, media URL, account, or community requires a new key.

This endpoint starts the write immediately. It does not schedule a future
post. Let your scheduler call it at the approved time. One request creates one
tweet or reply. Create each thread post with a separate approved request.
...
Use `media` for an image or MP4 at a public HTTPS URL. For local files, call
[Upload Media](/api-reference/x-write/upload-media) first. Pass its returned
`mediaUrl` here. Send up to 4 image URLs or exactly 1 MP4 URL. Keep the MP4 at
100 MB or less. Never send `media_ids`; that field is for DMs only. Attached
...
<ParamField header="x-api-key" type="string" required>
  Your API key. You can also use a session cookie.
</ParamField>
