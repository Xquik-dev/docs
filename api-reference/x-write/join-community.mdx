...
Call `POST /x/communities/{id}/join` with one connected account and numeric ID.
X decides whether that account can join. The route cannot discover, create, or
moderate Communities. Approve each account and Community pair first.
...
Search the X Communities page by name, creator, or handle.
X warns that some Communities never appear in search results.

Use a direct URL for an account's first Community. Confirm its name, rules,
moderators, membership mode, and recent posts. Match the numeric ID with
[Community Info](/api-reference/x/community-info).

[Community Search](/api-reference/x/community-search) searches posts inside one
known Community. It cannot find Community directories or topics.

## Compare Manual and API Community Joining

X handles invitations, rules, and moderator review. Use X for phone membership.
Use Xquik for approved server workflows. The API cannot click controls, choose
a membership mode, or bypass moderator approval.
...
They review restricted requests. Moderator blocks can stop membership.
The API cannot override them.
...
Check its rules and membership mode.
...
Store Tweet IDs, replies, reposts, likes, media, and cursors.

Membership never authorizes publishing.
Approve separate Create Tweet actions for members posting after joining.
Keep the reviewed rules with each approval.
...
An HTTP `202` confirms processing, not membership. Keep the existing action
during a timeout. A changed account or Community needs new approval.

## Join Communities in Controlled Batches

Assign one `Idempotency-Key` per approved pair. Process retries serially. Keep
one ledger row per result. Pause only the affected account after errors.
...
invitations. Preserve any `422` rejection.
...
Check the public account, numeric ID, connection, mode, invitation, and moderator
blocks. Read `terminal`, `retryable`, and `safeToRetry` before retrying.
...
Store the action ID and final membership result. Never infer failure from a
dropped connection. Never infer posting permission from membership.
