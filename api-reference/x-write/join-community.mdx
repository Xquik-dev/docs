...
Use a direct URL for an account's first Community.
Confirm the specific Community name.
Review its rules and moderators. Check its membership mode and recent posts.
Match the numeric ID with
...
Assign one `Idempotency-Key` per approved pair. Process retries serially. Store
each result in one ledger row. Pause only the affected account after errors.
