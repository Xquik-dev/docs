...
```json
{"record_type":"dm_media_send","message_id":"1893726451029384192","user_id":"44196397","account":"myxaccount","media_ids":["1893726451023847424"],"upload_media_id":"1893726451023847424","send_status":"sent","handoff_format":"jsonl"}
```

Keep `media_ids` as a one-item array in the request, keep `upload_media_id` as the original `POST /x/media` result, and use `message_id` as the external DM identifier after the send succeeds.
