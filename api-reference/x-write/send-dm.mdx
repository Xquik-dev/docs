...
description: "Send text or one media attachment through the Twitter DM API. Authenticate the connected X account, poll the durable action, and handle every response."
api: "POST /x/dm/{userId}"
keywords: ["Twitter DM API", "Twitter API DM", "Twitter API send DM", "send direct messages", "X direct message API", "send DM with media", "DM history", "messageId", "media_ids"]
---
...
The Twitter API DM route sends text or one uploaded media attachment.
...
Reuse an idempotency key only after an interrupted network request.
...
Yes. Queue approved replies and preserve the exact sent text.
