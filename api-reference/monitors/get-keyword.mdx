...
The Node.js, Python, and Go examples convert the fetched keyword monitor into
one state snapshot row. Store `keyword_monitor_id`, `query`, `event_types`,
`is_active`, `next_billing_at`, `update_endpoint`, and `events_endpoint` before
changing filters, pausing alerts, or reconciling webhooks.
...
<CardGroup cols={2}>
  <Card title="Current Filter" icon="funnel">
    ...
    `eventTypes` into webhook subscriptions before relying on signed alerts.
  </Card>
