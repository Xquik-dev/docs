...
keywords: ["Twitter monitoring alerts", "how to set Twitter alerts", "Twitter account alert settings", "Twitter monitor event filters", "pause Twitter monitoring", "resume Twitter monitor"]
---
...
Learn how to set Twitter alerts for one saved X profile. This Twitter
monitoring tool replaces tweet and profile filters. It can also pause or resume
the same monitor. The response returns the resulting monitor settings.
...
Send the complete `eventTypes` array when coverage changes. The array replaces
the saved filter. Omit it when only `isActive` changes. Use `false` to pause
checks and `true` to resume them.

An unavailable profile blocks resumption with `409 monitor_profile_unavailable`.
Wait for profile recovery, then retry. Finally, compare the returned account,
filter, and active state with the approved request.
...
Twitter account alert settings choose captured tweet and profile changes. API
access requires a valid API key or bearer token. To receive notifications,
...
Validate each signature and acknowledge valid requests quickly. Process slower
...
Select only required tweet and profile events. Use `tweet.mention` for mentions
and `tweet.reply` for replies. Add repost, quote, media, or link events only
when the receiver handles them.
...
These actions preserve the meaning of stored account events.

## Verify the First Event After Resuming

Read the monitor and confirm `isActive: true`. Then inspect the first matching
event's type and timestamp. Check deliveries separately. A successful PATCH is
not delivery proof.
