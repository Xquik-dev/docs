...
Select only required tweet and profile events. Capture mentions with the
`tweet.mention` event type. Capture replies with the `tweet.reply` event type.
Add repost, quote, media, or link events only when the receiver handles them.
