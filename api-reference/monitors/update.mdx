...
Select only required tweet and profile events. Use `tweet.mention` for
mentions. Use `tweet.reply` for replies. Add repost, quote, media, or link
events only when the receiver handles them.
...
| Billing checkpoint | `nextBillingAt` in the response | Schedule the next credit review. |
...
These actions keep stored account events consistent.
