...
[follower export guide](/guides/follower-export-crm) for controlled CSV exports
and follower archives.

Read `isActive` as the polling state. An active monitor can capture future
events that match its filters. A paused monitor keeps its profile and filters
without polling. Read `nextBillingAt` as the next active billing time. It does not
...
the full event through [Get Event](/api-reference/events/get). The response
...
Use stored monitor events for Twitter account activity captured after monitoring
started. Use [List Events](/api-reference/events/list) for a cursor-based list
of stored events. Use [X user tweets](/api-reference/x/user-tweets) for recent
profile tweets. Use follower snapshots or an extraction job for follower exports.
...
webhook delivery attempt. A follower export records a follower list for one
collection timestamp. None of those outputs is an engagement analytics dashboard.

## How Do Twitter Analytics Tools Differ From Monitor Status?

Twitter analytics tools usually calculate performance from posts and profile
statistics. They may summarize likes, replies, reposts, views, or follower
changes. Those values are engagement metrics. This status endpoint performs a
different job.

The response describes one saved monitor. It identifies the tracked account
and selected event filters. It also reports whether future checks remain
active. The response never calculates reach, impressions, engagement rates, or
audience growth.

Use the status response before investigating account activity. First, confirm
the stable `xUserId`. Then inspect `eventTypes` and `isActive`. These fields
...
attempts. This workflow separates monitor configuration from analytics
reporting.

Do not compare an empty event list with a complete analytics report. Event
history starts when the monitor captures a selected change. A paused monitor
creates no new records. An omitted filter also creates no record for that
change. Document both conditions before interpreting account activity.

## What Should a Twitter Account Activity Audit Store?

Start an audit with the monitor response. Store `id`, `username`, and
`xUserId`. Add `eventTypes`, `isActive`, `createdAt`, and `nextBillingAt`. These
fields describe the approved monitor state at one timestamp.

Store the request time beside the response. The API does not return a snapshot
timestamp. Your request time identifies when the audit observed the current
state. Keep the HTTP status and monitor ID with that entry.

Record the intended event beside the stored filters. For a reply alert, record
`tweet.reply`. For a profile-name change, record `profile.name`. This comparison
shows whether the required filter existed during the review.
...
ID. Store each `deliveryId` and delivery status. This chain distinguishes a
missing event from a failed receiver.

Repeat the status request after each approved update. Compare the account,
filters, active state, and next billing time. Preserve both snapshots in the
change log. Reject the update when `xUserId` unexpectedly changes.

Treat `404` as an ownership or existence failure. Do not create replacement
history from that response. Confirm the authenticated account and monitor ID
first. Treat `429` as a temporary request limit. Retry after the documented
delay without changing the audit conclusion.
...
time. This snapshot becomes the change baseline.
...
log. This proves the exact account-monitor change.
