...
description: "Retrieve one account monitor's X profile, tweet, follower, following, profile, and relationship event types, state, interval, and billing. See event fields."
api: "GET /monitors/{id}"
...
<ResponseExample>
  ...
  ```json 400 · monitors/twitter-account-monitor-status
  {
    "error": "invalid_input",
  ...
  ```json 401 · monitors/twitter-account-monitor-status
  {
  ...
  ```json 404 · monitors/twitter-account-monitor-status
  {
    "error": "not_found",
  ...
  ```json 429 · monitors/twitter-account-monitor-status
  {
