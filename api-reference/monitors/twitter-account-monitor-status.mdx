...
| Creation time | `createdAt` | Record when the monitor was created. |
...
A stable X user ID preserves identity after username changes.
...
[follower export guide](/guides/follower-export-crm) for controlled follower
CSV archives.

Read `isActive` as the polling state. Active monitors capture selected future
account changes. Pausing retains the profile and filters but stops future
checks. Read `nextBillingAt` as the next scheduled charge. It does not prove
that an event or webhook delivery succeeded.
...
alerts, require the `tweet.reply` event type. Either filter can operate alone or
alongside the other. The
...
Use stored monitor events for activity captured after monitor creation.
[List Events](/api-reference/events/list) returns stored event rows with cursor
pagination. Use [X user tweets](/api-reference/x/user-tweets) to retrieve recent
posts from the profile. Use an extraction job for follower CSV exports.
...
changes. Those values are engagement metrics. This endpoint only reports
...
not capture additional account events. An omitted filter also prevents that
event from being stored. Record both conditions before drawing activity
conclusions.
...
fields describe the approved monitor configuration during the request.

Store the request time beside the response. The API does not return a snapshot
time. Your request time identifies when the audit observed this configuration.
Keep the HTTP status and monitor ID with that entry.

Record the intended event beside the stored filters. For a reply alert, record
the `tweet.reply` type. For a profile-name change, record `profile.name`. This
...
filters, polling status, and next scheduled charge. Preserve both snapshots
with the approved request. Reject the update when `xUserId` unexpectedly changes.

Treat `404` as an ownership or existence failure. Never infer previous monitor
...
request. This proves the exact account-monitor change.
...
<ResponseField name="isActive" type="boolean">Indicates whether monitoring continues.</ResponseField>
<ResponseField name="createdAt" type="string">Creation time in ISO 8601 format.</ResponseField>
