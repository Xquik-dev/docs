...
description: "Create a 1-second keyword monitor for an X search query. Store matching tweet events and deliver selected events to signed webhooks."
...
Create a keyword monitor when one search query needs continuous tweet checks.
Store its ID, query, event types, billing time, and webhook destinations.
