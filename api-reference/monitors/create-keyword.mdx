...
<CardGroup cols={2}>
  ...
  <Card title="Signed Webhook Rollout" icon="shield-check">
    Create the webhook with matching `eventTypes` unless the receiver wants a
    subset. Store the one-time `secret`, then run `POST /webhooks/{id}/test`.
  </Card>
  <Card title="Delivery Verification" icon="key-round">
    Verify `X-Xquik-Signature`, `X-Xquik-Timestamp`, and `X-Xquik-Nonce` on the
    raw body. Deduplicate with `deliveryId`; join with `streamEventId` and
    `keywordMonitorId`.
  </Card>
</CardGroup>
