...
**Signed webhook rollout:** After creating a keyword monitor, use the same `eventTypes` when you create the webhook unless the receiver intentionally wants a subset. Store webhook `secret` once, run `POST /webhooks/{id}/test`, then reject unsigned or replayed deliveries before processing monitor events.
