...
<CardGroup cols={2}>
  ...
  <Card title="Event Filter" icon="funnel">
    Store `eventTypes`; create the webhook with matching event types unless the
  ...
  <Card title="Webhook Delivery Join" icon="webhook">
    Store the one-time webhook `secret`, run `POST /webhooks/{id}/test`, and
    verify `X-Xquik-Signature`, `X-Xquik-Timestamp`, and `X-Xquik-Nonce` on the
    raw body. Use `deliveryId` for receiver idempotency and `streamEventId` plus
    `keywordMonitorId` to join the signed webhook back to `GET /events/{id}`.
  </Card>
</CardGroup>
