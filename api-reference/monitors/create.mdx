...
Choose one surface from the required alert scope.

| Monitoring need | Xquik surface | Result |
| --- | --- | --- |
...
Do not replace one surface with another after collection starts.
Store the chosen monitor type beside every downstream alert.
...
Each selected event type creates a focused alert stream.
...
Choose only events that trigger a real downstream action.
...
This workflow records concrete account changes.
...
That separation prevents one competitor event from triggering unrelated jobs.
...
Do not create replacement monitors when a pause is sufficient.

Store each resolved X user ID beside the current username.
This join remains stable when a username-change event arrives.
...
Never substitute aggregate counts for missing participant or relationship records.
...
Webhook failures do not require replacing a healthy monitor.
