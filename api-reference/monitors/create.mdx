...
Use `POST /monitors` when a queue, CRM, warehouse, Slack alert, or agent tracks
one X account. It checks selected tweets and profile changes every second.
...
Creation and reactivation require 22 available credits. That total includes a 1-credit username lookup and the first active hour. Pause the monitor through
...
<ParamField header="x-api-key" type="string" required>
  Your API key. You can also authenticate with a session cookie. Generate a key
...
```
This request requires at least 22 available credits. The API may return
