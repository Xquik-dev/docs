---
title: "Create account monitor API"
...
| `isActive` | Whether Xquik is checking the account and sending events. |
| `nextBillingAt` | Next hourly billing time for the active monitor. |
| `deliveryId`, `streamEventId`, `eventType`, `occurredAt`, `username`, and `data` | Downstream webhook handoff fields. Use `deliveryId` for receiver idempotency and `streamEventId` to join back to `GET /events/{id}`. |
