...
or availability events. Store the monitor ID. Then connect a signed webhook.
...
Use this Twitter monitor when one known profile needs continuous checks.
Send the username without `@`. Choose only the required event types.
The endpoint returns one stable monitor ID for event and webhook joins.

Use `POST /monitors` to monitor Twitter account activity from one username.
...
| Creation time | `createdAt` | Preserve the monitor audit timestamp. |
| Billing checkpoint | `nextBillingAt` | Schedule the next credit review before this time. |
...
| Profile image changes | `profile.avatar.changed`, `profile.banner.changed` | Previous and current image references. |
