...
X account. It does not remove tweets, followers, following, replies, likes,
reposts, media, lists, communities, or profile details.
...
Do not treat a network timeout as deletion proof. Confirm the monitor state
...
Deletion cascades through stored events and linked delivery records. Decide
which records your team must keep before sending `DELETE`.
...
the complete `data` object when downstream analysis needs its details.
...
Account monitors and keyword monitors use different deletion routes. Use
...
Follow every cursor until `hasMore` becomes `false`. An incomplete cursor walk
can omit older tweet, follower, or profile events.
...
Store `occurredAt` with each event payload. It represents the observed event
time. Keep the monitor ID with every exported row. This association supports
...
A `200` response confirms the deletion. Require `success: true` before closing
...
retrying. Do not substitute an X username for the monitor ID.
...
A `404` response means the monitor is unavailable. It may already be deleted.
It may also belong to another account. Verify the account context before taking
another action.

A `429` response means the rate limit blocked the request. Read
`Retry-After`, wait for that interval, and verify the monitor before retrying.
...
Update downstream jobs with the new monitor ID. Replace stored event filters,
