...
api: "GET /monitors"
keywords: ["twitter account monitoring", "monitor multiple twitter accounts", "twitter monitor list"]
---
...
profiles. Filter rows by `isActive` before estimating active-monitor costs.
...
Each monitor row contains its monitor ID, normalized username, and stable X
user ID. The `eventTypes` array defines which tweet and profile changes matter.
...
cover the avatar, banner, name, username, biography, and location. They also
...
events. Use [Get Event](/api-reference/events/get) for one complete event
payload.
...
`isActive` before waiting for a new alert. A paused monitor retains its setup
but does not poll for new events.
...
The active state describes polling, while delivery status describes one
webhook attempt.

## How Should Teams Manage Many Tracked Profiles?

Managing multiple Twitter accounts requires one named owner per monitor. Store
that owner outside the API response. Save the monitor ID, X user ID, selected
event types, and active state. Add the owner and review date.

Assign ownership by workflow. Support teams can own reply and mention events.
...
per monitor-hour. A paused monitor remains listed but does not incur the active
monitor charge. Review `isActive` and `nextBillingAt` before enabling more
profiles.
