...
description: "Inspect a webhook's tweet, follower, profile, relationship, or keyword event delivery attempts, HTTP statuses, errors, and retry times."
...
| Delivery state | `deliveries[].status` | Route pending, failed, exhausted, or delivered rows. |
...
| Delivery timing | `createdAt` and `deliveredAt` | Measure queue and recovery latency. |
...
<Note>
  **Related:** [Webhooks Overview](/webhooks/overview) &middot; [Resume Webhook](/api-reference/webhooks/resume) &middot; [Webhook Testing Guide](/guides/webhook-testing)
</Note>
