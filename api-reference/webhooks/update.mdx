...
description: "Change a webhook's HTTPS URL, tweet, follower, profile, relationship, or keyword event filters, and active delivery state. Includes exact API examples."
...
| Delivery state | Response `deliveryStatus` | Route active, paused, or needs-attention receivers. |
| Failure cap | Response `failureHardCap` | Preserve the receiver recovery threshold. |
