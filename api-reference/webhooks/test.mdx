...
[Update Webhook](/api-reference/webhooks/update), then test again before sending
...
<Tabs>
  ...
  <Tab title="400 Invalid Request">
    ...
    ```json
    { "error": "webhook_inactive", "message": "Cannot test an inactive webhook." }
    ```
...
<CardGroup cols={2}>
  ...
  <Card title="Inactive webhook" icon="toggle-left">
    `webhook_inactive` means no test was sent. Reactivate the webhook with
    [Update Webhook](/api-reference/webhooks/update), then test again.
  </Card>
...
<Note>
  ...
  **Related:** [List Webhooks](/api-reference/webhooks/list) &middot; [List Deliveries](/api-reference/webhooks/deliveries) &middot; [Get Event](/api-reference/events/get) &middot; [Webhook Verification](/webhooks/verification)
</Note>
