...
The `health` field is derived from recent login and cookie state. Use it to decide whether to attempt writes or trigger a reauth.
...
<CardGroup cols={2}>
  ...
  <Card title="needsReauth" icon="refresh-cw">
    Credentials were rejected. Submit fresh credentials via
    [reauth](/api-reference/x-accounts/reauth).
  </Card>

  <Card title="locked" icon="lock">
    X locked the account. Unlock it at `x.com/account/access`, then reauth.
  </Card>

  <Card title="suspended" icon="circle-x">
    X banned the account. Appeal on X; writes will not recover automatically.
  </Card>

  <Card title="recovering" icon="activity">
    Past a transient cooldown. Xquik will auto-retry on next use.
  </Card>
