...
Use this endpoint when a connected account should no longer be available for write actions, DMs, media upload, or profile updates.

## What disconnect does

<CardGroup cols={1}>
  <Card title="Removes this connection" icon="trash-2">
    The connection is removed from your Xquik account. Future `GET /x/accounts/{id}` calls for the same ID return `404`.
  </Card>
