...
Use this endpoint after [Connect X Account](/api-reference/x-accounts/connect) returns `202 Email Code Required`. Submit the code from the account email inbox before `expiresAt`.
...
<Tabs>
  <Tab title="201 Created">
    ...
    <ResponseField name="loginCountry" type="string">Optional ISO 3166-1 alpha-2 country code (for example `"US"`). Present only when the declared proxy region was unavailable and login used a one-time US region. Omitted on normal logins.</ResponseField>
