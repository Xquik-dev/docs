...
<Warning>
  This endpoint cannot reopen an expired, failed, completed, or replaced challenge. After `409`, `410`, or `422`, start [Connect X Account](/api-reference/x-accounts/connect) again with the current password and any required TOTP secret key.
</Warning>
...
<CardGroup cols={1}>
  ...
  <Card title="Start over when stale" icon="timer-reset">
    `410` means the code expired. `409` means the challenge was already completed, failed, expired, or replaced. Start a new connection to get a fresh challenge instead of resubmitting the old code.
  </Card>
</CardGroup>
