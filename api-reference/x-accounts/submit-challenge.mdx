---
title: "Submit X account email code"
description: "Submit the email verification code returned during X account connection"
...
Use this endpoint after [Connect X Account](/api-reference/x-accounts/connect) returns `202 Email Code Required`. Submit the one-time code from the account email inbox before `expiresAt`.
...
<CardGroup cols={1}>
  ...
  <Card title="Start over when stale" icon="timer-reset">
    `410` means the code expired. `409` means the challenge was already completed, failed, expired, or replaced. Start a new connection to get a fresh challenge.
  </Card>
</CardGroup>
