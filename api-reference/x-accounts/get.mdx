...
Use this endpoint before a workflow acts with a specific connected account. The response tells you whether the account is ready, needs a credential fix, is waiting out a temporary failure, or is blocked on X.
...
<CardGroup cols={1}>
  ...
  <Card title="Needs credentials" icon="key-round">
    `health: "needsReauth"` means the account needs fresh credentials or a resolved security challenge. Use [Re-authenticate](/api-reference/x-accounts/reauth) before retrying writes.
  </Card>
  <Card title="Temporary recovery" icon="refresh-cw">
    `health: "temporaryIssue"` means wait or use [Bulk retry](/api-reference/x-accounts/bulk-retry). `health: "recovering"` means Xquik will retry on the next account use.
  </Card>
  <Card title="X restriction" icon="shield-alert">
    `health: "locked"` or `health: "suspended"` means resolve the account on X first, then re-authenticate or reconnect when the account is usable again.
  </Card>
