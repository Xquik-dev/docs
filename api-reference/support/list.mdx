...
keywords: ["support ticket API", "list support tickets", "support ticket status", "API support tickets", "Xquik support", "ticket history"]
---
...
This endpoint returns ticket summaries. It never returns message bodies or
attachment metadata. Use [Get Ticket](/api-reference/support/get) for the full
conversation and its private attachments.
...
Use the list as a compact support queue. Store ticket IDs and timestamps before
...
| Current state | `status` | Route open, in-progress, resolved, and closed cases. |
...
state changes. These routes preserve the existing ticket ID.
...
No. It returns `messageCount`, not message text. Fetch the ticket by `publicId`
to read its chronological conversation.

### Does the List Include Attachments?

No. Attachment filenames, types, sizes, states, and download URLs appear only
in the detailed ticket response.
