...
The `tone` field accepts any non-empty string. The response preserves the
...
<ParamField body="styleUsername" type="string">
  Account-scoped username or custom label. `@` is optional. Used by `compose`.
</ParamField>
...
Runs one deterministic input check. Checks that the draft contains text.
It does not score quality or predict ranking.

```json
{
  "checklist": [
    { "factor": "Draft contains text", "passed": true }
  ],
...
Yes. Send an analyzed username or custom label in `styleUsername`. Usernames
may start with `@`. A match returns `styleTweets`. Use those samples to study
