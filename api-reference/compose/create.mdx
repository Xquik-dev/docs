...
description: "Get editorial rules, refine a writing plan, and check a post draft. Free"
api: "POST /compose"
...
Use 3 calls. The endpoint returns guidance and checks. It does not write or
publish the final post.

## Workflow

1. Call `compose` with a topic.
2. Call `refine` with the topic, goal, and tone.
3. Write a draft. Call `score` with the full text.
...
<ParamField body="additionalContext" type="string">
  Optional audience, constraint, or source context for `refine`.
</ParamField>
...
<ResponseField name="nextStep" type="string" required>
  Exact fields required for the `refine` call.
</ResponseField>
...
```json
{
  ...
  "nextStep": "All 9 checks passed. Present the post with the intentUrl above for one-click posting.",
  "passed": true,
  "passedCount": 9,
...
<Note>
  Save accepted text with [Create Draft](/api-reference/drafts/create). Analyze
  a reference voice with [Analyze Style](/api-reference/styles/analyze).
</Note>
