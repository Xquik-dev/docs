...
description: "Cache recent tweet writing samples for one X username. Reuse fresh profiles, refresh stale samples, inspect authors, timestamps, and exact Tweet IDs safely."
api: "POST /styles"
keywords: ["tweet writing", "Twitter writing style", "X writing style", "tweet writing samples", "Twitter brand voice"]
---
...
Send a 1-15 character username with or without one leading `@`. Xquik removes
the prefix and lowercases the value. Do not send spaces or a profile URL.
...
| --- | --- | --- | --- |
| `username` | string | Yes | Send a 1-15 character X handle. `@` is optional. |

Missing, empty, or non-string values return `400 invalid_input`. Preserve the
...
| Hashtags | Count `#` tokens | Does it label topics explicitly? |
...
| `402` | `no_cached_style` | No cache exists and refresh cannot be funded | Top up or save approved examples. |
...
<ParamField body="username" type="string" required>
  A 1-15 character X username. `@` is optional. Xquik stores lowercase.
</ParamField>
...
No cache exists and the refresh cannot be funded. Top up through the
...
The route reused an existing cache. Check `fetchedAt`. A `200` can also be a
stale fallback when refresh funding is unavailable.

### Can I force a Twitter style refresh?

No force parameter exists. Fresh profiles return from cache. Delete and
recreate a profile only when that destructive workflow is appropriate.

### Can I analyze another Twitter account?

Yes, for recent public tweets. The cached profile remains isolated to your
authenticated Xquik account.
