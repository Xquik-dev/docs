...
api: "PUT /styles/{id}"
keywords: ["tweet writing", "save tweet style", "Twitter writing style", "X brand voice", "tweet writing samples", "social media style guide", "tweet examples", "custom tweet style API"]
---
...
The `{id}` path value controls the stored profile key. Xquik trims it and
converts it to lowercase before saving.

The body `label` is optional. Older clients may still send it. When present,
it must match `{id}`, ignoring case.
...
Spaces are valid inside `{id}`. Encode them in the URL.
...
Sending the same path ID replaces the entire saved Tweet array. It also resets
...
To rename a profile, create the new path ID. Then delete the old profile.
...
Custom samples contain local sequential IDs. Therefore, they cannot provide
...
<ParamField path="id" type="string" required>
  Saved profile name containing 1-50 characters. Use letters, numbers, spaces,
  dots, hyphens, apostrophes, or underscores. Xquik trims and lowercases it.
</ParamField>
...
<ParamField body="label" type="string">
  Optional compatibility label. It follows the path rules and must match `{id}`.
</ParamField>
...
<ResponseField name="xUsername" type="string">
  Normalized lowercase value from the path `{id}`.
</ResponseField>
...
| `400` | `invalid_input` | Path ID, label, or Tweet array failed validation | Correct the rejected field. |
...
| --- | --- |
| Blank or invalid path `{id}` | Send a supported profile name. |
| Non-string body `label` | Remove it or send the path ID. |
...
{
  "error": "invalid_input",
  "message": "Label can only contain letters, numbers, spaces, dots, hyphens, apostrophes, and underscores."
}
```
...
- Put the saved profile name in the path `{id}`.
...
Yes. Send the same normalized path ID with the complete replacement array.
The operation replaces every stored example under that account-scoped key.

### Does the path ID rename a tweet style?

No. The path ID controls the stored key. Create the new key, then delete the
old one.
