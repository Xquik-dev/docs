---
title: "Save Custom Tweet Styles & Writing Rules for Posts"
sidebarTitle: "Save custom style"
description: "Save a custom tweet writing-style profile from supplied post text. Reuse its tone, vocabulary, and sentence patterns during composition. See examples."
api: "PUT /styles/{id}"
...
| --- | --- | --- |
| Style ID | Path `{id}` | Use the same label for later retrieval. |
| Style label | Request `label` | Keep the human-readable writing profile name. |
| Source tweets | Request `tweets[].text` | Review every supplied post before saving. |
| Normalized key | Response `xUsername` | Use this lowercase value for profile lookup. |
| Saved tweet count | Response `tweetCount` | Confirm every intended example was stored. |
| Custom profile | Response `isOwnAccount` | Expect `false` for supplied tweet examples. |
| Saved at | Response `fetchedAt` | Record the style-profile refresh time. |
