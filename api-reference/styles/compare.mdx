...
keywords: ["tweet writing", "compare tweet styles", "Twitter writing style", "X brand voice", "tweet writing samples"]
---
...
The response preserves request order. `style1` matches `username1`. `style2`
matches `username2`.
...
<CodeGroup>
  ...
  const response = await fetch(
    `https://xquik.com/api/v1/styles/compare?${params}`,
    {
      method: "GET",
      headers: {
...
Both query values accept custom labels. X usernames may start with `@`. Do not
send numeric database IDs.
...
| `xUsername` | Lowercase X username or custom label | Preserve each profile key. |
...
the current stored profiles until another route changes them.
...
<ParamField query="username1" type="string" required>
  First X username or saved custom label. `@` is optional. Returns as `style1`.
</ParamField>

<ParamField query="username2" type="string" required>
  Second X username or saved custom label. `@` is optional. Returns as `style2`.
</ParamField>
