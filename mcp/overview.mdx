...
The agent writes code like `async xquik => xquik.request('/api/v1/radar')`. Authentication is injected, and required idempotency headers are injected automatically. The server reuses each generated key for bounded transient retries. After an unresolved write failure, verify state. Start a new attempt only when `safe_to_retry` is true.
