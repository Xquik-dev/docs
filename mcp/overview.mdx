...
An `execute` call can run for up to 55 seconds. API requests stop after 45 seconds. The reserve lets completed work return. Disconnecting cancels the call. Timeouts do not cancel durable extraction jobs. Find the job through `GET /extractions`, then resume with its stored ID. Results use snake_case fields, Unix timestamps, structured errors, `has_more`, and `next_cursor`. Rejections include `error.status` when available.
