...
Xquik exposes 118 of its 123 REST operations through 2 [Model Context Protocol](https://modelcontextprotocol.io) tools. Agents search a compact catalog with `explore`, then run authenticated requests with `xquik`.
...
Create and manage API keys in the dashboard. Use `POST /api/v1/credits/topup` when an MCP workflow needs a checkout URL. Never ask an agent to send saved-payment details or manage the credential authenticating its own MCP session.
