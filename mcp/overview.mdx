...
An `execute` call can run for up to 55 seconds. Disconnecting cancels the call. A timeout does not cancel a durable extraction job. Find it through `GET /extractions`, then resume with its stored ID. Repeat writes only when the response marks the retry safe.
