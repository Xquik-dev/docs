...
<CardGroup cols={2}>
  ...
  <Card title="Server version" icon="badge-check">
    Current API MCP server version: `2.6.42`.
  </Card>
...
Native mode does not expose `search` or `execute`. It also sends no global model
instructions. The larger tool catalog uses more model context than Code Mode.
Choose Code Mode when the client supports it.
