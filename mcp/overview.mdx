...
Xquik API MCP v2.6.41 exposes documentation search and a scoped REST catalog through 3 [Model Context Protocol](https://modelcontextprotocol.io) tools. Full credentials see 118 JSON or text routes. Private support media downloads use REST. Guest `paid_reads` keys see exactly 33 GET routes.
...
<Warning>
  Codex CLI 0.147.0 and newer preserve the RFC 9207 `iss` value. Older Codex
...
<CardGroup cols={2}>
  ...
  <Card title="Server version" icon="badge-check">
    Current API MCP server version: `2.6.41`.
  </Card>
...
- **API key** (`x-api-key` or `Authorization: Bearer xq_your_api_key_here`): This is an Xquik-specific fallback, not an OAuth token. Do not apply OAuth discovery or refresh rules. Use it only with secure header storage. Full account keys expose 118 catalog routes. Active guest keys expose 33 `paid_reads` GET routes.
...
The MCP server uses 3 tools. API discovery and execution run in isolated sandboxes:

<CardGroup cols={3}>
  <Card title="docs" icon="book-open">
    Search canonical public documentation. Read-only. No credits.
  </Card>
  <Card title="search" icon="search">
    Search the authenticated catalog. Full credentials see 118 routes. Guest keys see 33 GET routes. No network calls. No credits.
  </Card>
...
`server/discover` sends no global model instructions. Tool descriptions explain code mechanics only.
`docs` searches the public documentation server. `search` reads the OpenAPI-derived catalog. OpenAPI and these docs define endpoint behavior.
...
For a guest `paid_reads` session, `execute` is read-only, idempotent, and limited to live calls across the 33 eligible GET routes.
...
The agent writes code like `async () => xquik.request('/api/v1/radar')`. Authentication is injected, and required idempotency headers are injected automatically. The server reuses each generated key for bounded transient retries. After an unresolved write failure, verify state. Start a new attempt only when `safe_to_retry` is true.
...
| [Codex](https://learn.chatgpt.com/docs/extend/mcp) | OAuth 2.1 or environment-backed API key | Codex CLI 0.147.0 and newer preserve RFC 9207 `iss`; older or failing builds use an environment-backed key |
...
Works with Claude Code, Cursor, GitHub Copilot, Codex, Windsurf, VS Code, Gemini CLI, and other Skill-capable agents. It covers MCP tools and 129 REST API operations.
