...
Read `X-Request-Id` on MCP responses, including authentication and protocol errors.
Include it and the UTC time when reporting failures.
It identifies the HTTP request, not the JSON-RPC message or authenticated session.
Allowed browser clients can send and read this header.
