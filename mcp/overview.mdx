...
`docs` searches the public documentation server. `search` accepts an async arrow function; `spec` is global and its first argument. OpenAPI and these docs define endpoint behavior.
...
Native mode omits `search` and `execute`, sends no global model instructions, and
uses more model context. Choose Code Mode when supported.
