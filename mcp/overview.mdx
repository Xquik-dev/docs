...
ARD lists MCP and REST with search examples and domain-bound identity.
The agent catalog adds the public A2A docs agent, capabilities, OAuth metadata, and `https://xquik.com/auth.md`.
`auth.md` covers CIMD, DCR, PKCE, and the `mcp:tools` scope.
...
Authentication and required idempotency headers are injected automatically. The server reuses each generated key for bounded transient retries. Verify unresolved writes. Retry only when `safe_to_retry` is true.

An `execute` call can run for up to 55 seconds. API requests stop after 45 seconds. The reserve lets completed work return. Disconnecting cancels the call. Timeouts do not cancel durable extraction jobs. Find the job through `GET /extractions`, then resume with its stored ID. Rejections include `error.status` when available.
