...
<Note>
  ...
  Every X write requires one. Private DMs and bookmarks also require one. See [Connect X account](/api-reference/x-accounts/connect).
</Note>
...
Authentication and required idempotency headers are injected automatically. The server reuses each generated key for bounded transient retries. After an unresolved write failure, verify state. Start a new attempt only when `safe_to_retry` is true.

An `execute` call can run for up to 55 seconds. API requests stop after 45 seconds. The sandbox uses the reserve to return completed work. Disconnecting cancels the call. A timeout does not cancel a durable extraction job. Find it through `GET /extractions`, then resume with its stored ID. Repeat writes only when the response marks the retry safe. `xquik.request()` automatically uses the normalized v1 contract. Responses use snake_case fields, date-time fields as Unix seconds, structured error objects, `has_more`, and `next_cursor`. A default REST `createdAt` field becomes `created`, not `created_at`, in MCP results.
