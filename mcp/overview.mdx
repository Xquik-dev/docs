...
Xquik API MCP v2.6.38 exposes a scoped REST catalog through 2 [Model Context Protocol](https://modelcontextprotocol.io) tools. Full credentials see 118 JSON or text routes. Private support media downloads use REST. Guest `paid_reads` keys see exactly 33 GET routes.
...
<CardGroup cols={2}>
  ...
  <Card title="Server version" icon="badge-check">
    Current API MCP server version: `2.6.38`.
  </Card>
...
`explore` reads the OpenAPI-derived catalog. OpenAPI and these docs define endpoint behavior.
