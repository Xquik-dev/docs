...
description: Connect AI agents to Xquik API MCP v2.5.2 via Streamable HTTP
...
Xquik API MCP v2.5.2 exposes a credential-scoped REST catalog through 2 [Model Context Protocol](https://modelcontextprotocol.io) tools. Full account keys and OAuth tokens see 118 operations. Guest `paid_reads` keys see exactly 33 eligible GET routes.
...
<CardGroup cols={2}>
  ...
  <Card title="Server version" icon="badge-check">
    Current API MCP server version: `2.5.2`.
  </Card>
...
- **API key** (`x-api-key` or `Authorization: Bearer xq_your_api_key_here`): Full account keys expose 118 operations. Active guest keys expose only 33 `paid_reads` GET routes.
...
<Tabs>
  ...
  <Tab title="Codex CLI">
    ...
    [mcp_servers.xquik]
    url = "https://xquik.com/mcp"
    http_headers = { "x-api-key" = "xq_your_api_key_here" }
    ```
  </Tab>
</Tabs>
