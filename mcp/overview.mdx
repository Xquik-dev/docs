...
Supported browsers expose the website's tool search as a declarative WebMCP form. It performs a read-only GET and never starts an extraction.
...
Supported browsers discover `open_xquik_tool_search` on `https://xquik.com`. Pass a `query` string with 1 to 200 characters. The read-only tool opens matching website tools. It needs no authentication or credits. The page uses the current `document.modelContext` API.
