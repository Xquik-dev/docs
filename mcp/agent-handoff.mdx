...
<CardGroup cols={2}>
  ...
  <Card title="API MCP" icon="terminal">
    ...
    reads, extraction jobs, monitors, webhooks, or writes. It exposes 118 of
    123 REST operations and requires `x-api-key` or OAuth 2.1.
  </Card>
...
API-key lifecycle operations and direct saved-payment mutations are not in the MCP catalog. Manage keys in the dashboard. Use `POST /api/v1/credits/topup` for a checkout URL instead of calling quick top-up from an agent.
