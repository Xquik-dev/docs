...
<CardGroup cols={2}>
  ...
  <Card title="API MCP" icon="terminal">
    Use API MCP v2.5.1 at `https://xquik.com/mcp` when the agent needs account
    actions, live X reads, extraction jobs, monitors, webhooks, or writes.
    Streamable HTTP exposes 118 of 123 REST operations and requires `x-api-key`
    or OAuth 2.1.
  </Card>
...
API-key lifecycle operations and direct saved-payment mutations are not in the MCP catalog. Manage keys in the dashboard. Call `POST /api/v1/credits/topup` only after the user explicitly requests a checkout URL. Never call quick top-up from an agent.
