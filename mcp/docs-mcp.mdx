...
<CardGroup cols={2}>
  ...
  <Card title="API MCP server" icon="terminal">
    ...
    eligible GET routes. API MCP supports MCP `2026-07-28` over
    Streamable HTTP.
  </Card>
</CardGroup>
...
`POST https://xquik.com/ask` exposes the public docs index through NLWeb 0.54.
Use it when a client supports NLWeb instead of MCP. It needs no authentication.

```bash
curl https://xquik.com/ask \
  -H 'content-type: application/json' \
  --data '{
    "query": {"text": "How does MCP OAuth work?"},
    "prefer": {"mode": "list", "streaming": false},
    "meta": {"api_version": "0.54"}
  }'
```

Responses contain `_meta` and schema.org `TechArticle` results. Set
`prefer.response_format` to `chatgpt_app`, `conv_search`, or
`conversational_search`.

Set `prefer.streaming` to `true` for SSE. You can also send
`Accept: text/event-stream`. Streams emit `start`, `result`, and `complete`.
The `complete` event contains the full response.

NLWeb searches public documentation only. Never send secrets or private data.
...
<Tabs>
  ...
  <Tab title="Codex">
    ...
    Codex CLI 0.147.0 and newer support Xquik OAuth. If an older release reports `Authorization server response missing required issuer: expected https://xquik.com`, upgrade or keep Docs MCP connected and follow [Codex and Goose OAuth issuer validation](/guides/troubleshooting#codex-oauth-issuer-validation-error).
  </Tab>
  <Tab title="Cursor">
    ```json
    {
