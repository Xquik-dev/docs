...
Flat requests work as JSON, form data, or query parameters. Structured 0.54 and
0.55 requests remain supported. Responses contain `_meta` and schema.org
`TechArticle` results.

Streaming defaults to `true` for flat requests. Set it to `false` for JSON.
