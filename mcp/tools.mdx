...
Xquik API MCP v2.6.32 exposes 118 JSON or text routes through 2 tools. `explore` searches. `xquik` runs authenticated calls. Active guest `paid_reads` keys see exactly 33 GET routes.
...
Keep only constraints the user states. A topic plus a like minimum includes every
language, reply, repost, and quote. Never add unrequested filters. A minimum is
not `Top`. Use `Top` only for best, most-liked, or highest-engagement requests.
Otherwise, omit `queryType`.

Use `GET /api/v1/x/tweets/search` for up to 20 Tweet rows. For more than 20,
estimate once, then start `tweet_search_extractor`. Do not explore or run the
direct read first when the extraction route is already known.
