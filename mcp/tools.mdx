...
| Discover tweet routes | `search` over `spec.paths` | Save the chosen method, path, and parameters. |
...
<CardGroup cols={2}>
  ...
  <Card title="Spec access" icon="search">
    The function reads `spec.paths`. Each path groups its allowed HTTP methods.
    OpenAPI references are resolved inline.
  </Card>
</CardGroup>
...
async () => Object.entries(spec.paths).flatMap(([path, methods]) =>
  Object.entries(methods).flatMap(([method, operation]) =>
    operation?.summary?.toLowerCase().includes('tweet')
...
Tool output stops at 24,000 characters. Oversized output keeps its prefix and ends with `[TRUNCATED]`. The response stays readable, so clients can retry without a parser failure. Return fewer fields, fewer rows, or an aggregate. Use REST, SDKs, or extraction exports to persist every row.
