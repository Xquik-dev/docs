...
Xquik API MCP v2.6.42 supports 2 explicit tool modes. Code Mode uses `docs`, `search`, and `execute`. Native mode exposes `docs` plus one tool for each eligible OpenAPI operation. Active guest `paid_reads` keys see exactly 33 GET routes in either mode.
...
credentials receive 119 tools: `docs` and 118 API operations. Guest keys receive
34 tools: `docs` and 33 eligible GET operations.

Native tool names match OpenAPI `operationId` values. For example,
`getExtraction`, `createExtraction`, and `estimateExtraction` use the same path,
query, JSON body, validation, authentication, billing, and response contracts as
their REST operations. Clients read each generated JSON Schema from
`tools/list`. The server embeds no model prompt, routing table, sample count, or
endpoint preference.

Use `https://xquik.com/mcp` for the smaller Code Mode catalog. Use native mode
when the MCP client expects one tool per operation. Binary download and credential
lifecycle operations remain REST-only in both modes.

| Agent task | MCP call | Durable output |
| --- | --- | --- |
| Search product docs | `docs` with a documentation query | Keep source links with cited guidance. |
| Discover tweet routes | `search` over `spec.endpoints` | Save the chosen method, path, and parameters. |
...
Use `search` to query all 118 catalog routes and their response shapes.
