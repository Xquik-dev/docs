...
Native tool names match OpenAPI `operationId` values. Schemas and contracts come
from OpenAPI. `estimateExtraction` is read-only and creates no job. Clients read
each generated JSON Schema from `tools/list`. The server embeds no model prompt,
routing table, sample count, or endpoint preference.
