...
description: API MCP v2.5.5 tools, scoped catalogs, sandbox contracts, pagination, and compact workflows
...
Xquik API MCP v2.5.5 exposes a scoped catalog through 2 tools. `explore` searches. `xquik` runs authenticated calls. Full credentials see 119 catalog routes. Of these, 118 return JSON or text through MCP. Active guest `paid_reads` keys see exactly 33 GET routes.
...
```javascript
async () => {
  // Step 1: Get editorial rules and follow-up questions.
  const compose = await xquik.request('/api/v1/compose', {
    method: 'POST',
    body: { step: 'compose', topic: 'AI agents' }
  });
  return compose;
  // Returns contentRules, followUpQuestions, and published signal names.
