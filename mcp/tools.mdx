...
Xquik API MCP v2.5.6 exposes a scoped catalog through 2 tools. `explore` searches. `xquik` runs authenticated calls. Full credentials see 120 catalog routes. Of these, 119 return JSON or text through MCP. Active guest `paid_reads` keys see exactly 33 GET routes.
