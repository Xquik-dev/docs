...
<CardGroup cols={2}>
  ...
  <Card title="Spec access" icon="search">
    ...
    OpenAPI refs are resolved inline. Return only needed fields. Read `parameters`
    and `requestBody` before `execute`. Query responses only when needed.
  </Card>
</CardGroup>
...
```javascript
async () => {
  const { operationId, parameters, requestBody, summary } =
    spec.paths['/api/v1/x/tweets/search']?.get ?? {};
  return { operationId, parameters, requestBody, summary };
}
```
...
Tool output stops at 24,000 characters. Project large arrays to requested fields before returning them. Oversized structured output returns `response_too_large` with the original size, limit, and retry guidance. It never returns partial JSON. Retry with fewer rows, shorter strings, or an aggregate. Use REST, SDKs, or extraction exports to persist every row.
