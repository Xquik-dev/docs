...
description: API MCP v2.5.3 tools, scoped catalogs, sandbox contracts, pagination, and compact workflows
...
Xquik API MCP v2.5.3 exposes a credential-scoped catalog through 2 tools: `explore` searches, and `xquik` executes authenticated calls. Full account keys and OAuth tokens see 118 operations. Active guest `paid_reads` keys see exactly 33 eligible GET routes.
