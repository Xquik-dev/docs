...
The limit bounds tool output, not catalog access or available results. Resolved response schemas can exceed it. Oversized output returns `isError: true` with a recoverable `response_too_large` diagnostic. It includes size, limit, and retry guidance, never partial JSON. An ordinary result containing an `error` field is not a tool failure.
Retry with selected fields, fewer rows, shorter strings, or an aggregate. Use REST, SDKs, or extraction exports to persist every row.
Output errors do not undo completed API actions or charges. Retrieve existing results instead of repeating completed writes.
