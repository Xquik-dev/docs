...
Xquik API MCP v2.6.35 exposes 118 JSON or text routes through 2 tools. `explore` searches. `xquik` runs authenticated calls. Active guest `paid_reads` keys see exactly 33 GET routes.
...
Tool output is limited to 24,000 characters. If a result includes `[TRUNCATED]`, request fewer rows, project fields, and shorten long text while preserving each row's URL. Use REST, SDKs, or extraction exports to persist every full row.
