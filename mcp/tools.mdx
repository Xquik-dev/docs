...
Xquik API MCP v2.6.38 exposes 118 JSON or text routes through 2 tools. `explore` searches. `xquik` runs authenticated calls. Active guest `paid_reads` keys see exactly 33 GET routes.
...
Pass `next_cursor` unchanged for later pages.
