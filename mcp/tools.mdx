...
Xquik API MCP v2.6.34 exposes 118 JSON or text routes through 2 tools. `explore` searches. `xquik` runs authenticated calls. Active guest `paid_reads` keys see exactly 33 GET routes.
...
language, reply, repost, and quote. Never add unrequested filters. Use `Top` only
for best, most-liked, or highest-engagement requests; otherwise omit it.

Use `GET /api/v1/x/tweets/search` for 20 Tweet rows or fewer, including exactly 20. A count alone never requests an extraction.
Use extractions only above 20, for saved output, or when explicitly requested. The create route handles required preflight.
