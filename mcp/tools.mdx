...
description: API MCP v2.5.1 tools, sandbox contracts, pagination, and compact workflows
...
Xquik API MCP v2.5.1 exposes 118 of 123 REST operations through 2 tools: `explore` searches a compact catalog, and `xquik` executes authenticated calls. Connect through Streamable HTTP at `https://xquik.com/mcp` with `x-api-key` or OAuth 2.1 Bearer auth.

## explore

Search the compact 118-operation API catalog. `explore` makes no network calls and consumes no credits. The call still requires MCP authentication through an API key or OAuth Bearer token. Use it to inspect paths, parameters, costs, and response shapes before execution.
...
### Operations unavailable through MCP

The REST contract documents 123 operations. MCP exposes 118 and omits 5 browser or session-bound operations:
...
Manage API keys in the dashboard. Call `POST /api/v1/credits/topup` only after the user explicitly requests checkout, then return its URL. Do not attempt `/api/v1/credits/quick-topup` or the redirect route through `xquik.request()`.
...
- **402 / `no_subscription` / `subscription_inactive`**: Report the billing state. Call `POST /api/v1/subscribe` only after the user explicitly asks to continue, then return its URL.
- **402 / `no_credits` / `insufficient_credits`**: Report the available context. Call `POST /api/v1/credits/topup` only after the user explicitly asks, then wait for payment before retrying. Direct saved-payment quick top-up is unavailable through MCP.
