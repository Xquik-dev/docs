...
description: API MCP v2.5.4 tools, scoped catalogs, sandbox contracts, pagination, and compact workflows
...
Xquik API MCP v2.5.4 exposes a credential-scoped catalog through 2 tools: `explore` searches, and `xquik` executes authenticated calls. Full account keys and OAuth tokens see 118 operations. Active guest `paid_reads` keys see exactly 33 eligible GET routes.
...
The REST contract documents 126 operations. A full account key or OAuth token exposes 118 and omits 8 credential or session-bound operations:
...
> Summarize up to 100 tweets with guarded pagination (subscription required)
...
> Summarize up to 100 followers with guarded pagination (subscription required)
...
Use `explore` to browse the 118-operation MCP catalog with parameters and response shapes.
...
- **402 / `no_subscription` / `subscription_inactive`**: Report the billing state and available account actions. Ask the user to choose and confirm before calling `POST /api/v1/subscribe`.
