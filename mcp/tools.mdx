...
<CardGroup cols={2}>
  ...
  <Card title="Spec access" icon="search">
    Read `spec.paths`. Return each matching path, method, and full operation.
    Inspect its parameters, body, and responses before `execute`.
  </Card>
</CardGroup>
...
async () => ({
  ...
  method: 'GET',
  operation: spec.paths['/api/v1/x/tweets/search']?.get,
})
```
...
`xquik.request()` resolves to the endpoint body without a `data`, `body`, `result`, or `status` wrapper. A rejected request includes the numeric HTTP status in `error.status` when available. Hosted MCP injects a unique `Idempotency-Key`. The sandbox reuses that key for bounded transient retries. After an unresolved write failure, verify state. Start a new attempt only when `safe_to_retry` is true. Responses use snake_case fields, Unix timestamps, structured errors, `has_more`, and `next_cursor`.
