...
Tool output stops at 24,000 characters. Project large arrays to requested fields before returning them. Oversized output keeps its prefix and ends with `[TRUNCATED]`. The response stays readable, so clients can retry without a parser failure. Return fewer fields, fewer rows, or an aggregate. Use REST, SDKs, or extraction exports to persist every row.
