...
Xquik API MCP v2.6.37 exposes 118 JSON or text routes through 2 tools. `explore` searches. `xquik` runs authenticated calls. Active guest `paid_reads` keys see exactly 33 GET routes.
...
Keep only constraints the user states. A topic plus a like minimum includes every language, reply, repost, and quote. Use `Top` only for best, most-liked, or highest-engagement requests; otherwise omit it.

Use `GET /api/v1/x/tweets/search` for 20 Tweet rows or fewer, including exactly 20. A count alone never requests an extraction. The catalog labels extractions for saved output or requests above 20. Explicit extraction requests remain valid. The create route handles required preflight.
...
Tool output is limited to 24,000 characters. Xquik first shortens long string values after 500 characters. This preserves every projected row when the compacted structure fits. If `[TRUNCATED]` remains, do not retry automatically. Ask before another paid request with fewer rows, or a compact aggregate. Use REST, SDKs, or extraction exports to persist every full row.
...
async () => {
  ...
  // The score step checks that the draft contains text.
}
```
