...
Write and media responses also use the MCP-normalized snake_case contract. Read `tweet_id`, `write_action_id`, `media_id`, `media_url`, and `message_id` from `xquik.request()` results. REST and generated SDK pages may show camelCase fields such as `tweetId`, `writeActionId`, `mediaId`, and `messageId`; keep MCP agents on snake_case when reading tool results.
