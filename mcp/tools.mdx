...
description: API MCP v2.5.6 tools, scoped catalogs, sandbox contracts, pagination, and compact workflows
...
Xquik API MCP v2.5.6 exposes a scoped catalog through 2 tools. `explore` searches. `xquik` runs authenticated calls. Full credentials see 119 catalog routes. Of these, 118 return JSON or text through MCP. Active guest `paid_reads` keys see exactly 33 GET routes.

## explore

Search the authenticated API catalog. `explore` makes no network calls and uses no credits. Full credentials search 119 routes. Guest keys search 33 GET routes.
...
The REST contract documents 127 operations. Full credentials expose 119 MCP catalog routes. Eight credential and session operations stay outside:
...
The catalog includes private support attachment downloads. MCP rejects their binary responses. Use REST for those downloads. The other 118 routes return JSON or text.
...
Use `explore` to browse all 119 catalog routes and their response shapes.
