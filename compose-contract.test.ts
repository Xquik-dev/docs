...
describe('compose documentation contract', (): void => {
    ...
    expect(
      FOCUSED_COMPOSER_COPY.filter(
        (snippet: string): boolean =>
          !normalizedPage.includes(snippet.replace(/\s+/gu, ' ')),
      ),
    ).toEqual([]);
  });
  ...
  it('locks the OpenAPI variants and source-backed counts', (): void => {
    ...
    expect(OPENAPI).toMatch(
      /contentRules:[\s\S]*?minItems: 10[\s\S]*?maxItems: 10/gu,
    );
    expect(OPENAPI).toMatch(
      /radarRecommendations:[\s\S]*?minItems: 0[\s\S]*?maxItems: 0/gu,
    );
    expect(OPENAPI).toMatch(
      /engagementMultipliers:[\s\S]*?minItems: 26[\s\S]*?maxItems: 26/gu,
    );
    expect(OPENAPI).toMatch(
      /scorerWeights:[\s\S]*?minItems: 26[\s\S]*?maxItems: 26/gu,
    );
    expect(OPENAPI).toContain(
  ...
  it('remains synchronized with draft validation and style lookup', (): void => {
    ...
    expect({
      oneTextCheck:
        scorer === undefined ||
        (scorer.includes("factor: 'Draft contains text'") &&
          scorer.includes('totalChecks: 1')),
      sourceLimit:
        scorer === undefined ||
        scorer.includes('Ranking uses per-viewer predictions.'),
