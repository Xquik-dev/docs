...
| `schemaVersion` | number | Webhook payload schema version. Current value is `1`. |
| `deliveryId` | string | Webhook delivery attempt ID. Use this for delivery-log correlation. |
| `streamEventId` | string | Stored event ID. Use this to match the payload to event history. |
| `occurredAt` | string | ISO timestamp for when the event occurred. |
| `username` | string | X username for account monitor events. Omitted for keyword-only monitor events and `webhook.test`. |
| `query` | string | Keyword query that matched the event. Present for keyword monitor events. |
